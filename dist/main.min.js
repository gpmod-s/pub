...
"scripts_";var c=document.createElement("div");c.className="title_";c.textContent=this.l.UPDATE_SCRIPTS_TITLE;b.appendChild(c);const d=document.createElement("div");d.className="reload-row_";c=document.createElement("a");c.className="reload-btn_";c.href="/";c.textContent=this.l.RELOAD_BTN_LBL;c.addEventListener("click",h=>{h.preventDefault();window.onbeforeunload=null;location.reload()});c.addEventListener("auxclick",h=>{h.preventDefault()});d.appendChild(c);const e=new Set;Array.from(a.values()).forEach((h,
f)=>{var g=`${h.url}?${Date.now()}.user.js`;const k=document.createElement("div");k.className="oudated-script_";var l=document.createElement("div");l.textContent=h.name;k.appendChild(l);l=document.createElement("div");l.textContent=`${h.version} \u2794 ${h.latestVersion}`;k.appendChild(l);h=document.createElement("div");k.appendChild(h);const m=document.createElement("a");m.className="script-update-link_";m.href=g;m.target="_blank";m.textContent=this.l.UPDATE_SCRIPT_LBL;g=n=>{e.add(f);e.size>=a.size&&
...
b);this.data=m;this.background=n;this.auth=c;this.author=d;this.title=e;this.date=new Date(h);this.eraserAlpha=!p;this.fi=h&&this.date.getTime()>GPTimelapsePlayer_.FV2_DATE?5:4;this.container=f;this.container.isPlayerRendered=!0;this.mainContainer=g;this.defaultDisplayMode=k;this.defaultDisplayModeDensity=this.getModeDensity(k);this.cache;this.canvas;this.ctx;this.isPlaying=!1;this.currentSpeed=JSON.parse(window.localStorage.getItem("gp-timelapse-player-speed"))||GPTimelapsePlayer_.SPEED;this.speed=
...
let n;self.background&&(g.fillStyle=d,g.fillRect(0,0,f.width,f.height),self.draw(g,this.background,h,!1),n=g.getImageData(0,0,f.width,f.height),g.clearRect(0,0,f.width,f.height));for(let p=0;p<m;p++){var k=p*l;k=self.data.slice(k,k+l);self.draw(g,k,h,self.eraserAlpha);k=g.getImageData(0,0,f.width,f.height);this.cache[h].push(k)}self.background&&this.cache[h].push(n)}}let b,c,d,e;self.data;self.draw;self.cache=[];self.addEventListener("message",({data:h})=>{"init"===h.type?(self.data=h.data,self.background=
...
this.preventAlbumContentInteraction=this.preventAlbumContentInteraction.bind(this);this.imagePreviewOverHandler=this.imagePreviewOverHandler.bind(this);this.imagePreviewLeaveHandler=this.imagePreviewLeaveHandler.bind(this);this.imagePreviewDownHandler=this.imagePreviewDownHandler.bind(this);this.handlePreviewImage=this.handlePreviewImage.bind(this);this.addEventListener("user_updated",()=>{this.saveBanlist()});this.addEventListener("user_updated",()=>{this.handleItems()});this.gpproxy.addEventListener("game_rejoin",
()=>{this.gpproxy.isAuthorized&&this.parseUnknownUsers()});this.gpproxy.addEventListener("album_rejoin",()=>{this.gpproxy.isAuthorized&&(this.init(),this.gpproxy.isAuthorized&&(this.gpproxy.addWSMsgMiddleware([GPProxy_.EVENT.ALBUM_ITEM,GPProxy_.EVENT.ALBUM_INFO],this.middleware),this.isMiddlewareActive=!0))});this.gpproxy.addEventListener("data",({detail:{event:g}})=>{this.gpproxy.isAuthorized&&(g===GPProxy_.EVENT.GAME_STARTED?this.parseUnknownUsers():g===GPProxy_.EVENT.GALLERY_SETTINGS?(this.gpproxy.addWSMsgMiddleware([GPProxy_.EVENT.ALBUM_ITEM,
GPProxy_.EVENT.ALBUM_INFO],this.middleware),this.isMiddlewareActive=!0,this.init()):this.isMiddlewareActive&&[GPProxy_.EVENT.NEW_GAME,GPProxy_.EVENT.ALBUM_SCORE,GPProxy_.EVENT.GAME_QUIT,GPProxy_.EVENT.GAME_STARTED].includes(g)&&(this.gpproxy.removeWSMsgMiddleware(this.middleware),this.isMiddlewareActive=!1,this.terminate()))});this.gpproxy.addEventListener("albums_ended",()=>{if(this.gpproxy.isAuthorized&&this.gpproxy.isOwner()){var g=this.gpproxy.getUsers().filter(({nick:k,viewer:l})=>!this.gpproxy.isYou(k)&&
!l&&!this.banlist.has(this.formatUsername(k))).map(({nick:k})=>k);this.usersDB.addUsers(g)}});this.gpproxy.addEventListener("users_updated",({detail:{users:g}})=>{this.clearPlayers(g);this.renderUsers(g);this.requestUserDatesCreated(g)});this.gpproxy.addEventListener("owner_status",()=>{this.updatePlayers()});document.addEventListener("_user_dates_created_updated",({detail:{userDatesCreated:g}})=>{this.updateUserDatesCreatedCache(g);this.renderUsers()});document.addEventListener("_tw_ids_usernames",
({detail:{usernamesMap:g}})=>{this.updateUserIdsCache(g)});document.addEventListener("_url_changed",({detail:{path:g}})=>{this.isPreviewScreen(g)&&(document.addEventListener("pointerover",this.imagePreviewOverHandler),document.addEventListener("pointerdown",this.imagePreviewDownHandler),document.addEventListener("_url_changed",()=>{document.removeEventListener("pointerover",this.imagePreviewOverHandler);document.removeEventListener("pointerdown",this.imagePreviewDownHandler)},{once:!0}),this.searchPreviewImage().then(this.handlePreviewImage))});
window.addEventListener("message",g=>{"https://gpmod.github.io"!==g.origin&&"https://localhost"!==g.origin||"gp-twitch-token"!==g.data?.type||(this.twitchAuthWindow.postMessage({type:"close"},g.origin),this.twitchAuthWindow=null,this.s.twitchToken=g.data.token)});this.ac&&this.ac.addEventListener("trigger",this.acTriggerHandler.bind(this));this.initTwitchTokenBtn();this.render();this.initInitialState();this.s.banTwitchUsers&&this.gpproxy.isTwitchAuth()&&this.validateTwitchToken()}getElement(){return this.container}init(){document.addEventListener("pointerdown",
