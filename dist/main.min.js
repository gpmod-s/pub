...
!1),km:Number(c.m??!1),at:d,af:Number(e)}}))});document.addEventListener("_auth_id",async({detail:{authId:c,authTypeId:d,isAuthorized:e}})=>{function f(n){this.lm=this.lm.filter(q=>1<<q.id&n?(q.disabled||q.initialized||this.initModuleInstance(q,this.root,this.loc.get(q.name)),q.locked=!1):q.locked=!0);this.renderModulesList()}async function g(n,q){q=(new TextEncoder).encode(q);var t=await crypto.subtle.digest("SHA-256",q);q=window.atob(n).slice(0,12);q={name:"AES-GCM",iv:new Uint8Array(Array.from(q).map(v=>
v.charCodeAt(0)))};t=await crypto.subtle.importKey("raw",t,q,!1,["decrypt"]);n=window.atob(n).slice(12);n=new Uint8Array(Array.from(n).map(v=>v.charCodeAt(0)));try{const v=await crypto.subtle.decrypt(q,t,n);return(new TextDecoder).decode(v)}catch(v){throw new GPDecryptError_;}}function h(n){document.dispatchEvent(new CustomEvent("_auth_perms",{detail:{ps:n}}))}async function k(n){n=await g(n,l());return fetch("https://id.twitch.tv/oauth2/validate",{headers:{Authorization:`OAuth ${n}`},credentials:"omit",
cache:"no-cache"}).then(q=>{if(q.ok)return q.json();throw Error();}).then(q=>[q.user_id,q.login]).catch(()=>[])}function l(){function n(x,y){const z=q();return n=function(C,L){C-=195;let I=z[C];void 0===n.IvNImK&&(n.biwoho=function(E){let D="",G="";for(let H=0,K,J,M=0;J=E.charAt(M++);~J&&(K=H%4?64*K+J:J,H++%4)?D+=String.fromCharCode(255&K>>(-2*H&6)):0)J="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789+/=".indexOf(J);for(let H=0,K=D.length;H<K;H++)G+="%"+("00"+D.charCodeAt(H).toString(16)).slice(-2);
...
{this.requestTwitchToken()}}),new GPAuthError_;}else throw new GPAuthError_;const n=1===d?"t":"d";m=JSON.parse(await g(a,this.publicKey));const q=JSON.parse(await g(m[n],c));m.e&&this.setEditionTitle(m.e,m.m);m.g&&(r=[GPModulesManager_.ICON.GIFT]);if(q.ed){if(q.ed<Date.now())throw r=[GPModulesManager_.ICON.EXPIRED],2&q.p&&(p|=16),new GPAuthError_;r=[GPModulesManager_.ICON.TIME_LEFT,q.ed-Date.now()]}else m.g||(r=[GPModulesManager_.ICON.SUBSCRIPTION]);r&&this.setEditionState(...r);f.call(this,p|q.p);
h.call(this,p|q.p);this.renderModulesList()}catch(n){n instanceof GPAuthError_||n instanceof GPDecryptError_||console.error(n),e?r??=[GPModulesManager_.ICON.SUBSCRIBE]:this.publicKey&&(r=[GPModulesManager_.ICON.AUTH]),r&&this.setEditionState(...r),f.call(this,p),h.call(this,p)}}this.dispatchEvent(new CustomEvent("modules-loaded",{detail:{decryptedData:m,authTypeId:d,isAuthorized:e}}))}else this.setEditionState(GPModulesManager_.ICON.ANALYTICS),this.root.abt?.showAnalyticsNotice()});document.addEventListener("_auth_key_changed",
