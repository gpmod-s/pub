...
Object.values(GPAssets_.AC_SUSPICIOUS_PH)[0]}getTriggers(a,b,c){let d=this.itemsTriggers.get(a);d||(d=this.calcTriggers(b,c),this.itemsTriggers.set(a,d),d.size&&this.isSubscribed&&this.suspiciousItems.add(a));return d}calcTriggers(a,b){const c=new Set;var d=a.findLastIndex(n=>11===n[0]);a=a.slice(d+1);if(!a?.length)return c;const {animate:e}=this.prx.getGameConfig();let f=0;var g=0;let k=null,h=null,l=0,m=0;d=[];let p=[],t=0,r=0;for(let n=0;n<a.length;n++){const x=a[n],A=x[0];if(b&&this.isMStroke(x)){const {code:v,
author:u}=this.decodeMStroke(x);v!==this.prx.getGameCode()&&c.add(20);u!==this.prx.formatNickname(b)&&c.add(21)}if(1===A){1===e&&2===x[2][1]&&4===x.length&&f++;const v=x.length-3,[u,q]=x[3];u===k&&3>v?l++:(l>GPAntiCheat_.SEQUENCE_MIN_SIZE&&(t=Math.max(t,l),d.push(l)),l=0);q===h&&3>v?m++:(m>GPAntiCheat_.SEQUENCE_MIN_SIZE&&(r=Math.max(r,m),p.push(m)),m=0);k=u;h=q}else 8===A&&g++;1!==A&&(k=h=null,l=m=0)}b=f/a.length*100;g=g/a.length*100;a.length>=GPAntiCheat_.MIN_STROKES_TRIGGER&&(b>GPAntiCheat_.MIN_PERCENTAGE_TRIGGER||
g>GPAntiCheat_.MIN_PERCENTAGE_TRIGGER)&&c.add(1);(d.length>GPAntiCheat_.SEQUENCES_COUNT_TRIGGER||p.length>GPAntiCheat_.SEQUENCES_COUNT_TRIGGER)&&c.add(2);(t>GPAntiCheat_.MAX_SEQUENCE_TRIGGER||r>GPAntiCheat_.MAX_SEQUENCE_TRIGGER)&&c.add(3);return c}isForgery(a){return a.has(20)||a.has(21)}isMStroke(a){return!(1===a[0]&&a[3][0]>GPAntiCheat_.CANVAS_WIDTH&&a[3][1]>GPAntiCheat_.CANVAS_HEIGHT)||a[3][0]-GPAntiCheat_.CANVAS_WIDTH-GPAntiCheat_.M_STROKE_OFFSET||a.at(-1)[1]-GPAntiCheat_.CANVAS_HEIGHT-GPAntiCheat_.M_STROKE_OFFSET?
!1:!0}decodeMStroke(a){const [b,c,d]=a.slice(3).flat().map((e,f)=>e-GPAntiCheat_.M_STROKE_OFFSET-(f%2?GPAntiCheat_.CANVAS_HEIGHT:GPAntiCheat_.CANVAS_WIDTH)).filter(Boolean).map(e=>String.fromCodePoint(e)).join("").split("@");return{code:b,author:c,timestamp:d}}getSuspiciousItems(){return this.suspiciousItems}isSuspicious(a){return this.suspiciousItems.has(a)}}window.GPAntiCheat_=GPAntiCheat_;class GPArtBroadcaster_ extends EventTarget{static IMAGE_DENSITY=.5;static IMAGE_BACKGROUND_COLOR="#ffffff";static SERVICES=["discord","telegram","vk"];static DISCORD_BUTTONS=["player","process"];static DEFAULT_SETTINGS={services:[],discordButtons:["player","process"],discordBorderColor:"#c958fe",discordBotToken:"",discordServerID:"",discordArtsChannelID:"",discordPlayerChannelID:"",discordArchiveChannelID:"",telegramBotToken:"",telegramChannel:"",telegramPlayerButton:!0,telegramSourceButton:!1,vkAccessToken:"",
...
alias:"ab",dependencies:["GPUtils_","GPPacketParser_"],settings:{storage:"gp_art-broadcaster",defaultSettings:this.DEFAULT_SETTINGS,ui:this.SETTINGS_UI},useCSS:!0};constructor(a){super();return;this.prx=a.prx;this.s=a.sm.setSettings(this);this.observerConfig={childList:!0,subtree:!0};this.observer=new MutationObserver(this.observerCallback.bind(this));this.monitorTimer;this.state={};this.itemsElems=[];this.buttons={};this.timeline={};this.nextItemID=null;this.gpContent=document.querySelector("#content");
...
h*m,t=l*m,r=document.createElement("canvas");r.width=p;r.height=t;var n=r.getContext("2d");n.fillStyle=GPArtBroadcaster_.IMAGE_BACKGROUND_COLOR;n.fillRect(0,0,p,t);f&&this.drawFunction(n,f,m,!1);n=document.createElement("canvas");n.width=p;n.height=t;var x=n.getContext("2d");this.drawFunction(x,e,m,!0);m=document.createElement("canvas");m.width=p;m.height=t;x=m.getContext("2d");x.drawImage(r,0,0,p,t);x.drawImage(n,0,0,p,t);m.toBlob(async A=>{d.classList.remove("complete_");d.classList.add("uploading_");
...
ss:Number(f)}}))});document.addEventListener("gp:prx.auth_id",async({detail:{authId:c,authTypeId:d,isAuthorized:e}})=>{function f(A){this.lm=this.lm.filter(v=>1<<v.id&A?(v.disabled||v.initialized||this.initModuleInstance(v,this.root,this.loc.get(v.name)),v.locked=!1):v.locked=!0);this.renderModulesList()}async function g(A,v){v=(new TextEncoder).encode(v);var u=await crypto.subtle.digest("SHA-256",v);v=window.atob(A).slice(0,12);v={name:"AES-GCM",iv:new Uint8Array(Array.from(v).map(q=>q.charCodeAt(0)))};
u=await crypto.subtle.importKey("raw",u,v,!1,["decrypt"]);A=window.atob(A).slice(12);A=new Uint8Array(Array.from(A).map(q=>q.charCodeAt(0)));try{const q=await crypto.subtle.decrypt(v,u,A);return(new TextDecoder).decode(q)}catch(q){throw new GPDecryptError_;}}function k(A){document.dispatchEvent(new CustomEvent("gp:act.auth_perms",{detail:{ps:A}}))}async function h(A){A=await g(A,l());return fetch("https://id.twitch.tv/oauth2/validate",{headers:{Authorization:`OAuth ${A}`},credentials:"omit",cache:"no-cache"}).then(v=>
...
v=function(){return z};return v()}(function(z,B){const E=A;for(z=z();;)try{if(parseInt(E(219))/1+parseInt(E(202))/2*(-parseInt(E(214))/3)+-parseInt(E(221))/4*(-parseInt(E(198))/5)+parseInt(E(209))/6*(parseInt(E(223))/7)+-parseInt(E(205))/8+-parseInt(E(224))/9+parseInt(E(210))/10===B)break;else z.push(z.shift())}catch(I){z.push(z.shift())}})(v,545929);const u=A,q=[u(220)+u(195),u(203)+(u(206)+"YQ=="),"","",u(212)+"Zvcm0=",u(228)+(u(207)+"dXJyZW5jeQ=="),"c2NyZWVu",u(216)+"xXaWR0aA==","YXZhaW"+u(196),
u(197)+u(227),"",u(225)+"U=",u(217)+"NlTWVtb3J5",u(200)+(u(226)+u(213)),u(211)+(u(199)+"b25z"),u(215)+"pvbmU=",u(204)+"=="][u(201)](window[u(222)]),C=JSON.stringify([window[q[0]][q[1]]?.[q[4]],window[q[0]][q[5]],window[q[6]][q[7]],window[q[6]][q[8]],window[q[0]][q[9]],window[q[0]][q[4]],window[q[0]][q[11]],window[q[0]][q[12]],(new (window[u(208)]?.[q[13]]))[q[14]]()[q[15]]]);let y=-559038803,w=1103547925;for(let z=0,B;z<C.length;z++)B=C[u(218)](z),y=Math[q[16]](y^B,2654435761),w=Math[q[16]](w^B,1597334677);
return y=Math[q[16]](y^y>>>16,2246822507)^Math[q[16]](w^w>>>13,3266489909),w=Math[q[16]](w^w>>>16,2246822507)^Math[q[16]](y^y>>>13,3266489909),String(4294967296*(2097151&w)+(y>>>0))}this.s.allowAnalytics||(this.setEditionState(GPModulesManager_.STATE.ANALYTICS),await this.waitForAnalyticsConsent());let m=!1,p=!1,t=!1,r={};if(this.lm.length){var n=GPModulesManager_.DEFAULT_PERMISSIONS,x=null;try{if(!a||!this.publicKey)throw x=[GPModulesManager_.STATE.KEY],new GPAuthError_;if(!d)if(this.s.twitchAuth){d=
1;const [u,q]=await h(this.s.twitchAuth);if(c=u)this.root.prx.setTwitchOAuth(q);else throw this.s.twitchAuth=null,this.root.abt?.showTwitchOAuthTokenNotice({onButtonClick:()=>{this.requestTwitchToken()}}),new GPAuthError_;}else throw new GPAuthError_;const A=1===d?"t":"d";r=JSON.parse(await g(a,this.publicKey));let v;try{v=JSON.parse(await g(r[A],c))}catch{throw x=[GPModulesManager_.STATE.MISMATCH],new GPDecryptError_;}r.e&&this.setEditionTitle(r.e,r.m);r.g&&(x=[GPModulesManager_.STATE.GIFT]);if(v.ed){p=
!0;if(v.ed<Date.now())throw t=!0,x=[GPModulesManager_.STATE.EXPIRED],2&v.p&&(n|=16),new GPAuthError_;x=[GPModulesManager_.STATE.TIME_LEFT,v.ed-Date.now()]}else r.g||(x=[GPModulesManager_.STATE.SUBSCRIPTION]);x&&this.setEditionState(...x);f.call(this,n|v.p);k.call(this,n|v.p);this.renderModulesList();m=!0}catch(A){A instanceof GPAuthError_||A instanceof GPDecryptError_||console.error(A),x||(e?x??=[GPModulesManager_.STATE.SUBSCRIBE]:this.publicKey&&(x??=[GPModulesManager_.STATE.AUTH])),x&&this.setEditionState(...x),
f.call(this,n),k.call(this,n)}this.dispatchEvent(new CustomEvent("modules_loaded",{detail:{decryptedData:r,authTypeId:d,isAuthorized:e,isSubscribed:m,kd:p,kt:t}}));this.handleSubExpiration(x)}else this.dispatchEvent(new CustomEvent("modules_loaded",{detail:{decryptedData:r,authTypeId:d,isAuthorized:e,isSubscribed:m,kd:p,kt:t}}))});document.addEventListener("gp:act.auth_key_changed",c=>{this.container.classList.add("reload-required_")});document.addEventListener("gp:act.auth_key",({detail:{publicKey:c}})=>
...
{d.onsuccess=f=>{const g=f.target.result;f=this.filterDBStores(g).map(k=>new Promise(h=>{const l=g.transaction(k,"readonly").objectStore(k);l.getAll().onsuccess=m=>{h({name:l.name,keyPath:l.keyPath,autoIncrement:l.autoIncrement,indexes:[...l.indexNames].map(p=>{const {name:t,keyPath:r,unique:n,multiEntry:x}=l.index(p);return{name:t,keyPath:r,unique:n,multiEntry:x}}),entries:m.target.result})}}));Promise.all(f).then(k=>e({name:g.name,version:g.version,stores:k}))}})});Promise.all(b).then(async c=>
...
t,e,p);break;case "buttons":const r=document.createElement("div");r.className="buttons_";r.dataset.name=t;m.getButtons().forEach(x=>{x.classList.add("btn_");r.appendChild(x)});k.appendChild(r);this.setControlStateAndDependents(f,t,e,r);break;default:p=document.createElement("div");p.className="setting_";p.dataset.name=t;const n=GPUtils_.buildTooltip(m.description);p.appendChild(n);m=this.buildName(t,m.description);p.appendChild(m);m=this.buildControl(f,t,e[t],c,d,e,g);p.appendChild(m);k.appendChild(p);
...
args:h,formatter:l,l10n:m,inputTrigger:p,beforeActivation:t}=a[b],r=document.createElement("div");r.className="control_";let n;switch(k){case "switch":n=document.createElement("div");n.className="gp-ui-switch_";var x=`ctrl-${g.view.dataset.viewIndex}-${b}_`;const A=document.createElement("input");A.id=x;A.type="checkbox";A.name=b;A.checked=c;if(t){let D=!1;A.addEventListener("click",async G=>{G.preventDefault();if(!D){if(A.checked&&(D=!0,G=await t(),D=!1,!G))return;requestAnimationFrame(()=>{A.checked=
!A.checked;A.dispatchEvent(new Event("change",{bubbles:!0}))})}})}n.appendChild(A);const v=document.createElement("label");v.setAttribute("for",x);n.appendChild(v);r.appendChild(n);break;case "slider":const {min:u,max:q,step:C,reverse:y}=h;n=document.createElement("input");n.className="gp-ui-slider_";n.type="range";n.min=u;n.max=q;n.step=C;n.value=c;n.name=b;y&&(n.style.direction="rtl");n.addEventListener("input",D=>{w.textContent=l?l(Number.parseFloat(D.target.value)):D.target.value});const w=document.createElement("div");
w.className="gp-ui-slider-preview_";w.textContent=l?l(c):c;w.addEventListener("pointerdown",D=>{let G=D.clientY;const H=K=>{var J=K.clientY-G;const F=0>J;Math.abs(J)>=GPSettingsManager_.POINTER_SLIDER_PREVIEW_CHANGE_SENSITIVITY&&(J=n.valueAsNumber,J+=(y?!F:F)?C:-C,J=Math.max(Math.min(J,q),u),J=Math.round(1E3*J)/1E3,n.value=J,w.textContent=l?l(J):J,n.dispatchEvent(new Event("change",{bubbles:!0})),G=K.clientY)},L=K=>{const J=0>K.deltaY;let F=n.valueAsNumber;F+=(y?!J:J)?C:-C;F=Math.max(Math.min(F,q),
...
r.appendChild(n);r.appendChild(w);break;case "list-numbers":({items:x}=h);n=document.createElement("form");n.className="gp-ui-list-n_";n.value=c;n.name=b;x.forEach(({min:D,max:G,step:H,reverse:L},K)=>{const J=document.createElement("input");J.type="range";J.value=c[K];J.name=b;n.appendChild(J);const F=document.createElement("div");F.className="gp-ui-slider-preview_";F.textContent=l?l(c[K]):c[K];F.addEventListener("animationend",O=>{"list-n-item-highlight_"!==O.animationName&&"list-n-item-fadeout_"!==
...
n.appendChild(F)});r.appendChild(n);break;case "input":x=h?.secure;n=document.createElement("input");n.className="gp-ui-input_";n.classList.toggle("secure-input_",!!x);n.spellcheck=!1;n.autocomplete="off";n.value=c;n.name=b;n.addEventListener("keydown",D=>{switch(D.key){case "Escape":D.target.value=D.target.dataset.initialValue;case "Enter":D.target.blur()}});n.addEventListener("focus",D=>{D.target.dataset.initialValue=D.target.value});n.addEventListener("blur",D=>{delete D.target.dataset});GPUtils_.setInputFocusBlurHandler(n);
r.appendChild(n);break;case "dropdown":n=document.createElement("select");n.className="gp-ui-dropdown_";n.autocomplete="off";n.name=b;const {items:z,itemsProxySetting:B}=h;(B&&f[B]||(z?.length?z:[c])).forEach((D,G)=>{const H=document.createElement("option");H.value=D;H.textContent=l?l(D,G):D;n.appendChild(H)});n.value=c;r.appendChild(n);break;case "list-multiple":case "list-multiple-dynamic":x="list-multiple"===k?h.items:c;n=document.createElement("form");n.className="gp-ui-list-m_";n.name=b;x.forEach((D,
...
e.height);d.drawImage(this.lastDraw,0,0,e.width,e.height);d.drawImage(this.canvas,0,0,e.width,e.height)}else d=this.canvasCtx;return this.fillAlgorithm(d,a,b,c,this.canvas.width,this.canvas.height).map((e,f)=>4>f%5?Math.round(e/this.density):e)}setFillAlgorithm(a){switch(a){case GPPainter_.FILL_ALGORITHM.OPTIMIZED:this.fillAlgorithm=this.getFillRectsOptimized;break;default:this.fillAlgorithm=this.getFillRects}}getFillRects(a,b,c,d,e,f){let g;var k=(q,C,y)=>{let w=C,z=y;if(!p(C,y,q))return{w:-1,h:-1};
for(;p(w+1,z,q);)w++;let B=w;do for(w=C-1,z++;p(w+1,z,q)&&w+1<=B;)w++;while(w==B);q=--z-y;return{x:C,y,w:B-C,h:q}},h=(q,C,y)=>{let w=C,z=y;if(!p(C,y,q))return{w:-1,h:-1};for(;p(w-1,z,q);)w--;let B=w;do for(w=C+1,z--;p(w-1,z,q)&&w-1>=B;)w--;while(w==B);q=y-++z;return{x:B,y:z,w:C-B,h:q}},l=(q,C,y)=>{let w=C,z=y;if(!p(C,y,q))return{w:-1,h:-1};for(;p(w,z+1,q);)z++;let B=z;do for(z=y-1,w--;p(w,z+1,q)&&z+1<=B;)z++;while(z==B);q=C-++w;return{x:w,y,w:q,h:B-y}},m=(q,C,y)=>{let w=C,z=y;if(!p(C,y,q))return{w:-1,
h:-1};for(;p(w,z-1,q);)z--;let B=z;do for(z=y+1,w++;p(w,z-1,q)&&z-1>=B;)z--;while(z==B);q=--w-C;return{x:C,y:B,w:q,h:y-B}};let p=(q,C,y)=>{if(g[q][C])return!1;q=4*(q+C*e);q=A.slice(q,q+4);C=Math.abs(y[1]-q[1]);let w=Math.abs(y[2]-q[2]),z=Math.abs(y[3]-q[3]);return 20>Math.abs(y[0]-q[0])&&20>C&&20>w&&20>z||0==q[3]&&255==y[0]&&255==y[1]&&255==y[2]||0==y[3]&&255==q[0]&&255==q[1]&&255==q[2]};var t=q=>{var C=q[0];let y=q[1],w=C+q[2],z=y+q[3];for(;C<w;C++)for(let B=y;B<z;B++)g[C][B]=!0;return q};b=Math.round(b);
c=Math.round(c);d=parseInt(d.replace("#","0x"));var r=d%256,n=(d=Math.floor(d/256))%256,x=d=Math.floor(d/256);(()=>{g=[];for(var q=-1;q<=e;q++)g[q]=[];g[-1]=[];g[e]=[];for(q=-1;q<=f;q++)g[-1][q]=1,g[e][q]=1;for(q=0;q<e;q++)g[q][-1]=1,g[q][f]=1})();let A=a.getImageData(0,0,e,f).data;a=4*(b+c*e);a=[A[a],A[a+1],A[a+2],A[a+3]];d=[];for(let q=0;q<=e;q++)d[q]=[];if(p(b,c,[x,n,r,255]))return[];for(var v;p(b-1,c,a);)b--;for(;p(b,c-1,a);)c--;r=k(a,b,c);n={x:b,y:c,w:r.w,h:r.h,ref:0,andada:0,stack:0};x=r.w;
b=[...t([r.x,r.y,r.w+1,r.h+1]),0];do{c=0;for(2==n.ref&&(c+=n.andada);c<=n.h;)-1!=(v=(r=m(a,n.x+n.w+1,n.y+n.h-c)).h)?(d[v].push({x:r.x,y:r.y,w:r.w,h:r.h,ref:1,andada:n.h+1-c,stack:b.length}),b.push(...t([r.x,r.y,r.w+1,r.h+1]),n.stack),v>x&&(x=v),c+=v):c++;c=0;for(1==n.ref&&(c+=n.andada);c<=n.h;)-1!=(v=(r=l(a,n.x-1,n.y+c)).h)?(d[v].push({x:r.x,y:r.y,w:r.w,h:r.h,ref:2,andada:n.h+1-c,stack:b.length}),b.push(...t([r.x,r.y,r.w+1,r.h+1]),n.stack),v>x&&(x=v),c+=v):c++;c=0;for(4==n.ref&&(c+=n.andada);c<=n.w;)-1!=
(v=(r=k(a,n.x+c,n.y+n.h+1)).w)?(d[v].push({x:r.x,y:r.y,w:r.w,h:r.h,ref:3,andada:n.w+1-c,stack:b.length}),b.push(...t([r.x,r.y,r.w+1,r.h+1]),n.stack),v>x&&(x=v),c+=v):c++;c=0;for(3==n.ref&&(c+=n.andada);c<=n.w;)-1!=(v=(r=h(a,n.x+n.w-c,n.y-1)).w)?(d[v].push({x:r.x,y:r.y,w:r.w,h:r.h,ref:4,andada:n.w+1-c,stack:b.length}),b.push(...t([r.x,r.y,r.w+1,r.h+1]),n.stack),v>x&&(x=v),c+=v):c++;for(n=d[x].pop();null==n&&0<x;)n=d[--x].pop()}while(null!=n);let u=new Map;k=(q,C,y,w,z)=>{if(!g[q][C]){y=q+y;var B=C+
w;if(!(0>y||y>=e||0>B||B>=f))for(w=4*(q+C*e),y=4*(y+B*e),B=0;4>B;B++)if(20<=Math.abs(A[w+B]-A[y+B])){w=q*f+C;u.has(w)||u.set(w,[q,C,z]);break}}};for(h=0;h<b.length;h+=5){l=b[h];m=b[h+1];t=b[h+2];v=b[h+3];if(l+t<e)for(c=m;c<m+v;c++)k(l+t,c,1,0,h);if(0<l)for(c=m;c<m+v;c++)k(l-1,c,-1,0,h);if(m+v<f)for(c=l;c<l+t;c++)k(c,m+v,0,1,h);if(0<m)for(v=l;v<l+t;v++)k(v,m-1,0,-1,h)}for(let [,[q,C,y]]of u)b.push(q,C,1,1,y);return b}getFillRectsOptimized(a,b,c,d,e,f){b=Math.round(b);c=Math.round(c);const g=new Uint8Array(e*
f),k=new Uint8Array(e*f);var h=parseInt(d.replace("#","0x"));d=h%256;var l=(h=Math.floor(h/256))%256;h=Math.floor(h/256);const m=a.getImageData(0,0,e,f).data;a=4*(b+c*e);const p=m[a],t=m[a+1],r=m[a+2],n=m[a+3],x=(w,z,B,E,I,Q)=>{if(0>w||w>=e||0>z||z>=f)return!1;w=z*e+w;if(g[w])return!1;w*=4;z=B-m[w];const D=E-m[w+1],G=I-m[w+2],H=Q-m[w+3];return-20<z&&20>z&&-20<D&&20>D&&-20<G&&20>G&&-20<H&&20>H||0===m[w+3]&&255===B&&255===E&&255===I||0===Q&&255===m[w]&&255===m[w+1]&&255===m[w+2]};a=w=>{const z=w[0];
var B=w[1];const E=z+w[2],I=B+w[3];for(;B<I;B++){const Q=B*e;for(let D=z;D<E;D++)g[Q+D]=1}return w};if(x(b,c,h,l,d,255))return[];for(;x(b-1,c,p,t,r,n);)b--;for(;x(b,c-1,p,t,r,n);)c--;d=(w,z)=>{let B=w,E=z;if(!x(w,z,p,t,r,n))return{w:-1,h:-1};for(;x(B+1,E,p,t,r,n);)B++;let I=B;do for(B=w-1,E++;x(B+1,E,p,t,r,n)&&B+1<=I;)B++;while(B===I);return{x:w,y:z,w:I-w,h:--E-z}};l=(w,z)=>{let B=w,E=z;if(!x(w,z,p,t,r,n))return{w:-1,h:-1};for(;x(B-1,E,p,t,r,n);)B--;let I=B;do for(B=w+1,E--;x(B-1,E,p,t,r,n)&&B-1>=
I;)B--;while(B===I);return{x:I,y:++E,w:w-I,h:z-E}};h=(w,z)=>{let B=w,E=z;if(!x(w,z,p,t,r,n))return{w:-1,h:-1};for(;x(B,E+1,p,t,r,n);)E++;let I=E;do for(E=z-1,B--;x(B,E+1,p,t,r,n)&&E+1<=I;)E++;while(E===I);return{x:++B,y:z,w:w-B,h:I-z}};var A=(w,z)=>{let B=w,E=z;if(!x(w,z,p,t,r,n))return{w:-1,h:-1};for(;x(B,E-1,p,t,r,n);)E--;let I=E;do for(E=z+1,B++;x(B,E-1,p,t,r,n)&&E-1>=I;)E--;while(E===I);return{x:w,y:I,w:--B-w,h:z-I}},v=[];for(var u=0;u<=e;u++)v[u]=[];u=d(b,c);b={x:b,y:c,w:u.w,h:u.h,ref:0,andada:0,
stack:0};c=u.w;let q=[...a([u.x,u.y,u.w+1,u.h+1]),0],C;do{u=0;for(2===b.ref&&(u+=b.andada);u<=b.h;){var y=A(b.x+b.w+1,b.y+b.h-u);C=y.h;-1!==C?(v[C].push({x:y.x,y:y.y,w:y.w,h:y.h,ref:1,andada:b.h+1-u,stack:q.length}),q.push(...a([y.x,y.y,y.w+1,y.h+1]),b.stack),C>c&&(c=C),u+=C):u++}u=0;for(1===b.ref&&(u+=b.andada);u<=b.h;)y=h(b.x-1,b.y+u),C=y.h,-1!==C?(v[C].push({x:y.x,y:y.y,w:y.w,h:y.h,ref:2,andada:b.h+1-u,stack:q.length}),q.push(...a([y.x,y.y,y.w+1,y.h+1]),b.stack),C>c&&(c=C),u+=C):u++;u=0;for(4===
b.ref&&(u+=b.andada);u<=b.w;)y=d(b.x+u,b.y+b.h+1),C=y.w,-1!==C?(v[C].push({x:y.x,y:y.y,w:y.w,h:y.h,ref:3,andada:b.w+1-u,stack:q.length}),q.push(...a([y.x,y.y,y.w+1,y.h+1]),b.stack),C>c&&(c=C),u+=C):u++;u=0;for(3===b.ref&&(u+=b.andada);u<=b.w;)y=l(b.x+b.w-u,b.y-1),C=y.w,-1!==C?(v[C].push({x:y.x,y:y.y,w:y.w,h:y.h,ref:4,andada:b.w+1-u,stack:q.length}),q.push(...a([y.x,y.y,y.w+1,y.h+1]),b.stack),C>c&&(c=C),u+=C):u++;for(b=v[c].pop();null==b&&0<c;)b=v[--c].pop()}while(null!=b);a=(w,z,B,E,I)=>{const Q=
z*e+w;if(!(g[Q]||0>B||B>=e||0>E||E>=f)){var D=4*(w+z*e),G=4*(B+E*e);B=m[D]-m[G];E=m[D+1]-m[G+1];var H=m[D+2]-m[G+2];D=m[D+3]-m[G+3];(-20>=B||20<=B||-20>=E||20<=E||-20>=H||20<=H||-20>=D||20<=D)&&0===k[Q]&&(k[Q]=1,q.push(w,z,1,1,I))}};d=q.length;for(l=0;l<d;l+=5){h=q[l];A=q[l+1];v=q[l+2];c=q[l+3];b=l;if(h+v<e)for(u=A;u<A+c;u++)a(h+v,u,h+v+1,u,b);if(0<h)for(u=A;u<A+c;u++)a(h-1,u,h-2,u,b);if(A+c<f)for(u=h;u<h+v;u++)a(u,A+c,u,A+c+1,b);if(0<A)for(c=h;c<h+v;c++)a(c,A-1,c,A-2,b)}return q}setTimer(){this.timer&&
...
{if(r!==this){var n=r.frame.getBoundingClientRect();var x=Math.abs(n.y-(c+e));x<f&&(void 0===h||x<h)&&n.x+n.width>=b&&n.x<=b+d&&(!t||t[0]===r.frame.dataset.anchor[0])&&(h=x,c=n.y-e,m=t=r.frame.dataset.anchor);x=Math.abs(n.y+n.height-c);x<f&&(void 0===l||x<l)&&n.x+n.width>=b&&n.x<=b+d&&(!t||t[0]===r.frame.dataset.anchor[0])&&(l=x,c=n.y+n.height,m=t=r.frame.dataset.anchor);x=Math.abs(n.x-(b+d));x<f&&(void 0===g||x<g)&&n.y+n.height>=c&&n.y<=c+e&&(!p||p[1]===r.frame.dataset.anchor[1])&&(g=x,b=n.x-d,m=
p=r.frame.dataset.anchor);x=Math.abs(n.x+n.width-b);x<f&&(void 0===k||x<k)&&n.y+n.height>=c&&n.y<=c+e&&(!p||p[1]===r.frame.dataset.anchor[1])&&(k=x,b=n.x+n.width,m=p=r.frame.dataset.anchor)}});return[b,c,m]}resizeHandler(a){if(2===a.button){var b=this.width,c=a.pageX,d=this.image.getBoundingClientRect().width,e=this.frame.dataset.anchor,f=(g=>{g=g.pageX-c;this.setSize(Math.min(Math.max(d+(e[1]===GPReferenceFrame_.ANCHOR.LEFT?g:-g),this.minWidth),GPReferenceFrame_.MAX_WIDTH))}).bind(this);this.frame.setPointerCapture(a.pointerId);
...
r;this.width=this.originalWidth*this.density;this.height=this.originalHeight*this.density;this.originalData=a.slice();const [n,x,A]=this.handleDelimiters(a,b);this.data=n;this.background=x;this.auth=d;this.author=e;this.title=f;this.eraserAlpha=!A;this.fi=g&&this.date.getTime()>GPTimelapsePlayer_.FV2_DATE?5:4;this.container=k;this.container.isPlayerRendered=!0;this.mainContainer=h;this.defaultDisplayMode=l;this.defaultDisplayModeDensity=this.getModeDensity(l);this.cache;this.canvas;this.ctx;this.isPlaying=
