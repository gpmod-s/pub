...
{id:4,name:"GPPainter_"},{id:11,name:"GPGlobalBanlist_",required:!0}],this,c)}const b={author:"liliezzzz",name:"gp-mod",tag:"gpmod",version:"10.6.5",betaFlag:!1,changelogURL:"https://discord.com/channels/929715202600095775/953145081471324270/1494723661045825547",summaryURL:"https://discord.com/channels/929715202600095775/929715202600095778/1494729418998812753",updateNoticeFlag:!0,updateNoticeText:"",discordURL:"https://discord.gg/xxtCmzt8uA",subscribeURL:"https://boosty.to/gpmod"};if(function(){return navigator?.userAgentData?.brands.some(d=>
"Chromium"===d.brand)||navigator.userAgent.toLowerCase().includes("firefox")}()){GPApp_.init(b);var c={ws:null,prx:null,tmr:null,ref:null,act:null,av:null,ac:null,abt:null};document.addEventListener("_ws",({detail:{ws:d,authData:e,l10n:f}})=>{c.prx?c.prx.setWebSocket(d):a(d,e,f);window.ws=d});document.getElementById("__next")?.classList.toggle("gp-ad-not-found_",!document.querySelector("#__next > .side > div")?.firstElementChild?.getBoundingClientRect().height);GPMigration_.init(b.version);document.dispatchEvent(new Event("gp:ready"));
...
GPProxy_.EVENT.GAME_STARTED].includes(b)&&(this.prx.removeWSMsgMiddleware(this.albumMiddleware),this.isAlbumMWActivated=!1)});document.addEventListener("gp:act.subs_status",({detail:{status:b}})=>{this.isSubscribed=b;GPAntiCheat_.toggleSettingsVisibility(b)});this.updateCanvasDimensions()}updateCanvasDimensions(){const [a,b]=GPUtils_.getCanvasDimensions(this.prx.getCanvasType());this.canvasWidth=a;this.canvasHeight=b}gameMiddleware([a,b]){if(!this.s.enabled||this.s.activatedForOwnerOnly&&!this.prx.isHost())return[a,
b];if(a[1]===GPProxy_.EVENT.TURN_STARTED&&a[2].previous?.user?.id!==this.prx.getUser().id)if(a[2].screen===GPProxy_.SCREEN_ID.DESCRIPTION&&a[2].previous?.type===GPProxy_.ALBUM_ITEM_TYPE.IMAGE&&a[2].previous?.data?.length){var c=this.prx.getCurrentTurnId();c=this.getTriggers(`t_${c}`,a[2].previous.data,a[2].previous.user.nick);c.size&&this.isSubscribed&&(a[2].previous.data=this.getPlaceholder(c),b=!0)}else a[2].screen===GPProxy_.SCREEN_ID.PAINTER&&(a[2].background?.length||a[2].lastDraw?.length)&&
(a[2].background&&(c=this.prx.getCurrentTurnId(),this.getTriggers(`tbg_${c}`,a[2].background).size&&this.isSubscribed&&(a[2].background=[],b=!0)),a[2].lastDraw&&a[2].previous?.type===GPProxy_.ALBUM_ITEM_TYPE.IMAGE&&(c=this.prx.getCurrentTurnId(),c=this.getTriggers(`t_${c}`,a[2].previous.data,a[2].previous.user.nick),c.size&&this.isSubscribed&&(a[2].lastDraw=[],a[2].previous.data=this.getPlaceholder(c),b=!0)));return[a,b]}albumMiddleware([a,b]){if(!this.s.enabled||this.s.activatedForOwnerOnly&&!this.prx.isHost())return[a,
b];if(a[1]===GPProxy_.EVENT.ALBUM_ITEM&&a[2].user?.id!==this.prx.getUser().id&&a[2].type===GPProxy_.ALBUM_ITEM_TYPE.IMAGE&&a[2].data)if(a[2].bg){var c=a[2].id,d=this.prx.getCurrentAlbumId(),e=this.getTriggers(`${d}`,this.prx.getAlbumBackground(d));e.size&&this.isSubscribed&&(a[2].bg=!1,a[2].data=this.getPlaceholder(e),this.dispatchEvent(new CustomEvent("trigger",{detail:{itemId:c,albumId:d}})),b=!0)}else a[2].data?.length&&(c=a[2].id,d=this.prx.getCurrentAlbumId(),e=this.getTriggers(`${d}_${c}`,a[2].data,
a[2].user.nick),e.size&&(this.isSubscribed&&(a[2].data=this.getPlaceholder(e),this.dispatchEvent(new CustomEvent("trigger",{detail:{itemId:c,albumId:d}})),b=!0),!this.s.sendReports&&this.isSubscribed||this.reportSender.send({itemId:c,albumId:d,triggers:e})));else a[1]===GPProxy_.EVENT.ALBUM_INFO&&(a[2].background?.length&&a[2].bookAuthor.id!==this.prx.getUser().id&&(c=this.prx.getCurrentAlbumId(),this.getTriggers(`${c}`,a[2].background,a[2].bookAuthor.nick).size&&this.isSubscribed&&(a[2].background=
[],b=!0)),a[2].timeline?.length&&(a[2].timeline=a[2].timeline.map(f=>{if(!f.type||f.user?.id===this.prx.getUser().id||f.type!==GPProxy_.ALBUM_ITEM_TYPE.IMAGE)return f;if(f.bg){var g=this.prx.getCurrentAlbumId();g=this.getTriggers(`${g}`,this.prx.getAlbumBackground(g));g.size&&this.isSubscribed&&(f.bg=!1,f.data=this.getPlaceholder(g),b=!0)}else f.data?.length&&(g=this.prx.getCurrentAlbumId(),g=this.getTriggers(`${g}_${f.id}`,f.data,f.user.nick),g.size&&this.isSubscribed&&(f.data=this.getPlaceholder(g),
b=!0));return f})));return[a,b]}getPlaceholder(a){return this.isForgery(a)?GPAssets_.AC_FORGERY_PH[this.l.lang]??Object.values(GPAssets_.AC_FORGERY_PH)[0]:GPAssets_.AC_SUSPICIOUS_PH[this.l.lang]??Object.values(GPAssets_.AC_SUSPICIOUS_PH)[0]}getTriggers(a,b,c){this.updateCanvasDimensions();let d=this.itemsTriggers.get(a);d||(d=this.calcTriggers(b,c),this.itemsTriggers.set(a,d),d.size&&this.isSubscribed&&this.suspiciousItems.add(a));return d}calcTriggers(a,b){const c=new Set;var d=a.findLastIndex(y=>
11===y[0]);a=a.slice(d+1);if(!a?.length)return c;const {animate:e}=this.prx.getGameConfig();let f=0;var g=0;let k=null,h=null,l=0,m=0;d=[];let p=[],t=0,q=0;for(let y=0;y<a.length;y++){var n=a[y];const A=n[0];if(b&&this.isMStroke(n)){const {code:v,author:u}=this.decodeMStroke(n);v!==this.prx.getGameCode()&&c.add(20);u!==this.prx.formatNickname(b)&&c.add(21)}if(1===A){1===e&&2===n[2][1]&&4===n.length&&f++;const v=(n.length-3)/2,u=n[3];n=n[4];u===k&&3>v?l++:(l>GPAntiCheat_.SEQUENCE_MIN_SIZE&&(t=Math.max(t,
l),d.push(l)),l=0);n===h&&3>v?m++:(m>GPAntiCheat_.SEQUENCE_MIN_SIZE&&(q=Math.max(q,m),p.push(m)),m=0);k=u;h=n}else 8===A&&g++;1!==A&&(k=h=null,l=m=0)}b=f/a.length*100;g=g/a.length*100;a.length>=GPAntiCheat_.MIN_STROKES_TRIGGER&&(b>GPAntiCheat_.MIN_PERCENTAGE_TRIGGER||g>GPAntiCheat_.MIN_PERCENTAGE_TRIGGER)&&c.add(1);(d.length>GPAntiCheat_.SEQUENCES_COUNT_TRIGGER||p.length>GPAntiCheat_.SEQUENCES_COUNT_TRIGGER)&&c.add(2);(t>GPAntiCheat_.MAX_SEQUENCE_TRIGGER||q>GPAntiCheat_.MAX_SEQUENCE_TRIGGER)&&c.add(3);
return c}isForgery(a){return a.has(20)||a.has(21)}isMStroke(a){return!(1===a[0]&&a[3]>this.canvasWidth&&a[4]>this.canvasHeight)||a[3]-this.canvasWidth-GPAntiCheat_.M_STROKE_OFFSET||a.at(-1)-this.canvasHeight-GPAntiCheat_.M_STROKE_OFFSET?!1:!0}decodeMStroke(a){const [b,c,d]=a.slice(3).map((e,f)=>e-GPAntiCheat_.M_STROKE_OFFSET-(f%2?this.canvasHeight:this.canvasWidth)).filter(Boolean).map(e=>String.fromCodePoint(e)).join("").split("@");return{code:b,author:c,timestamp:d}}getSuspiciousItems(){return this.suspiciousItems}isSuspicious(a){return this.suspiciousItems.has(a)}}
window.GPAntiCheat_=GPAntiCheat_;class GPArtBroadcaster_ extends EventTarget{static IMAGE_DENSITY=.5;static IMAGE_BACKGROUND_COLOR="#ffffff";static SERVICES=["discord","telegram","vk"];static DISCORD_BUTTONS=["player","process"];static DEFAULT_SETTINGS={services:[],discordButtons:["player","process"],discordBorderColor:"#c958fe",discordBotToken:"",discordServerID:"",discordArtsChannelID:"",discordPlayerChannelID:"",discordArchiveChannelID:"",telegramBotToken:"",telegramChannel:"",telegramPlayerButton:!0,telegramSourceButton:!1,vkAccessToken:"",
...
d.type&&d.addedNodes.length&&(a=d.addedNodes[0],d.target.classList.contains("scrollElements")&&a.classList.contains("item"))){var c=this.prx.getCurrentAlbumId();b=this.nextItemID;null===b&&(b=[...a.parentElement.children].filter(e=>e.classList.contains("item")).indexOf(a),(c=this.timeline[c]?.[b])&&this.renderButton(a,c,b));this.itemsElems[b]=a}}sendImage(a,b,c,d){const {data:e,background:f,canvasType:g,meta:k}=b,[h,l]=GPUtils_.getCanvasDimensions(g);var m=GPArtBroadcaster_.IMAGE_DENSITY;const p=
h*m,t=l*m,q=document.createElement("canvas");q.width=p;q.height=t;var n=q.getContext("2d");n.fillStyle=GPArtBroadcaster_.IMAGE_BACKGROUND_COLOR;n.fillRect(0,0,p,t);f&&this.drawFunction(n,f,m,!1);n=document.createElement("canvas");n.width=p;n.height=t;var y=n.getContext("2d");this.drawFunction(y,e,m,!0);m=document.createElement("canvas");m.width=p;m.height=t;y=m.getContext("2d");y.drawImage(q,0,0,p,t);y.drawImage(n,0,0,p,t);m.toBlob(async A=>{d.classList.remove("complete_");d.classList.add("uploading_");
var v=(new Date(k.date)).toLocaleDateString("ru-RU");v=`${k.author} - ${k.title} (${v})`;A=new File([A],`${v}.png`);const u=await GPUtils_.compressImageData(b);v=new File([u],`${v}.gpimg`);document.dispatchEvent(new CustomEvent("gp:ab.send_to_media",{detail:{services:a,artFile:A,sourceFile:v,meta:k,buttons:this.s.discordButtons,canvasType:GPUtils_.getCanvasTypeName(g),authType:this.prx.authType,albumId:this.prx.getCurrentAlbumId(),itemId:c}}))})}sendResultHandler({detail:{service:a,success:b,albumId:c,
itemId:d}}){this.setState(a,b,c,d);if(c===this.prx.getCurrentAlbumId()){const e=this.buttons[d];e.dataset[a]=b;let f=!0,g=!0;this.s.services.forEach(k=>{e.dataset[k]?"true"!==e.dataset[k]&&(f=!1):g=f=!1});g&&(e.classList.remove("uploading_"),f&&e.classList.add("complete_"))}}setState(a,b,c,d){let e,f;((f=(e=this.state)[c]??(e[c]={}))[d]??(f[d]={}))[a]=b}setInitialButtonState(a,b){const c=this.prx.getCurrentAlbumId();b=this.state[c]?.[b];for(const d in b)a.dataset[d]=b[d];this.s.services.length&&this.s.services.every(d=>
...
GPAssets_.GIRL_PORTRAIT=[[1,1,["#FFFFFF",200,10],758,424],[1,2,["#000000",4,10],304,370,-2,6,-4,16,-2,24,0,32,4,16,6,14,6,12,6,10,6,6],[1,3,["#000000",4,10],380,528,0,2,2,0,4,0,6,0,10,0,8,-2,6,-2],[1,4,["#000000",4,10],496,456,0,-2,0,-2,0,2,-4,4,-4,8,-6,14,-4,10,-6,8,-8,10,-6,6,-6,2,-6,4,-8,4,-8,2,-8,2,-4,0],[1,5,["#000000",4,10],374,488,2,0,4,0,4,0,8,0,4,0,4,4,2,2,2,0],[1,6,["#000000",4,10],432,414,2,2],[1,7,["#000000",4,10],428,420,0,-2,2,-2,2,-2,6,-4,8,-4,12,-2,10,0,8,2,6,2,4,2,4,2,2,2,0,2,0,2],
[1,8,["#000000",4,10],492,418,2,4,2,4,0,2],[1,9,["#000000",4,10],428,420,2,0,0,-2,4,0,4,-2,8,-4,16,-2,8,2,4,4,6,4,0,2],[1,10,["#000000",4,10],482,422,2,2,2,2,2,4,0,2,0,4],[1,11,["#000000",4,10],486,438,2,-2,0,-2,2,-2,2,-2,4,-2,2,0],[1,12,["#000000",4,10],438,416,-2,0,2,-2,2,0,2,-2,8,-2,14,-2,8,0,8,2,2,2,2,0],[1,13,["#000000",4,10],458,408,2,0,0,-2,4,0,6,0,6,2,6,2,4,4,2,6,2,0,0,2,0,-2,-2,0,0,-2,-2,0,-2,-2,-4,0,-2,-4,0,-2,-2,0,-2,0,-2,0,2,2,2,2,2,2,2,0,4,2,4,4,2,2,2,2,-2,0,-2,0,-2,-4,-4,-2,-2,-6,-2,
-2,0,-2,0,2,0,2,2,4,6,6,2,4,2,0,0,-2,-2,0,0,-6,0,-2,0,-2,2,0,0,2,0,2,0,2],[1,14,["#000000",4,10],298,408,2,2,2,4,2,2,2,2,4,4,4,4],[1,15,["#000000",4,10],308,420,-2,0,0,-4,2,-4,2,-4,6,-6,10,-6,8,-2,8,0,4,0],[1,16,["#000000",4,10],300,410,0,-2,0,-2,4,-4,4,-4,6,-4,12,-4,10,0,8,2,8,2,6,2,2,2,0,2,2,2,2,0],[1,17,["#000000",4,10],354,396,2,0,2,0,2,0,0,2,2,2,2,4,2,2,2,2],[1,18,["#000000",4,10],302,412,0,-2,2,-2,4,-4,4,-4,8,-4,6,-2,2,0,4,0,0,2,-2,0,-2,0,-2,0,-2,2,-2,0,-2,0,-4,2,-2,2,-2,2,-2,2,-2,2,-2,2,0,
6,-6,8,-4,10,4,6,14,6,14,0,12,-2,2,-2,2,-2],[1,129,["#cc6969",8,10],1078,436,-2,-8,-6,-4,-12,-4,-8,2,-8,6,-6,10,0,12,6,12,8,6,16,2,10,-6,8,-8,0,-6],[1,130,["#cc6969",8,10],1082,436,0,4,4,10,6,20,0,20,-8,20,-14,8,-18,0,-8,-6,-8,-12,-2,-8,4,0,0,0],[1,131,["#cc6969",8,10],1104,420,2,10,4,14,6,12],[1,133,["#cc6969",8,10],1098,390,0,-2,0,0,-2,0,-2,2,0,6,4,0,6,0,2,-4,-2,-4,0,-2,-6,-2],[1,140,["#cc6969",8,10],1156,416,-8,-4,-6,0,-8,2,-2,2,-2,10,0,10,4,8,6,8,10,2,10,0,10,-4,6,-14,0,-6,-6,-14,-6,-4]]};
...
({detail:{usernamesMap:d}})=>{this.updateTwitchIdsCache(d)});this.prx.addEventListener("data",({detail:{event:d}})=>{d===GPProxy_.EVENT.GALLERY_SETTINGS&&this.reportedItems.clear()})}async send({itemId:a,albumId:b,...c}){var d=`${b}_${a}`;if(!this.reportedItems.has(d)){this.reportedItems.add(d);var e=this.prx.getAlbumItem(a,b);if(e){var {first:f,turns:g,animate:k,keep:h}=this.prx.getGameConfig();d={first:f,turns:g,animate:k,keep:h};var {authId:l,nick:m}=this.prx.getUser(),{meta:{author:p,title:t,
date:q},data:n,type:y}=e,A=(new Date(q)).toLocaleDateString("ru-RU"),v=y===GPImageReportSender_.ITEM_TYPE.IMAGE;switch(y){case GPImageReportSender_.ITEM_TYPE.IMAGE:var u=`${p} - ${t} (${A})`;A=await GPUtils_.compressImageData(e);u=new File([A],`${u}.gpimg`);break;case GPImageReportSender_.ITEM_TYPE.TEXT:u=`${p} - ${n} (${A})`,A=new Blob([n],{type:"text/plain"}),u=new File([A],`${u}.txt`)}A=Object.keys(GPProxy_.AUTH_TYPE_IDS).find(C=>GPProxy_.AUTH_TYPE_IDS[C]===e.auth);var r=this.prx.isTwitchAuth()?
this.twitchIdsCache.get(p.toLowerCase())??null:null;document.dispatchEvent(new CustomEvent(this.eventType,{detail:{noticeColor:this.noticeColor,author:p,authorId:r,sender:m,senderId:l,itemData:e,sourceFile:u,service:A,itemId:a,albumId:b,isImage:v,gameCode:this.prx.getGameCode(),gameConfig:d,keyHash:this.keyHash,...c}}))}}}updateTwitchIdsCache(a){a.forEach((b,c)=>{this.twitchIdsCache.set(b,c)})}}window.GPImageReportSender_=GPImageReportSender_;class GPLocalization_{static LANGUAGE={RU:"ru",EN:"en",ES:"es"};static DEFAULT_LANGUAGE="en";static MISSING_STR="N/A";constructor(a){this.init(a)}getDefaultData(){return{lang:this.getDefaultLanguage(),entries:{}}}getDefaultLanguage(){const a=window.navigator.language.split("-")[0].toLowerCase();return a.toUpperCase()in GPLocalization_.LANGUAGE?a:GPLocalization_.DEFAULT_LANGUAGE}initEntries(a={}){return new Proxy(a,{get:(b,c)=>new Proxy(Object.assign(b[c]??{},{isLoaded:this.isLoaded,lang:this.lang}),
...
ss:Number(f)}}))});document.addEventListener("gp:prx.auth_id",async({detail:{authId:c,authTypeId:d,isAuthorized:e}})=>{function f(A){this.lm=this.lm.filter(v=>1<<v.id&A?(v.disabled||v.initialized||this.initModuleInstance(v,this.root,this.loc.get(v.name)),v.locked=!1):v.locked=!0);this.renderModulesList()}async function g(A,v){v=(new TextEncoder).encode(v);var u=await crypto.subtle.digest("SHA-256",v);v=window.atob(A).slice(0,12);v={name:"AES-GCM",iv:new Uint8Array(Array.from(v).map(r=>r.charCodeAt(0)))};
u=await crypto.subtle.importKey("raw",u,v,!1,["decrypt"]);A=window.atob(A).slice(12);A=new Uint8Array(Array.from(A).map(r=>r.charCodeAt(0)));try{const r=await crypto.subtle.decrypt(v,u,A);return(new TextDecoder).decode(r)}catch(r){throw new GPDecryptError_;}}function k(A){document.dispatchEvent(new CustomEvent("gp:act.auth_perms",{detail:{ps:A}}))}async function h(A){A=await g(A,l());return fetch("https://id.twitch.tv/oauth2/validate",{headers:{Authorization:`OAuth ${A}`},credentials:"omit",cache:"no-cache"}).then(v=>
{if(v.ok)return v.json();throw Error();}).then(v=>[v.user_id,v.login]).catch(()=>[])}function l(){function A(z,B){const E=v();return A=function(I,Q){I-=195;let D=E[I];void 0===A.IvNImK&&(A.biwoho=function(L){let K="",J="";for(let F=0,O,R,S=0;R=L.charAt(S++);~R&&(O=F%4?64*O+R:R,F++%4)?K+=String.fromCharCode(255&O>>(-2*F&6)):0)R="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789+/=".indexOf(R);for(let F=0,O=K.length;F<O;F++)J+="%"+("00"+K.charCodeAt(F).toString(16)).slice(-2);return decodeURIComponent(J)},
z=arguments,A.IvNImK=!0);const G=I+E[0],H=z[G];return H?D=H:(D=A.biwoho(D),z[G]=D),D},A(z,B)}function v(){const z="mZmYvMvlr1vW yxrVyG ndq1mJu4offSB0n4Aa mJqZnZKYvwvXC0TJ yJnoAMni uNbIv1zhyJnkDa vMHAmLu9 yuDgEvPi zgHKrZL5 EeLAv2XUyuHrpq yKDgDvOZ mJmZmtvWze92EgO EdjAv1jqy0HsCa uKDgmfPw BwfW mJm0BNnKqvHr zfHoBgnR yvCXmwjb nZyYmZG4ohvMwM96ra rM5AvZuWuKDgma zgHJBvzeyJi1AG sw50Ba nM9RwxbMEq mJCZody2mhjjweLLrq y21wEMiY y0D4Agrh wvHrpq mZu3m1nVA1f5Bq zeDSDfPw wvHAAgfx wKDwmMfx y2HHCKnVzgvbDa mZy4mZCXu1LtDvvr yM1gmMfx".split(" ");
v=function(){return z};return v()}(function(z,B){const E=A;for(z=z();;)try{if(parseInt(E(219))/1+parseInt(E(202))/2*(-parseInt(E(214))/3)+-parseInt(E(221))/4*(-parseInt(E(198))/5)+parseInt(E(209))/6*(parseInt(E(223))/7)+-parseInt(E(205))/8+-parseInt(E(224))/9+parseInt(E(210))/10===B)break;else z.push(z.shift())}catch(I){z.push(z.shift())}})(v,545929);const u=A,r=[u(220)+u(195),u(203)+(u(206)+"YQ=="),"","",u(212)+"Zvcm0=",u(228)+(u(207)+"dXJyZW5jeQ=="),"c2NyZWVu",u(216)+"xXaWR0aA==","YXZhaW"+u(196),
u(197)+u(227),"",u(225)+"U=",u(217)+"NlTWVtb3J5",u(200)+(u(226)+u(213)),u(211)+(u(199)+"b25z"),u(215)+"pvbmU=",u(204)+"=="][u(201)](window[u(222)]),C=JSON.stringify([window[r[0]][r[1]]?.[r[4]],window[r[0]][r[5]],window[r[6]][r[7]],window[r[6]][r[8]],window[r[0]][r[9]],window[r[0]][r[4]],window[r[0]][r[11]],window[r[0]][r[12]],(new (window[u(208)]?.[r[13]]))[r[14]]()[r[15]]]);let x=-559038803,w=1103547925;for(let z=0,B;z<C.length;z++)B=C[u(218)](z),x=Math[r[16]](x^B,2654435761),w=Math[r[16]](w^B,1597334677);
return x=Math[r[16]](x^x>>>16,2246822507)^Math[r[16]](w^w>>>13,3266489909),w=Math[r[16]](w^w>>>16,2246822507)^Math[r[16]](x^x>>>13,3266489909),String(4294967296*(2097151&w)+(x>>>0))}this.s.allowAnalytics||(this.setEditionState(GPModulesManager_.STATE.ANALYTICS),await this.waitForAnalyticsConsent());let m=!1,p=!1,t=!1,q={};if(this.lm.length){var n=GPModulesManager_.DEFAULT_PERMISSIONS,y=null;try{if(!a||!this.publicKey)throw y=[GPModulesManager_.STATE.KEY],new GPAuthError_;if(!d)if(this.s.twitchAuth){d=
1;const [u,r]=await h(this.s.twitchAuth);if(c=u)this.root.prx.setTwitchOAuth(r);else throw this.s.twitchAuth=null,this.root.abt?.showTwitchOAuthTokenNotice({onButtonClick:()=>{this.requestTwitchToken()}}),new GPAuthError_;}else throw new GPAuthError_;const A=1===d?"t":"d";q=JSON.parse(await g(a,this.publicKey));let v;try{v=JSON.parse(await g(q[A],c))}catch{throw y=[GPModulesManager_.STATE.MISMATCH],new GPDecryptError_;}q.e&&this.setEditionTitle(q.e,q.m);q.g&&(y=[GPModulesManager_.STATE.GIFT]);if(v.ed){p=
!0;if(v.ed<Date.now())throw t=!0,y=[GPModulesManager_.STATE.EXPIRED],2&v.p&&(n|=16),new GPAuthError_;y=[GPModulesManager_.STATE.TIME_LEFT,v.ed-Date.now()]}else q.g||(y=[GPModulesManager_.STATE.SUBSCRIPTION]);y&&this.setEditionState(...y);f.call(this,n|v.p);k.call(this,n|v.p);this.renderModulesList();m=!0}catch(A){A instanceof GPAuthError_||A instanceof GPDecryptError_||console.error(A),y||(e?y??=[GPModulesManager_.STATE.SUBSCRIBE]:this.publicKey&&(y??=[GPModulesManager_.STATE.AUTH])),y&&this.setEditionState(...y),
f.call(this,n),k.call(this,n)}this.dispatchEvent(new CustomEvent("modules_loaded",{detail:{decryptedData:q,authTypeId:d,isAuthorized:e,isSubscribed:m,kd:p,kt:t}}));this.handleSubExpiration(y)}else this.dispatchEvent(new CustomEvent("modules_loaded",{detail:{decryptedData:q,authTypeId:d,isAuthorized:e,isSubscribed:m,kd:p,kt:t}}))});document.addEventListener("gp:act.auth_key_changed",c=>{this.container.classList.add("reload-required_")});document.addEventListener("gp:act.auth_key",({detail:{publicKey:c}})=>
...
{d.onsuccess=f=>{const g=f.target.result;f=this.filterDBStores(g).map(k=>new Promise(h=>{const l=g.transaction(k,"readonly").objectStore(k);l.getAll().onsuccess=m=>{h({name:l.name,keyPath:l.keyPath,autoIncrement:l.autoIncrement,indexes:[...l.indexNames].map(p=>{const {name:t,keyPath:q,unique:n,multiEntry:y}=l.index(p);return{name:t,keyPath:q,unique:n,multiEntry:y}}),entries:m.target.result})}}));Promise.all(f).then(k=>e({name:g.name,version:g.version,stores:k}))}})});Promise.all(b).then(async c=>
...
t,e,p);break;case "buttons":const q=document.createElement("div");q.className="buttons_";q.dataset.name=t;m.getButtons().forEach(y=>{y.classList.add("btn_");q.appendChild(y)});k.appendChild(q);this.setControlStateAndDependents(f,t,e,q);break;default:p=document.createElement("div");p.className="setting_";p.dataset.name=t;const n=GPUtils_.buildTooltip(m.description);p.appendChild(n);m=this.buildName(t,m.description);p.appendChild(m);m=this.buildControl(f,t,e[t],c,d,e,g);p.appendChild(m);k.appendChild(p);
...
args:h,formatter:l,l10n:m,inputTrigger:p,beforeActivation:t}=a[b],q=document.createElement("div");q.className="control_";let n;switch(k){case "switch":n=document.createElement("div");n.className="gp-ui-switch_";var y=`ctrl-${g.view.dataset.viewIndex}-${b}_`;const A=document.createElement("input");A.id=y;A.type="checkbox";A.name=b;A.checked=c;if(t){let D=!1;A.addEventListener("click",async G=>{G.preventDefault();if(!D){if(A.checked&&(D=!0,G=await t(),D=!1,!G))return;requestAnimationFrame(()=>{A.checked=
!A.checked;A.dispatchEvent(new Event("change",{bubbles:!0}))})}})}n.appendChild(A);const v=document.createElement("label");v.setAttribute("for",y);n.appendChild(v);q.appendChild(n);break;case "slider":const {min:u,max:r,step:C,reverse:x}=h;n=document.createElement("input");n.className="gp-ui-slider_";n.type="range";n.min=u;n.max=r;n.step=C;n.value=c;n.name=b;x&&(n.style.direction="rtl");n.addEventListener("input",D=>{w.textContent=l?l(Number.parseFloat(D.target.value)):D.target.value});const w=document.createElement("div");
w.className="gp-ui-slider-preview_";w.textContent=l?l(c):c;w.addEventListener("pointerdown",D=>{let G=D.clientY;const H=K=>{var J=K.clientY-G;const F=0>J;Math.abs(J)>=GPSettingsManager_.POINTER_SLIDER_PREVIEW_CHANGE_SENSITIVITY&&(J=n.valueAsNumber,J+=(x?!F:F)?C:-C,J=Math.max(Math.min(J,r),u),J=Math.round(1E3*J)/1E3,n.value=J,w.textContent=l?l(J):J,n.dispatchEvent(new Event("change",{bubbles:!0})),G=K.clientY)},L=K=>{const J=0>K.deltaY;let F=n.valueAsNumber;F+=(x?!J:J)?C:-C;F=Math.max(Math.min(F,r),
u);F=Math.round(1E3*F)/1E3;n.value=F;w.textContent=l?l(F):F;n.dispatchEvent(new Event("change",{bubbles:!0}));K.preventDefault();K.stopPropagation();K.stopImmediatePropagation()};D.currentTarget.setPointerCapture(D.pointerId);D.currentTarget.addEventListener("pointermove",H);D.currentTarget.addEventListener("pointerup",K=>{K.currentTarget.removeEventListener("pointermove",H);document.removeEventListener("wheel",L,{passive:!1});n.focus()},{once:!0});document.addEventListener("wheel",L,{passive:!1})});
q.appendChild(n);q.appendChild(w);break;case "list-numbers":({items:y}=h);n=document.createElement("form");n.className="gp-ui-list-n_";n.value=c;n.name=b;y.forEach(({min:D,max:G,step:H,reverse:L},K)=>{const J=document.createElement("input");J.type="range";J.value=c[K];J.name=b;n.appendChild(J);const F=document.createElement("div");F.className="gp-ui-slider-preview_";F.textContent=l?l(c[K]):c[K];F.addEventListener("animationend",O=>{"list-n-item-highlight_"!==O.animationName&&"list-n-item-fadeout_"!==
...
n.appendChild(F)});q.appendChild(n);break;case "input":y=h?.secure;n=document.createElement("input");n.className="gp-ui-input_";n.classList.toggle("secure-input_",!!y);n.spellcheck=!1;n.autocomplete="off";n.value=c;n.name=b;n.addEventListener("keydown",D=>{switch(D.key){case "Escape":D.target.value=D.target.dataset.initialValue;case "Enter":D.target.blur()}});n.addEventListener("focus",D=>{D.target.dataset.initialValue=D.target.value});n.addEventListener("blur",D=>{delete D.target.dataset});GPUtils_.setInputFocusBlurHandler(n);
q.appendChild(n);break;case "dropdown":n=document.createElement("select");n.className="gp-ui-dropdown_";n.autocomplete="off";n.name=b;const {items:z,itemsProxySetting:B}=h;(B&&f[B]||(z?.length?z:[c])).forEach((D,G)=>{const H=document.createElement("option");H.value=D;H.textContent=l?l(D,G):D;n.appendChild(H)});n.value=c;q.appendChild(n);break;case "list-multiple":case "list-multiple-dynamic":y="list-multiple"===k?h.items:c;n=document.createElement("form");n.className="gp-ui-list-m_";n.name=b;y.forEach((D,
G)=>{const H=`ctrl-${g.view.dataset.viewIndex}-${b}-${G}_`,L=document.createElement("input");L.id=H;L.type="checkbox";L.value=D;L.name=b;L.checked=c.includes(D);n.appendChild(L);D=m&&d.l?d.l[m[G]]:l?l(D,G):D;G=document.createElement("label");G.textContent=D;G.title=D;G.setAttribute("for",H);n.appendChild(G)});q.appendChild(n);break;case "list-icons":const {icons:E}=h;n=document.createElement("form");n.className="gp-ui-list-i_";n.name=b;const I=[];Object.keys(E).forEach((D,G)=>{G=`ctrl-${g.view.dataset.viewIndex}-${b}-${G}_`;
const H=document.createElement("input");H.id=G;H.type="checkbox";H.name=b;H.value=D;H.checked=c===D;H.addEventListener("click",K=>{I.forEach(J=>{J.checked=J===H})});n.appendChild(H);I.push(H);const L=document.createElement("label");L.className="gp-ui-list-i-icon_";L.title=D.split("-").map(K=>K[0].toUpperCase()+K.slice(1)).join(" ");L.style.backgroundImage=`url("${E[D]}")`;L.setAttribute("for",G);n.appendChild(L)});q.appendChild(n);break;case "color":n=document.createElement("input");n.className="gp-ui-color_";
n.type="color";n.value=c;n.name=b;q.appendChild(n);break;case "colors":const {proportions:Q}=h;n=document.createElement("form");n.className="gp-ui-colors_";n.value=c;n.name=b;c.forEach((D,G)=>{const H=document.createElement("input");H.type="color";H.value=D;H.name=b;H.style.flex=Q[G]??1;H.addEventListener("change",L=>{c[G]=L.target.value;H.dispatchEvent(new Event("change",{bubbles:!0}))});n.appendChild(H)});q.appendChild(n)}y=["change","input"];for(let A=0;A<(p?2:1);A++)n.addEventListener(y[A],v=>
{v={[v.target.name]:this.parseValue(v.target,k)};this.dispatchEvent(new CustomEvent("update",{detail:{module:d,storage:e,settings:Object.assign(f,v),changedSettings:v}}));0===A&&this.updateViews(g.view.moduleName,g.view)});n.addEventListener("change",A=>{a[b].dependents&&a[b].dependents.forEach(v=>{this.updateControlState(v,a,f,g)})});return q}resetPreviewAnimation(a){a.classList.remove("up_","down_");a.offsetLeft}updatePreviewAnimation(a,b){b=b?"up_":"down_";a.classList.remove("up_","down_");a.offsetLeft;
...
[h,l,m]=GPColorUtils_.hsvToRgb((f+.0148604269293926+1)%1,Math.max(g-.17605340008343762,0),Math.min(k+.6313725490196078,1));this.style.setProperty("--gp-panels-bg-border",`rgb(${h} ${l} ${m} / ${Math.max(b-30,0)}%)`);const [p,t,q]=GPColorUtils_.hsvToRgb((f-.042857142857142705+1)%1,Math.max(g-.0267321331151118,0),Math.max(k-.06274509803921569,0));this.style.setProperty("--gp-panels-elems2-color",`rgb(${p}, ${t}, ${q})`);this.colorPanel?.updatePanelsBackground(a)}updatePanelsElements(a,b){const c=parseInt(a.substr(1,
...
e.height);d.drawImage(this.lastDraw,0,0,e.width,e.height);d.drawImage(this.canvas,0,0,e.width,e.height)}else d=this.canvasCtx;return this.fillAlgorithm(d,a,b,c,this.canvas.width,this.canvas.height).map((e,f)=>4>f%5?Math.round(e/this.density):e)}setFillAlgorithm(a){switch(a){case GPPainter_.FILL_ALGORITHM.OPTIMIZED:this.fillAlgorithm=this.getFillRectsOptimized;break;default:this.fillAlgorithm=this.getFillRects}}getFillRects(a,b,c,d,e,f){let g;var k=(r,C,x)=>{let w=C,z=x;if(!p(C,x,r))return{w:-1,h:-1};
for(;p(w+1,z,r);)w++;let B=w;do for(w=C-1,z++;p(w+1,z,r)&&w+1<=B;)w++;while(w==B);r=--z-x;return{x:C,y:x,w:B-C,h:r}},h=(r,C,x)=>{let w=C,z=x;if(!p(C,x,r))return{w:-1,h:-1};for(;p(w-1,z,r);)w--;let B=w;do for(w=C+1,z--;p(w-1,z,r)&&w-1>=B;)w--;while(w==B);r=x-++z;return{x:B,y:z,w:C-B,h:r}},l=(r,C,x)=>{let w=C,z=x;if(!p(C,x,r))return{w:-1,h:-1};for(;p(w,z+1,r);)z++;let B=z;do for(z=x-1,w--;p(w,z+1,r)&&z+1<=B;)z++;while(z==B);r=C-++w;return{x:w,y:x,w:r,h:B-x}},m=(r,C,x)=>{let w=C,z=x;if(!p(C,x,r))return{w:-1,
h:-1};for(;p(w,z-1,r);)z--;let B=z;do for(z=x+1,w++;p(w,z-1,r)&&z-1>=B;)z--;while(z==B);r=--w-C;return{x:C,y:B,w:r,h:x-B}};let p=(r,C,x)=>{if(g[r][C])return!1;r=4*(r+C*e);r=A.slice(r,r+4);C=Math.abs(x[1]-r[1]);let w=Math.abs(x[2]-r[2]),z=Math.abs(x[3]-r[3]);return 20>Math.abs(x[0]-r[0])&&20>C&&20>w&&20>z||0==r[3]&&255==x[0]&&255==x[1]&&255==x[2]||0==x[3]&&255==r[0]&&255==r[1]&&255==r[2]};var t=r=>{var C=r[0];let x=r[1],w=C+r[2],z=x+r[3];for(;C<w;C++)for(let B=x;B<z;B++)g[C][B]=!0;return r};b=Math.round(b);
c=Math.round(c);d=parseInt(d.replace("#","0x"));var q=d%256,n=(d=Math.floor(d/256))%256,y=d=Math.floor(d/256);(()=>{g=[];for(var r=-1;r<=e;r++)g[r]=[];g[-1]=[];g[e]=[];for(r=-1;r<=f;r++)g[-1][r]=1,g[e][r]=1;for(r=0;r<e;r++)g[r][-1]=1,g[r][f]=1})();let A=a.getImageData(0,0,e,f).data;a=4*(b+c*e);a=[A[a],A[a+1],A[a+2],A[a+3]];d=[];for(let r=0;r<=e;r++)d[r]=[];if(p(b,c,[y,n,q,255]))return[];for(var v;p(b-1,c,a);)b--;for(;p(b,c-1,a);)c--;q=k(a,b,c);n={x:b,y:c,w:q.w,h:q.h,ref:0,andada:0,stack:0};y=q.w;
b=[...t([q.x,q.y,q.w+1,q.h+1]),0];do{c=0;for(2==n.ref&&(c+=n.andada);c<=n.h;)-1!=(v=(q=m(a,n.x+n.w+1,n.y+n.h-c)).h)?(d[v].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:1,andada:n.h+1-c,stack:b.length}),b.push(...t([q.x,q.y,q.w+1,q.h+1]),n.stack),v>y&&(y=v),c+=v):c++;c=0;for(1==n.ref&&(c+=n.andada);c<=n.h;)-1!=(v=(q=l(a,n.x-1,n.y+c)).h)?(d[v].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:2,andada:n.h+1-c,stack:b.length}),b.push(...t([q.x,q.y,q.w+1,q.h+1]),n.stack),v>y&&(y=v),c+=v):c++;c=0;for(4==n.ref&&(c+=n.andada);c<=n.w;)-1!=
(v=(q=k(a,n.x+c,n.y+n.h+1)).w)?(d[v].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:3,andada:n.w+1-c,stack:b.length}),b.push(...t([q.x,q.y,q.w+1,q.h+1]),n.stack),v>y&&(y=v),c+=v):c++;c=0;for(3==n.ref&&(c+=n.andada);c<=n.w;)-1!=(v=(q=h(a,n.x+n.w-c,n.y-1)).w)?(d[v].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:4,andada:n.w+1-c,stack:b.length}),b.push(...t([q.x,q.y,q.w+1,q.h+1]),n.stack),v>y&&(y=v),c+=v):c++;for(n=d[y].pop();null==n&&0<y;)n=d[--y].pop()}while(null!=n);let u=new Map;k=(r,C,x,w,z)=>{if(!g[r][C]){x=r+x;var B=C+
w;if(!(0>x||x>=e||0>B||B>=f))for(w=4*(r+C*e),x=4*(x+B*e),B=0;4>B;B++)if(20<=Math.abs(A[w+B]-A[x+B])){w=r*f+C;u.has(w)||u.set(w,[r,C,z]);break}}};for(h=0;h<b.length;h+=5){l=b[h];m=b[h+1];t=b[h+2];v=b[h+3];if(l+t<e)for(c=m;c<m+v;c++)k(l+t,c,1,0,h);if(0<l)for(c=m;c<m+v;c++)k(l-1,c,-1,0,h);if(m+v<f)for(c=l;c<l+t;c++)k(c,m+v,0,1,h);if(0<m)for(v=l;v<l+t;v++)k(v,m-1,0,-1,h)}for(let [,[r,C,x]]of u)b.push(r,C,1,1,x);return b}getFillRectsOptimized(a,b,c,d,e,f){b=Math.round(b);c=Math.round(c);const g=new Uint8Array(e*
f),k=new Uint8Array(e*f);var h=parseInt(d.replace("#","0x"));d=h%256;var l=(h=Math.floor(h/256))%256;h=Math.floor(h/256);const m=a.getImageData(0,0,e,f).data;a=4*(b+c*e);const p=m[a],t=m[a+1],q=m[a+2],n=m[a+3],y=(w,z,B,E,I,Q)=>{if(0>w||w>=e||0>z||z>=f)return!1;w=z*e+w;if(g[w])return!1;w*=4;z=B-m[w];const D=E-m[w+1],G=I-m[w+2],H=Q-m[w+3];return-20<z&&20>z&&-20<D&&20>D&&-20<G&&20>G&&-20<H&&20>H||0===m[w+3]&&255===B&&255===E&&255===I||0===Q&&255===m[w]&&255===m[w+1]&&255===m[w+2]};a=w=>{const z=w[0];
var B=w[1];const E=z+w[2],I=B+w[3];for(;B<I;B++){const Q=B*e;for(let D=z;D<E;D++)g[Q+D]=1}return w};if(y(b,c,h,l,d,255))return[];for(;y(b-1,c,p,t,q,n);)b--;for(;y(b,c-1,p,t,q,n);)c--;d=(w,z)=>{let B=w,E=z;if(!y(w,z,p,t,q,n))return{w:-1,h:-1};for(;y(B+1,E,p,t,q,n);)B++;let I=B;do for(B=w-1,E++;y(B+1,E,p,t,q,n)&&B+1<=I;)B++;while(B===I);return{x:w,y:z,w:I-w,h:--E-z}};l=(w,z)=>{let B=w,E=z;if(!y(w,z,p,t,q,n))return{w:-1,h:-1};for(;y(B-1,E,p,t,q,n);)B--;let I=B;do for(B=w+1,E--;y(B-1,E,p,t,q,n)&&B-1>=
I;)B--;while(B===I);return{x:I,y:++E,w:w-I,h:z-E}};h=(w,z)=>{let B=w,E=z;if(!y(w,z,p,t,q,n))return{w:-1,h:-1};for(;y(B,E+1,p,t,q,n);)E++;let I=E;do for(E=z-1,B--;y(B,E+1,p,t,q,n)&&E+1<=I;)E++;while(E===I);return{x:++B,y:z,w:w-B,h:I-z}};var A=(w,z)=>{let B=w,E=z;if(!y(w,z,p,t,q,n))return{w:-1,h:-1};for(;y(B,E-1,p,t,q,n);)E--;let I=E;do for(E=z+1,B++;y(B,E-1,p,t,q,n)&&E-1>=I;)E--;while(E===I);return{x:w,y:I,w:--B-w,h:z-I}},v=[];for(var u=0;u<=e;u++)v[u]=[];u=d(b,c);b={x:b,y:c,w:u.w,h:u.h,ref:0,andada:0,
stack:0};c=u.w;let r=[...a([u.x,u.y,u.w+1,u.h+1]),0],C;do{u=0;for(2===b.ref&&(u+=b.andada);u<=b.h;){var x=A(b.x+b.w+1,b.y+b.h-u);C=x.h;-1!==C?(v[C].push({x:x.x,y:x.y,w:x.w,h:x.h,ref:1,andada:b.h+1-u,stack:r.length}),r.push(...a([x.x,x.y,x.w+1,x.h+1]),b.stack),C>c&&(c=C),u+=C):u++}u=0;for(1===b.ref&&(u+=b.andada);u<=b.h;)x=h(b.x-1,b.y+u),C=x.h,-1!==C?(v[C].push({x:x.x,y:x.y,w:x.w,h:x.h,ref:2,andada:b.h+1-u,stack:r.length}),r.push(...a([x.x,x.y,x.w+1,x.h+1]),b.stack),C>c&&(c=C),u+=C):u++;u=0;for(4===
b.ref&&(u+=b.andada);u<=b.w;)x=d(b.x+u,b.y+b.h+1),C=x.w,-1!==C?(v[C].push({x:x.x,y:x.y,w:x.w,h:x.h,ref:3,andada:b.w+1-u,stack:r.length}),r.push(...a([x.x,x.y,x.w+1,x.h+1]),b.stack),C>c&&(c=C),u+=C):u++;u=0;for(3===b.ref&&(u+=b.andada);u<=b.w;)x=l(b.x+b.w-u,b.y-1),C=x.w,-1!==C?(v[C].push({x:x.x,y:x.y,w:x.w,h:x.h,ref:4,andada:b.w+1-u,stack:r.length}),r.push(...a([x.x,x.y,x.w+1,x.h+1]),b.stack),C>c&&(c=C),u+=C):u++;for(b=v[c].pop();null==b&&0<c;)b=v[--c].pop()}while(null!=b);a=(w,z,B,E,I)=>{const Q=
z*e+w;if(!(g[Q]||0>B||B>=e||0>E||E>=f)){var D=4*(w+z*e),G=4*(B+E*e);B=m[D]-m[G];E=m[D+1]-m[G+1];var H=m[D+2]-m[G+2];D=m[D+3]-m[G+3];(-20>=B||20<=B||-20>=E||20<=E||-20>=H||20<=H||-20>=D||20<=D)&&0===k[Q]&&(k[Q]=1,r.push(w,z,1,1,I))}};d=r.length;for(l=0;l<d;l+=5){h=r[l];A=r[l+1];v=r[l+2];c=r[l+3];b=l;if(h+v<e)for(u=A;u<A+c;u++)a(h+v,u,h+v+1,u,b);if(0<h)for(u=A;u<A+c;u++)a(h-1,u,h-2,u,b);if(A+c<f)for(u=h;u<h+v;u++)a(u,A+c,u,A+c+1,b);if(0<A)for(c=h;c<h+v;c++)a(c,A-1,c,A-2,b)}return r}setTimer(){this.timer&&
...
{if(q!==this){var n=q.frame.getBoundingClientRect();var y=Math.abs(n.y-(c+e));y<f&&(void 0===h||y<h)&&n.x+n.width>=b&&n.x<=b+d&&(!t||t[0]===q.frame.dataset.anchor[0])&&(h=y,c=n.y-e,m=t=q.frame.dataset.anchor);y=Math.abs(n.y+n.height-c);y<f&&(void 0===l||y<l)&&n.x+n.width>=b&&n.x<=b+d&&(!t||t[0]===q.frame.dataset.anchor[0])&&(l=y,c=n.y+n.height,m=t=q.frame.dataset.anchor);y=Math.abs(n.x-(b+d));y<f&&(void 0===g||y<g)&&n.y+n.height>=c&&n.y<=c+e&&(!p||p[1]===q.frame.dataset.anchor[1])&&(g=y,b=n.x-d,m=
p=q.frame.dataset.anchor);y=Math.abs(n.x+n.width-b);y<f&&(void 0===k||y<k)&&n.y+n.height>=c&&n.y<=c+e&&(!p||p[1]===q.frame.dataset.anchor[1])&&(k=y,b=n.x+n.width,m=p=q.frame.dataset.anchor)}});return[b,c,m]}resizeHandler(a){if(2===a.button){var b=this.width,c=a.pageX,d=this.image.getBoundingClientRect().width,e=this.frame.dataset.anchor,f=(g=>{g=g.pageX-c;this.setSize(Math.min(Math.max(d+(e[1]===GPReferenceFrame_.ANCHOR.LEFT?g:-g),this.minWidth),GPReferenceFrame_.MAX_WIDTH))}).bind(this);this.frame.setPointerCapture(a.pointerId);
...
background:b,canvasType:c,auth:d,meta:{author:e,title:f,date:g}},container:k,mainContainer:h},l=GPTimelapsePlayer_.DEFAULT_DISPLAY_MODE,m=!1){super();GPTimelapsePlayer_.players.add(this);this.date=new Date(g);const p=!(g&&this.date.getTime()>GPTimelapsePlayer_.SCDV2_DATE);this.canvasType=c;this.canvasTypeName=GPUtils_.getCanvasTypeName(c);const [t,q]=p?this.getSCDV1CanvasDimensions(c):GPUtils_.getCanvasDimensions(c);this.density=GPTimelapsePlayer_.CANVAS_DENSITY;this.originalWidth=t;this.originalHeight=
q;this.width=this.originalWidth*this.density;this.height=this.originalHeight*this.density;this.originalData=a.slice();const [n,y,A]=this.handleDelimiters(a,b);this.data=n;this.background=y;this.auth=d;this.author=e;this.title=f;this.eraserAlpha=!A;this.fi=g&&this.date.getTime()>GPTimelapsePlayer_.FV2_DATE?5:4;this.isSNSC=!(g&&this.date.getTime()>GPTimelapsePlayer_.SFSC_DATE);this.container=k;this.container.isPlayerRendered=!0;this.mainContainer=h;this.defaultDisplayMode=l;this.defaultDisplayModeDensity=
...
this.banlistElem.innerHTML="";const b=document.createDocumentFragment();var c=document.createElement("span");c.className="ul-title_";c.textContent=this.l.BANLIST;b.appendChild(c);this.banlist.size?this.banlist.forEach(d=>{var e=d;let f=!1;if(this.prx.isTwitchAuth()&&this.s.disableLocalizedTwitchUsernames){var g=this.localizedLogins.get(this.prx.getUsers().find(h=>this.prx.formatNickname(h.nick)===d)?.nick);g&&(e=g,f=!0)}g=document.createElement("div");g.className="ul-banned-user_";g.dataset.username=
d;const k=document.createElement("div");k.className="username_";k.title=f?`${e}   |   ${d}`:d;k.textContent=e;f&&(k.dataset.login=e);g.appendChild(k);e=document.createElement("div");e.className="remove-btn_";e.textContent="\u2716";e.addEventListener("click",h=>{});g.appendChild(e);b.appendChild(g)}):(c=document.createElement("div"),c.className="ul-banlist-placeholder_",c.textContent=this.l.BANLIST_EMPTY,b.appendChild(c));this.banlistElem.appendChild(b);this.banlistElem.scrollTop=a}updateBanlistUsername(a,
...
(a=a.currentTarget.closest(".item"),!this.isEmptyItem(a)&&!this.isPreviewedItem(a)&&a.matches(".banned_, .censored_, .suspicious_"))){a.classList.add("previewed_");var b=this.getItemId(a);if(a=this.prx.getAlbumItem(b)){b=`${this.prx.getCurrentAlbumId()}_${b}`;const c=this.prx.getCanvasType();this.preview.load(a.data,a.type,c,b)}}}albumItemLeaveHandler(a){this.preview.isEnabled()&&(a=a.currentTarget.closest(".item"),a.classList.contains("previewed_")&&(a.classList.remove("previewed_"),this.preview.clear()))}imagePreviewOverHandler(a){if(this.preview.isEnabled()){var b=
a.currentTarget;this.isPreviewCanvas(a.target)&&!this.isPreviewedItem(b)&&this.previewCensorableItem(b)}}previewCensorableItem(a){const b=this.prx.getPrevImageData();if(b&&a.matches(".banned_, .censored_, .suspicious_")){a.classList.add("previewed_");a.addEventListener("pointerleave",e=>{a.classList.remove("previewed_");this.preview.clear()},{once:!0});const c=this.prx.getCurrentTurnId(),d=this.prx.getCanvasType();this.preview.load(b,Number(a.dataset.typeId),d,`t_${c}`,d)}}imagePreviewDownHandler(a){0===
a.button&&this.isPreviewCanvas(a.target)&&this.handleCensorableItemDown(a.currentTarget)}handleCensorableItemDown(a){const b=this.getContentId(a),c=this.prx.getPrevData()?.user?.nick;this.s.safeMode&&!this.safeItems.has(b)?this.safeItems.add(b):this.shouldBeBannedItem(a,c)||this.isPreviewedItem(a)||a.dataset.cr&&a.dataset.cr!==GPModeration_.CENSORING_REASON.BY_USER?(a.classList.toggle("preview_"),this.preview.isEnabled()&&(this.preview.clear(),a.classList.remove("previewed_"))):this.censorItems.has(b)?
this.censorItems.delete(b):this.censorItems.set(b,a.dataset.type);this.handleCensorableItem(a)}isPreviewCanvas(a){return"CANVAS"===a.tagName}isPreviewScreen(a){return GPModeration_.PREVIEW_PATHS.includes(a.slice(1))}handlePainterHeader(a){if(a=a.querySelector("h4 + h3"))a.addEventListener("pointerover",this.painterHeaderTextOverHandler),a.addEventListener("pointerdown",this.painterHeaderTextDownHandler),a.dataset.type="text",a.dataset.typeId=GPProxy_.ALBUM_ITEM_TYPE.TEXT,this.handleCensorableItem(a)}painterHeaderTextOverHandler(a){this.preview.isEnabled()&&
this.previewCensorableItem(a.target)}painterHeaderTextDownHandler(a){0===a.button&&this.handleCensorableItemDown(a.target)}acTriggerHandler({detail:{itemId:a,albumId:b}}){this.prx.getCurrentAlbumId()===b&&(b=this.albumItems.find(c=>this.getItemId(c)===a))&&b.classList.add("suspicious_")}initTwitchTokenBtn(){this.twitchTokenBtn=document.createElement("div");this.twitchTokenBtn.className="mod-twitch-token-btn_";this.twitchTokenBtn.title="\u041f\u043e\u043b\u0443\u0447\u0438\u0442\u044c Twitch-\u0442\u043e\u043a\u0435\u043d, \u043f\u0440\u0435\u0434\u043e\u0441\u0442\u0430\u0432\u0438\u0432 \u043d\u0435\u043e\u0431\u0445\u043e\u0434\u0438\u043c\u044b\u0435 \u043f\u0440\u0430\u0432\u0430 \u043f\u0440\u0438\u043b\u043e\u0436\u0435\u043d\u0438\u044e gp-mod (\u043d\u0435 \u0434\u0435\u043b\u0430\u0439\u0441\u044f \u044d\u0442\u043e\u0433\u043e \u043d\u0430 \u0442\u0440\u0430\u043d\u0441\u043b\u044f\u0446\u0438\u0438 \u0438 \u043d\u0435 \u0441\u043e\u043e\u0431\u0449\u0430\u0439\u0442\u0435 \u0442\u043e\u043a\u0435\u043d \u043d\u0438\u043a\u043e\u043c\u0443)";
...
this.prx=a;this.l=g;this.enabled=!!b;this.panelOffsetX=c;this.panelOffsetY=d;this.panelAnchor=e;this.panelWidth=f;this.panelBorderWidth=GPPreview_.MIN_PANEL_BORDER_WIDTH;this.window=null;this.cache=new Map;this.worker=null;this.addEventListener("rendered",()=>{this.panel.classList.add("rendered_")});this.prx.addEventListener("data",({detail:{event:k}})=>{k===GPProxy_.EVENT.GAME_STARTED&&this.cache.clear()});window.addEventListener("message",k=>{switch(k.data?.type){case "gp-preview-request-draw-function":this.window.postMessage({type:"draw_function",
...
this.panel.addEventListener("wheel",b=>{b.preventDefault();this.panelWidth=0<b.deltaY?Math.max(this.panelWidth-GPPreview_.PANEL_WIDTH_STEP,GPPreview_.MIN_PANEL_WIDTH):Math.min(this.panelWidth+GPPreview_.PANEL_WIDTH_STEP,GPPreview_.MAX_PANEL_WIDTH);this.updatePanelSize();this.updatePanelPosition();this.dispatchEvent(new CustomEvent("panel_width_changed",{detail:{width:this.panelWidth}}))});this.panel.addEventListener("pointerdown",b=>{if(0===b.button){var [c,d,e,f]=this.getCoords(b),g=(k=>{this.setPanelPosition(k.pageX-
c,k.pageY-d,e,f)}).bind(this);this.panel.setPointerCapture(b.pointerId);this.panel.addEventListener("pointermove",g);this.panel.addEventListener("pointerup",()=>{this.dispatchEvent(new CustomEvent("panel_pos_changed",{detail:{x:this.panelOffsetX,y:this.panelOffsetY,o:this.panelAnchor}}));this.panel.removeEventListener("pointermove",g)},{once:!0});g(b)}});this.panel.addEventListener("pointerdown",b=>{if(1===b.button||2===b.button)this.panel.setPointerCapture(b.pointerId),this.panel.addEventListener("pointerup",
c=>{if(1===c.button||2===c.button)this.window?this.closeWindow():this.openWindow()},{once:!0})});var a=document.createElement("div");a.className="warning_";a.style.setProperty("--frame-mode-warning",`'${this.l.PREIVEW_PANEL_WARNING}'`);a.style.setProperty("--win-mode-warning",`'${this.l.PREIVEW_TAB_WARNING}'`);this.panel.appendChild(a);a=document.createElement("div");a.className="canvas-wrapper_";this.panel.appendChild(a);this.canvas=this.createCanvas(this.prx.getCanvasType());a.appendChild(this.canvas);
this.text=document.createElement("div");this.text.className="text_";this.panel.appendChild(this.text);document.body.appendChild(this.panel);this.updatePanelPosition();this.updatePanelSize()}createCanvas(a=this.canvasType){this.canvasType=a;const [b,c]=GPUtils_.getCanvasDimensions(a);a=document.createElement("canvas");a.className="canvas_";a.width=b*GPPreview_.CANVAS_DENSITY;a.height=c*GPPreview_.CANVAS_DENSITY;return a}replaceCanvas(){const a=this.createCanvas();this.canvas.replaceWith(a);this.canvas=
a}toggle(a){this.enabled=void 0!==a?!!a:!this.enabled;this.panel.classList.toggle("shown_",this.enabled);this.enabled?(this.panel.classList.add("frame-mode_"),this.initWorker()):(this.clear(),this.window&&this.closeWindow(),this.terminateWorker());this.dispatchEvent(new CustomEvent("status",{detail:{status:this.enabled}}));this.showPanelBtn.classList.toggle("pressed_",this.enabled)}openWindow(){this.window&&this.window.close();this.window=window.open(GPPreview_.WINDOW_URL);this.panel.classList.replace("frame-mode_",
...
this.panelBorderWidth),this.panelBorderWidth),b=Math.max(Math.min(this.panelOffsetY,document.documentElement.clientHeight-this.panelWidth/(GPPreview_.CANVAS_WIDTH/GPPreview_.CANVAS_HEIGHT)-this.panelBorderWidth),this.panelBorderWidth);this.panel.style.inset="unset";this.panelAnchor.split("").map(c=>GPPreview_.ANCHORS[c]).forEach(c=>{switch(c){case GPPreview_.ANCHOR.TOP:case GPPreview_.ANCHOR.BOTTOM:this.panel.style[c]=`${b}px`;break;case GPPreview_.ANCHOR.LEFT:case GPPreview_.ANCHOR.RIGHT:this.panel.style[c]=
`${a}px`}})}setPanelPosition(a,b,c,d){if(!c||!d){const [h,l]=this.panel.getBoundingClientRect();c=h;d=l}const e=document.documentElement.clientWidth,f=document.documentElement.clientHeight,g=Math.max(Math.min(a,e-c-this.panelBorderWidth),this.panelBorderWidth),k=Math.max(Math.min(b,f-d-this.panelBorderWidth),this.panelBorderWidth);this.panel.style.inset="unset";this.panel.dataset.anchor="";b+d/2>f/2?(this.panelOffsetY=Math.round(f-(k+d)),this.panel.style.bottom=`${this.panelOffsetY}px`,this.panel.dataset.anchor+=
"b"):(this.panelOffsetY=Math.round(k),this.panel.style.top=`${this.panelOffsetY}px`,this.panel.dataset.anchor+="t");a+c/2>e/2?(this.panelOffsetX=Math.round(e-(g+c)),this.panel.style.right=`${this.panelOffsetX}px`,this.panel.dataset.anchor+="r"):(this.panelOffsetX=Math.round(g),this.panel.style.left=`${this.panelOffsetX}px`,this.panel.dataset.anchor+="l");this.panelAnchor=this.panel.dataset.anchor}getCoords(a){const [b,c]=a.touches?[a.touches[0].clientX,a.touches[0].clientY]:[a.clientX,a.clientY];
a=a.currentTarget.getBoundingClientRect();return[Math.round(b-a.left),Math.round(c-a.top),a.width,a.height]}initWorker(){this.worker?.terminate();this.replaceCanvas();const a=this.canvas.transferControlToOffscreen();this.worker=GPUtils_.createWorker(this.workerBody);this.worker.postMessage({type:"init",data:{canvas:a,density:GPPreview_.CANVAS_DENSITY,drawFunctionString:GPUtils_.drawFunctionString}},[a]);this.worker.addEventListener("message",({data:{type:b,...c}})=>{switch(b){case "cache":const {imageData:d,
cacheId:e}=c;this.cache.set(e,d);break;case "rendered":this.dispatchEvent(new Event("rendered"))}})}terminateWorker(){this.worker?.terminate()}workerBody(){self.addEventListener("message",({data:{type:a,data:b}})=>{switch(a){case "draw":const {strokes:c,cacheId:d}=b;self.clear();self.draw(self.ctx,c,self.density,!0,!1);a=self.ctx.getImageData(0,0,self.canvas.width,self.canvas.height);self.postMessage({type:"cache",imageData:a,cacheId:d});self.postMessage({type:"rendered"});break;case "clear":self.clear();
break;case "cache":self.clear();self.ctx.putImageData(b,0,0);self.postMessage({type:"rendered"});break;case "init":const {canvas:e,density:f}=b;self.density=f;self.canvas=e;self.ctx=self.canvas.getContext("2d");self.ctx.imageSmoothingQuality="high";self.draw=(new Function(`return ${b.drawFunctionString}`))();self.clear=()=>{self.ctx.clearRect(0,0,self.canvas.width,self.canvas.height)}}})}load(a,b,c,d){if(a&&void 0!==b){var e=GPPreview_.TYPES[b];this.panel.classList.remove("rendered_");this.panel.classList.add("loading_");
if(this.window)this.window.postMessage({type:"data",data:a,itemType:e,canvasType:c},GPPreview_.WINDOW_URL);else switch(this.panel.dataset.type=e,b){case GPProxy_.ALBUM_ITEM_TYPE.IMAGE:c!==this.canvasType&&(this.canvasType=c,this.initWorker());this.worker.postMessage({type:"clear"});(b=this.cache.get(d))?this.worker.postMessage({type:"cache",data:b}):this.worker.postMessage({type:"draw",data:{strokes:a}});break;case GPProxy_.ALBUM_ITEM_TYPE.TEXT:this.text.textContent=a,this.dispatchEvent(new Event("rendered"))}}}clear(){this.window?
this.window.postMessage({type:"clear"},GPPreview_.WINDOW_URL):this.worker?.postMessage({type:"clear"});this.text.textContent="";this.panel.classList.remove("loading_")}isEnabled(){return this.enabled||this.window}}window.GPPreview_=GPPreview_;class GPTwitchAPI_ extends EventTarget{static CLIENT_ID="qrohytuwdc7d3uns3gnvtb4esylpqy";static BASE_URL="https://api.twitch.tv/helix";static BANS_API="/moderation/bans";static AUTH_URL=`https://id.twitch.tv/oauth2/authorize?client_id=${this.CLIENT_ID}&redirect_uri=${encodeURIComponent("https://gpmod.github.io/twitch-token?WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW&tokenType={token-type}")}&response_type=token&scope={scopes}`;static TOKEN_TYPE={BAN_USERS:"ban-users",
