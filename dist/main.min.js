...
n;l.element.style.transform=e({scale:l.transformation.scale,translateX:l.transformation.translateX,translateY:l.transformation.translateY})},f=l=>({panBy:({originX:m,originY:n})=>h({state:l,originX:m,originY:n}),panTo:({originX:m,originY:n,scale:p})=>{l.transformation.scale=p;h({state:l,originX:m-l.transformation.translateX,originY:n-l.transformation.translateY})}}),g=l=>({zoom:({x:m,y:n,deltaScale:p,scaleSensitivity:r})=>{const {left:q,top:y}=l.element.getBoundingClientRect(),{minScale:z,maxScale:A,
defaultScaleSensitivity:t}=l,[w,u]=d({scale:l.transformation.scale,deltaScale:p,minScale:z,maxScale:A,scaleSensitivity:r||t});p=m-q;r=n-y;n=p/w;m=r/w;const x=c({scale:w,minScale:z,maxScale:A});p=x({pos:p,prevPos:l.transformation.originX,translate:l.transformation.translateX});r=x({pos:r,prevPos:l.transformation.originY,translate:l.transformation.translateY});l.element.style.transformOrigin=`${n/l.containerScale}px ${m/l.containerScale}px`;l.element.style.transform=e({scale:u,translateX:p/l.containerScale,
...
{id:4,name:"GPPainter_"},{id:11,name:"GPGlobalBanlist_",required:!0}],this,c)}const b={author:"liliezzzz",name:"gp-mod",version:"10.2",changelogURL:"https://discord.com/channels/929715202600095775/953145081471324270/1105901535721963541",summaryURL:"https://discord.com/channels/929715202600095775/929715202600095778/1105931364727013437",updateNoticeFlag:"true",updateNoticeText:"",discordURL:"https://discord.gg/hynZCVZyzN",donateURL:"https://boosty.to/gpmod"};if(function(){return navigator.userAgentData?.brands.some(d=>
...
h&&(e=document.createTextNode("\u00a0\u00a0|\u00a0\u00a0"),this.container.appendChild(e),e=document.createElement("a"),e.className="donate-link_",e.target="_blank",e.href=h,e.title=this.l.DONATE_LINK_TTL,e.textContent=this.l.DONATE_LINK,this.container.appendChild(e));document.body.appendChild(this.container)}resizeHandler(a){a=Math.max(GPAbout_.HEADER_MIN_SCALE,this.getContainerScale());this.container.style.fontSize=`${16*a}px`;this.container.style.lineHeight=`${28*a}px`;this.updateUpdateNoticePos()}getContainerScale(){let a=
(window.innerWidth-180)/1150;766*a>window.innerHeight&&(a=window.innerHeight/766);return a}shouldUpdateNoticeBeShown(){if(!this.updateNoticeFlag)return!1;const a=window.localStorage.getItem(GPAbout_.UPDATE_NOTICE_STORAGE)||"0.0";return this.isNewVersion(this.version,a,2)}isNewVersion(a,b,c=0){const [d,e]=[a,b].map(h=>h.split(".").map(f=>+f));a=Math.max(d.length,e.length);a=c&&Math.min(a,c)||a;for(c=0;c<a;c++){b=d[c]??0;const h=e[c]??0;if(b!==h)return b>h}return!1}updateUpdateNoticePos(){if(this.notice){var {width:a}=
...
h=>{h.preventDefault();window.onbeforeunload=null;location.reload()});c.addEventListener("auxclick",h=>{h.preventDefault()});d.appendChild(c);const e=new Set;Array.from(a.values()).forEach((h,f)=>{var g=`${h.url}?${Date.now()}.user.js`;const k=document.createElement("div");k.className="oudated-script_";var l=document.createElement("div");l.textContent=h.name;k.appendChild(l);l=document.createElement("div");l.textContent=`${h.version} \u2794 ${h.latestVersion}`;k.appendChild(l);h=document.createElement("div");
k.appendChild(h);const m=document.createElement("a");m.className="script-update-link_";m.href=g;m.target="_blank";m.textContent="\u041e\u0431\u043d\u043e\u0432\u0438\u0442\u044c";g=n=>{e.add(f);e.size>=a.size&&d.classList.add("shown_");m.closest(".oudated-script_").classList.add("clicked_")};m.addEventListener("click",g);m.addEventListener("auxclick",g);h.appendChild(m);b.appendChild(k)});b.appendChild(d);return b}}window.GPAbout_=GPAbout_;class GPActivation_ extends EventTarget{static AUTH_FILENAME_STORAGE="gp_auth-filename";static ACTIVATION_KEY_PATTERN=/^[a-zA-Z0-9]{32}:\d{14,16}$/;static DEFAULT_SETTINGS={activationKey:"",publicKey:"",filename:""};static SETTINGS_UI={activationKey:{type:"input",args:{hidden:!0},description:"\u041a\u043b\u044e\u0447 \u0430\u043a\u0442\u0438\u0432\u0430\u0446\u0438\u0438"}};static MODULE={title:"Activation",settings:{storage:"gp_activation",defaultSettings:this.DEFAULT_SETTINGS,ui:this.SETTINGS_UI}};static INIT(a){a.act=
...
this.updateKeyMark();this.broadcastPublicKey();document.dispatchEvent(new Event("_auth_key_changed"))}}}window.GPActivation_=GPActivation_;class GPAntiCheat_ extends EventTarget{static MIN_STROKES_TRIGGER=4;static MIN_PERCENTAGE_TRIGGER=75;static SEQUENCE_MIN_SIZE=5;static SEQUENCES_COUNT_TRIGGER=10;static MAX_SEQUENCE_TRIGGER=20;static CM_SEQUENCE_TRIGGER=4;static CANVAS_WIDTH=758;static CANVAS_HEIGHT=424;static M_STROKE_OFFSET=67;static DEFAULT_SETTINGS={enabled:!0,activatedForOwnerOnly:!0,sendReports:!0};static SETTINGS_UI={enabled:{type:"checkbox",description:"\u0410\u043a\u0442\u0438\u0432\u0438\u0440\u043e\u0432\u0430\u0442\u044c \u0410\u043d\u0442\u0438-\u0427\u0438\u0442"},
...
this.gameMiddleware);this.isGameMWActivated=!0});this.gpproxy.addEventListener("data",({detail:{event:e}})=>{e===GPProxy_.EVENT.GAME_STARTED?(this.gpproxy.addWSMsgMiddleware([GPProxy_.EVENT.TURN_STARTED],this.gameMiddleware),this.isGameMWActivated=!0):e===GPProxy_.EVENT.GALLERY_SETTINGS?(this.gpproxy.addWSMsgMiddleware([GPProxy_.EVENT.ALBUM_ITEM,GPProxy_.EVENT.ALBUM_INFO],this.albumMiddleware),this.isAlbumMWActivated=!0,this.gpproxy.removeWSMsgMiddleware(this.gameMiddleware),this.isGameMWActivated=
!1):this.isAlbumMWActivated&&[GPProxy_.EVENT.NEW_GAME,GPProxy_.EVENT.ALBUM_SCORE,GPProxy_.EVENT.GAME_QUIT,GPProxy_.EVENT.GAME_STARTED].includes(e)&&(this.gpproxy.removeWSMsgMiddleware(this.albumMiddleware),this.isAlbumMWActivated=!1)});document.addEventListener("_subs_status",({detail:{status:e}})=>{this.isSubscribed=e;GPAntiCheat_.toggleSettingsVisibility(e)})}gameMiddleware([a,b]){if(!this.s.enabled||this.s.activatedForOwnerOnly&&!this.gpproxy.isOwner())return[a,b];if(a[1]===GPProxy_.EVENT.TURN_STARTED&&
...
{detail:{itemId:c,albumId:d}})),b=!0),!this.s.sendReports&&this.isSubscribed||this.reportSender.send({itemId:c,albumId:d,triggers:e})));else a[1]===GPProxy_.EVENT.ALBUM_INFO&&(a[2].background?.length&&a[2].bookAuthor.id!==this.gpproxy.getUser().id&&(c=this.gpproxy.getCurrentAlbumId(),this.getTriggers(`${c}`,a[2].background,a[2].bookAuthor.nick).size&&this.isSubscribed&&(a[2].background=[],b=!0)),a[2].timeline?.length&&(a[2].timeline=a[2].timeline.map(h=>{if(!h.type||h.user?.id===this.gpproxy.getUser().id||
h.type!==GPProxy_.ALBUM_ITEM_TYPE.IMAGE)return h;if(h.bg){var f=this.gpproxy.getCurrentAlbumId();f=this.getTriggers(`${f}`,this.gpproxy.getAlbumBackground(f));f.size&&this.isSubscribed&&(h.bg=!1,h.data=this.getPlaceholder(f),b=!0)}else h.data?.length&&(f=this.gpproxy.getCurrentAlbumId(),f=this.getTriggers(`${f}_${h.id}`,h.data,h.user.nick),f.size&&this.isSubscribed&&(h.data=this.getPlaceholder(f),b=!0));return h})));return[a,b]}getPlaceholder(a){return this.isForgery(a)?GPAssets_.AC_FORGERY_PH[this.l.lang]??
Object.values(GPAssets_.AC_FORGERY_PH)[0]:GPAssets_.AC_SUSPICIOUS_PH[this.l.lang]??Object.values(GPAssets_.AC_SUSPICIOUS_PH)[0]}getTriggers(a,b,c){let d=this.itemsTriggers.get(a);d||(d=this.calcTriggers(b,c),this.itemsTriggers.set(a,d),d.size&&this.isSubscribed&&this.suspiciousItems.add(a));return d}calcTriggers(a,b){const c=new Set;var d=a.findLastIndex(q=>11===q[0]);a=a.slice(d+1);if(!a?.length)return c;const {animate:e}=this.gpproxy.getGameConfig();let h=0;var f=0;let g=null,k=null,l=0,m=0;d=[];
let n=[],p=0,r=0;for(let q=0;q<a.length;q++){const y=a[q],z=y[0];if(b&&this.isMStroke(y)){const {code:A,author:t}=this.decodeMStroke(y);A!==this.gpproxy.getGameCode()&&c.add(20);t!==this.formatUsername(b)&&c.add(21)}if(1===z){1===e&&2===y[2][1]&&4===y.length&&h++;const A=y.length-3,[t,w]=y[3];t===g&&3>A?l++:(l>GPAntiCheat_.SEQUENCE_MIN_SIZE&&(p=Math.max(p,l),d.push(l)),l=0);w===k&&3>A?m++:(m>GPAntiCheat_.SEQUENCE_MIN_SIZE&&(r=Math.max(r,m),n.push(m)),m=0);g=t;k=w}else 8===z&&f++;1!==z&&(g=k=null,
l=m=0)}b=h/a.length*100;f=f/a.length*100;a.length>=GPAntiCheat_.MIN_STROKES_TRIGGER&&(b>GPAntiCheat_.MIN_PERCENTAGE_TRIGGER||f>GPAntiCheat_.MIN_PERCENTAGE_TRIGGER)&&c.add(1);(d.length>GPAntiCheat_.SEQUENCES_COUNT_TRIGGER||n.length>GPAntiCheat_.SEQUENCES_COUNT_TRIGGER)&&c.add(2);(p>GPAntiCheat_.MAX_SEQUENCE_TRIGGER||r>GPAntiCheat_.MAX_SEQUENCE_TRIGGER)&&c.add(3);return c}isForgery(a){return a.has(20)||a.has(21)}isMStroke(a){return!(1===a[0]&&a[3][0]>GPAntiCheat_.CANVAS_WIDTH&&a[3][1]>GPAntiCheat_.CANVAS_HEIGHT)||
a[3][0]-GPAntiCheat_.CANVAS_WIDTH-GPAntiCheat_.M_STROKE_OFFSET||a.at(-1)[1]-GPAntiCheat_.CANVAS_HEIGHT-GPAntiCheat_.M_STROKE_OFFSET?!1:!0}decodeMStroke(a){const [b,c,d]=a.slice(3).flat().map((e,h)=>e-GPAntiCheat_.M_STROKE_OFFSET-(h%2?GPAntiCheat_.CANVAS_HEIGHT:GPAntiCheat_.CANVAS_WIDTH)).filter(Boolean).map(e=>String.fromCodePoint(e)).join("").split("@");return{code:b,author:c,timestamp:d}}formatUsername(a){return this.gpproxy.isDiscordAuth()?a:a.toLowerCase()}getSuspiciousItems(){return this.suspiciousItems}isSuspicious(a){return this.suspiciousItems.has(a)}}
window.GPAntiCheat_=GPAntiCheat_;class GPArtBroadcaster_ extends EventTarget{static SENT_IMAGE_DENSITY=1;static SENT_IMAGE_ORIGINAL_WIDTH=758;static SENT_IMAGE_ORIGINAL_HEIGHT=424;static SENT_IMAGE_BACKGROUND_COLOR="#ffffff";static SERVICES=["discord","telegram","vk"];static DEFAULT_SETTINGS={services:[],discordBorderColor:"#c958fe",discordBotToken:"",discordServerID:"",discordArtsChannelID:"",discordPlayerChannelID:"",discordArchiveChannelID:"",telegramBotToken:"",telegramChannel:"",telegramPlayerButton:!0,telegramSourceButton:!1,
vkAccessToken:"",vkGroupID:"",vkAlbumID:"",vkPhotoDescription:"\u0410\u0432\u0442\u043e\u0440: {author}"};static SETTINGS_UI={services:{type:"list-m",args:this.SERVICES,description:"\u0421\u0435\u0440\u0432\u0438\u0441\u044b \u0434\u043b\u044f \u043f\u0443\u0431\u043b\u0438\u043a\u0430\u0446\u0438\u0438 \u0440\u0438\u0441\u0443\u043d\u043a\u043e\u0432"},_1:{text:"Discord"},discordBorderColor:{type:"color",description:"\u0426\u0432\u0435\u0442 \u043b\u0435\u0432\u043e\u0439 \u0440\u0430\u043c\u043a\u0438 \u0441\u043e\u043e\u0431\u0449\u0435\u043d\u0438\u044f \u0441 \u043e\u043f\u0443\u0431\u043b\u0438\u043a\u043e\u0432\u0430\u043d\u043d\u044b\u043c \u0440\u0438\u0441\u0443\u043d\u043a\u043e\u043c"},
...
discordArchiveChannelID:{type:"input",hidden:!0,description:"ID \u043a\u0430\u043d\u0430\u043b\u0430 \u0434\u043b\u044f \u0438\u0441\u0445\u043e\u0434\u043d\u0438\u043a\u043e\u0432 \u0440\u0438\u0441\u0443\u043d\u043a\u043e\u0432"},_2:{text:"Telegram"},telegramBotToken:{type:"input",args:{hidden:!0},description:"\u0422\u043e\u043a\u0435\u043d Telegram-\u0431\u043e\u0442\u0430"},telegramChannel:{type:"input",description:"\u041d\u0430\u0437\u0432\u0430\u043d\u0438\u0435 \u0438\u043b\u0438 ID \u043a\u0430\u043d\u0430\u043b\u0430 (\u0432 \u0444\u043e\u0440\u043c\u0430\u0442\u0435 @channel_name)"},
telegramPlayerButton:{type:"checkbox",description:"\u0414\u043e\u0431\u0430\u0432\u043b\u044f\u0442\u044c \u043a\u043d\u043e\u043f\u043a\u0443 \u0434\u043b\u044f \u043f\u0440\u043e\u0441\u043c\u043e\u0442\u0440\u0430 \u043f\u0440\u043e\u0446\u0435\u0441\u0441\u0430 \u0440\u0438\u0441\u043e\u0432\u0430\u043d\u0438\u044f \u0432 \u043f\u043b\u0435\u0435\u0440\u0435"},telegramSourceButton:{type:"checkbox",description:"\u0414\u043e\u0431\u0430\u0432\u043b\u044f\u0442\u044c \u043a\u043d\u043e\u043f\u043a\u0443 \u0434\u043b\u044f \u0441\u043e\u0445\u0440\u0430\u043d\u0435\u043d\u0438\u044f \u0438\u0441\u0445\u043e\u0434\u043d\u0438\u043a\u0430 \u0440\u0438\u0441\u0443\u043d\u043a\u0430"},
_3:{text:"VK (\u0412 \u041a\u043e\u043d\u0442\u0430\u043a\u0442\u0435)"},vkAccessToken:{type:"input",args:{hidden:!0},description:"VK \u0442\u043e\u043a\u0435\u043d"},vkGroupID:{type:"input",description:"ID \u0433\u0440\u0443\u043f\u043f\u044b \u0438\u043b\u0438 \u0441\u043e\u043e\u0431\u0449\u0435\u0441\u0442\u0432\u0430 \u0441 \u0430\u043b\u044c\u0431\u043e\u043c\u043e\u043c \u0434\u043b\u044f \u043f\u0443\u0431\u043b\u0438\u043a\u0430\u0446\u0438\u0438 \u0440\u0438\u0441\u0443\u043d\u043a\u043e\u0432"},
...
"relative";var d=document.createElement("div");d.className="gp-art-broadcaster_";d.classList.add("hidden");d.addEventListener("click",h=>{d.classList.remove("hidden");h.ctrlKey?(GPArtBroadcaster_.SERVICES.forEach(f=>{delete d.dataset[f]}),this.sendImage(this.s.services,b,c,d)):(h=this.s.services.filter(f=>{if(d.dataset[f]&&"false"!==d.dataset[f])return!1;delete d.dataset[f];return!0}),h.length&&this.sendImage(h,b,c,d))});this.setInitialButtonState(d,c);var e=document.createElement("div");e.className=
"media_";GPArtBroadcaster_.SERVICES.forEach(h=>{const f=document.createElement("div");f.className=`${h}_ icon_`;e.appendChild(f)});d.appendChild(e);a.appendChild(d);this.buttons[c]=d}}startMonitor(){if(!this.inProgress){var a=document.querySelector(".scrapbook");a?(this.observer.observe(a,this.observerConfig),this.inProgress=!0):this.monitorTimer=setTimeout(()=>{this.startMonitor()},200)}}stopMonitor(){clearTimeout(this.monitorTimer);this.inProgress&&(this.observer.disconnect(),this.inProgress=!1)}observerCallback(a,
b){for(const d of a)if("childList"===d.type&&d.addedNodes.length&&(a=d.addedNodes[0],d.target.classList.contains("scrollElements")&&a.classList.contains("item"))){var c=this.gpproxy.getCurrentAlbumId();b=this.nextItemID;null===b&&(b=[...a.parentElement.children].filter(e=>e.classList.contains("item")).indexOf(a),(c=this.timeline[c]?.[b])&&this.renderButton(a,c,b));this.itemsElems[b]=a}}sendImage(a,b,c,d){const {data:e,background:h,meta:f}=b;var g=GPArtBroadcaster_.SENT_IMAGE_DENSITY;const k=GPArtBroadcaster_.SENT_IMAGE_ORIGINAL_WIDTH*
g,l=GPArtBroadcaster_.SENT_IMAGE_ORIGINAL_HEIGHT*g,m=document.createElement("canvas");m.width=k;m.height=l;var n=m.getContext("2d");n.fillStyle=GPArtBroadcaster_.SENT_IMAGE_BACKGROUND_COLOR;n.fillRect(0,0,k,l);h&&this.drawFunction(n,h,g,!1);n=document.createElement("canvas");n.width=k;n.height=l;var p=n.getContext("2d");this.drawFunction(p,e,g,!0);g=document.createElement("canvas");g.width=k;g.height=l;p=g.getContext("2d");p.drawImage(m,0,0,k,l);p.drawImage(n,0,0,k,l);g.toBlob(async r=>{d.classList.remove("complete_");
d.classList.add("uploading_");var q=(new Date(f.date)).toLocaleDateString("ru-RU");q=`${f.author} - ${f.title} (${q})`;r=new File([r],`${q}.png`);const y=await GPUtils_.compressImageData(b);q=new File([y],`${q}.gpimg`);document.dispatchEvent(new CustomEvent("_send_to_media",{detail:{services:a,artFile:r,sourceFile:q,meta:f,authType:this.gpproxy.authType,albumID:this.gpproxy.getCurrentAlbumId(),itemID:c}}))})}sendResultHandler({detail:{service:a,success:b,albumID:c,itemID:d}}){this.setState(a,b,c,
d);if(c===this.gpproxy.getCurrentAlbumId()){const e=this.buttons[d];e.dataset[a]=b;let h=!0,f=!0;this.s.services.forEach(g=>{e.dataset[g]?"true"!==e.dataset[g]&&(h=!1):f=h=!1});f&&(e.classList.remove("uploading_"),h&&e.classList.add("complete_"))}}setState(a,b,c,d){let e,h;((h=(e=this.state)[c]??(e[c]={}))[d]??(h[d]={}))[a]=b}setInitialButtonState(a,b){const c=this.gpproxy.getCurrentAlbumId();b=this.state[c]?.[b];for(const d in b)a.dataset[d]=b[d];this.s.services.length&&this.s.services.every(d=>
"true"===a.dataset[d])?a.classList.add("complete_"):a.classList.remove("hidden")}sendSocialMediaSettings(){document.dispatchEvent(new CustomEvent("_update_sm_settings",{detail:{settings:JSON.parse(JSON.stringify(this.settings))}}))}toggleSendButtons(a){(a=(a?.detail?.settings||this.settings).services)&&this.gpContent.classList.toggle("gp-sm-hidden_",!a.length)}drawFunction(a,b,c=this.density,d=!1,e=!1){a.lineCap=a.lineJoin="round";a.save();if(b){let n=[];for(var h=0;h<b.length;h++){var f=b[h],g=f[0];
f=(f[1],f.slice(2));if(10!=g){var k=void 0;a.save();if(8!=g){for(k=1;k<f.length;k++)f[k]=[f[k][0]*c,f[k][1]*c];if(2<f.length)switch(g){case 1:g=f[0];a.strokeStyle=g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;k=[f[1][0],f[1][1]];a.beginPath();a.moveTo(...k);for(var l=2;l<f.length;l++)k=[f[l][0],f[l][1]],g=[f[l-1][0],f[l-1][1]],a.quadraticCurveTo(g[0],g[1],g[0]+(k[0]-g[0])/2,g[1]+(k[1]-g[1])/2);a.lineTo(k[0],k[1]);a.stroke();break;case 2:a.strokeStyle="#FFFFFF";a.lineWidth=f[0]*c;k=[f[1][0],
f[1][1]];d&&(a.globalCompositeOperation="destination-out");a.beginPath();a.moveTo(...k);for(l=2;l<f.length;l++)k=[f[l][0],f[l][1]],g=[f[l-1][0],f[l-1][1]],a.quadraticCurveTo(g[0],g[1],g[0]+(k[0]-g[0])/2,g[1]+(k[1]-g[1])/2);a.lineTo(k[0],k[1]);a.stroke();d&&(a.globalCompositeOperation="source-over");break;case 3:g=f[0];a.strokeStyle=g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;a.beginPath();a.moveTo(f[1][0],f[1][1]);a.lineTo(f[2][0],f[2][1]);a.stroke();break;case 4:g=f[0];a.strokeStyle=
g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;a.beginPath();a.rect(f[1][0],f[1][1],f[2][0]-f[1][0],f[2][1]-f[1][1]);a.stroke();break;case 6:g=f[0];a.fillStyle=g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;a.beginPath();a.rect(f[1][0],f[1][1],f[2][0]-f[1][0],f[2][1]-f[1][1]);a.fill();break;case 5:g=f[0];a.strokeStyle=g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;g=(f[2][0]-f[1][0])/2;k=(f[2][1]-f[1][1])/2;l=Math.round(f[1][0]+g);f=Math.round(f[1][1]+k);var m=(Math.sqrt(2)-
1)/3*4;a.beginPath();a.moveTo(l,f-k);a.bezierCurveTo(l+m*g,f-k,l+g,f-m*k,l+g,f);a.bezierCurveTo(l+g,f+m*k,l+m*g,f+k,l,f+k);a.bezierCurveTo(l-m*g,f+k,l-g,f+m*k,l-g,f);a.bezierCurveTo(l-g,f-m*k,l-m*g,f-k,l,f-k);a.stroke();break;case 7:g=f[0],a.fillStyle=g[0],a.lineWidth=g[1],a.globalAlpha=g[2],a.lineWidth*=c,g=(f[2][0]-f[1][0])/2,k=(f[2][1]-f[1][1])/2,l=Math.round(f[1][0]+g),f=Math.round(f[1][1]+k),m=(Math.sqrt(2)-1)/3*4,a.beginPath(),a.moveTo(l,f-k),a.bezierCurveTo(l+m*g,f-k,l+g,f-m*k,l+g,f),a.bezierCurveTo(l+
g,f+m*k,l+m*g,f+k,l,f+k),a.bezierCurveTo(l-m*g,f+k,l-g,f+m*k,l-g,f),a.bezierCurveTo(l-g,f-m*k,l-m*g,f-k,l,f-k),a.fill()}else if(![6,7,11].includes(g)){switch(g){case 2:a.fillStyle="#FFFFFF";a.lineWidth=f[0]*c;k=f[1];d&&(a.globalCompositeOperation="destination-out");break;case 10:a.fillStyle="#FF0000";a.lineWidth=2*c;a.globalAlpha=1;k=f[1];break;default:k=f[0],a.fillStyle=k[0],a.lineWidth=k[1],a.globalAlpha=k[2],a.lineWidth*=c,k=f[1]}a.beginPath();a.arc(k[0],k[1],a.lineWidth/2,0,2*Math.PI);a.fill();
2==g&&d&&(a.globalCompositeOperation="source-over")}}else{g=f[0];a.fillStyle=g[0];a.globalAlpha=g[1];a.beginPath();for(g=1;g<f.length;g+=5)a.rect(f[g]*c,f[g+1]*c,f[g+2]*c,f[g+3]*c);a.fill()}a.restore()}else e&&n.push(f)}for(b=0;b<n.length;b++){d=n[b];for(e=1;e<d.length;e++)d[e]=[d[e][0]*c,d[e][1]*c];a.strokeStyle="#CC0053";a.lineWidth=2*c;a.globalAlpha=1;e=[d[1][0],d[1][1]];a.beginPath();a.moveTo(...e);for(h=2;h<d.length;h++)e=[d[h][0],d[h][1]],g=[d[h-1][0],d[h-1][1]],a.quadraticCurveTo(g[0],g[1],
g[0]+(e[0]-g[0])/2,g[1]+(e[1]-g[1])/2);a.lineTo(e[0],e[1]);a.stroke()}}a.restore()}}window.GPArtBroadcaster_=GPArtBroadcaster_;class GPAssets_{}
...
e=>{0!==e.button||e.ctrlKey?(1===e.button||0===e.button&&e.ctrlKey)&&this.saveCanvas(this.buildAvatar()):(this.isDragging=!0,this.livePreviewEnabled||(this.clearViewfinderFocus(),this.focusFrame.classList.remove("rendered")),this.viewfinder.addEventListener("pointerup",h=>{this.isDragging=!1;this.updatePreviewHandler(h)},{once:!0}))});this.viewfinder.addEventListener("wheel",e=>{this.updatePreviewHandler(e);!this.livePreviewEnabled&&this.isDragging&&this.clearViewfinderFocus()});this.viewfinder.addEventListener("drop",
...
const f=a.scale;a.scale=function(r,q){c=c.scaleNonUniform(r,q);return f.call(a,r,q)};const g=a.rotate;a.rotate=function(r){c=c.rotate(180*r/Math.PI);return g.call(a,r)};const k=a.translate;a.translate=function(r,q){c=c.translate(r,q);return k.call(a,r,q)};const l=a.transform;a.transform=function(r,q,y,z,A,t){const w=b.createSVGMatrix();w.a=r;w.b=q;w.c=y;w.d=z;w.e=A;w.f=t;c=c.multiply(w);return l.call(a,r,q,y,z,A,t)};const m=a.setTransform;a.setTransform=function(r,q,y,z,A,t){c.a=r;c.b=q;c.c=y;c.d=
z;c.e=A;c.f=t;return m.call(a,r,q,y,z,A,t)};const n=a.resetTransform;a.resetTransform=function(){c=b.createSVGMatrix();d.length=0;return n.call(a)};const p=b.createSVGPoint();a.transformedPoint=function(r,q){p.x=r;p.y=q;return p.matrixTransform(c.inverse())}}setImage(a){this.image=a;this.reset()}setScaleFactor(a){this.scaleFactor=a}mirror(){this.image&&(this.ctx.translate(this.canvas.width,0),this.ctx.scale(-1,1),this.ctx.drawImage(this.image,0,0),this.redraw())}};class GPAlbumObserver_{static MONITOR_INTERVAL=200;static ITEM_TYPE={IMAGE:"image",TEXT:"text",ANIMATION:"animation"};constructor(a){if(GPAlbumObserver_.instance)return GPAlbumObserver_.instance;GPAlbumObserver_.instance=this;this.gpproxy=a;this.isInitialized=!1;this.monitorTimer;this.inObserving=!1;this.nextItemId=null;this.items=[];this.observerConfig={childList:!0,subtree:!0};this.observer=new MutationObserver(this.observerCallback.bind(this));this.handlers=new Set}init(){this.gpproxy.addEventListener("album_rejoin",
...
e=e.slice(m)}var p=g.length;g.forEach(function(r,q){f(a(r,h,!0),q,p)})}var c=Object.keys({open:0,close:1,ping:2,pong:3,message:4,upgrade:5,noop:6}),d={type:"error",data:"parser error"};return{decodePayload:function(e,h,f){if("string"!==typeof e)return b(e,h,f);"function"===typeof h&&(h=null);if(""===e)return d;f="";for(var g,k,l=0,m=e.length;l<m;l++)if(k=e.charAt(l),":"!==k)f+=k;else{if(""===f||f!=(g=Number(f)))return d;k=e.substr(l+1,g);if(f!=k.length)return d;if(k.length)return e=a(k,h,!1),d.type===
...
{c&&3<c.length&&(c=this.parser.decode(c))&&this.wsSendHandler(c)});document.addEventListener("_onmessage_intercept",({detail:{handler:c,e:d}})=>{if(this.wsMsgMiddlewares.size)if("string"!==typeof d.data)c(d);else{var [,e,h]=d.data.match(/4(?:(?:2)|(51-))\[2,(\d+)/)||[];if(void 0!==h&&this.wsMsgMiddlewares.has(+h)){var f=this.wsMsgMiddlewares.get(+h),g=this.parser.decodeString(d.data).data;if(g){var [k,l]=[...f].reduce((m,n)=>n(m),[g,!1]);l?(f=this.encodePacket(k,!!e),d=this.buildMessageEvent(d,f),
...
const c=document.createElement("div");c.className="lang-list_";c.addEventListener("click",d=>{this.langBtn.classList.remove("shown_")});c.addEventListener("pointerup",d=>{if(d.target.classList.contains("btn_")){var e=d.target.dataset.lang,h=e!==this.lang;this.langBtn.dataset.lang=e;h&&(this.setLanguage(e),Array.from(c.children).forEach(f=>{f.classList.toggle("selected_",f===d.target)}),this.langBtn.classList.remove("shown_"))}});Object.entries(GPModulesManager_.LANGUAGES).forEach(([d,e])=>{const h=
document.createElement("div");h.className="btn_";h.title=e;h.dataset.lang=d;c.appendChild(h);d===this.lang&&h.classList.add("selected_")});this.langBtn.appendChild(c);a=document.createElement("div");a.className="title_";a.textContent=this.l.WIN_TITLE;this.container.appendChild(a);this.edition=document.createElement("div");this.edition.className="edition_ hidden";this.edition.addEventListener("click",d=>{d=d.target.dataset.state;if(d===GPModulesManager_.EDITION_STATE.FREE||d===GPModulesManager_.EDITION_STATE.EXPIRED)d=
...
var f=document.createElement("div");f.className="module-title_";f.classList.toggle("not-loaded_",!c.loaded);f.classList.toggle("disabled_",!!c.hiddenSettings||!e);f.title=c.loaded?"":"\u041c\u043e\u0434\u0443\u043b\u044c \u043d\u0435 \u0437\u0430\u0433\u0440\u0443\u0436\u0435\u043d";f.dataset.module=c.name;f.textContent=c.title;h.appendChild(f);f=document.createElement("div");f.className="module-settings_";f.classList.toggle("disabled_",!!c.hiddenSettings||!e);f.dataset.module=c.name;f.textContent=
"\ue92d";h.appendChild(f);f=this.initialState[c.name];e=document.createElement("div");e.className="module-tumbler_";var g=document.createElement("input");g.className="gp-cb_";g.id=`module-${d}_`;g.type="checkbox";g.checked=f;g.toggleAttribute("checked",!!f);g.toggleAttribute("disabled",!!c.required);g.addEventListener("change",k=>{c.required||(k.target.checked&&this.disabledList.has(c.name)?(c.disabled=!1,this.disabledList.delete(c.name)):(c.disabled=!0,this.disabledList.add(c.name)),this.saveSettings(),
this.compareToInitialState())});e.appendChild(g);f=document.createElement("label");f.className="gp-cb-label_";f.setAttribute("for",`module-${d}_`);e.appendChild(f);h.appendChild(e);a.appendChild(h)}});this.settingsBtn.classList.toggle("hidden",!this.settingsManager.hasModulesSettings());this.modulesList.innerHTML="";this.modulesList.appendChild(a)}renderError(a){const b=document.createElement("div");b.className="error_";b.innerHTML=`<div class="error-msg_">${a.message}</div>`;this.modulesList.before(b);
...
!this.isHiddenModule(a)){var {source:b,storage:c,settings:d,ui:e}=this.viewsData.get(a);this.settingsView.innerHTML="";this.settingsView.classList.remove("json-data_");var h=document.createElement("div");h.className="controls_ gp-scrollbar_";this.settingsView.appendChild(h);var f=document.createElement("div");f.className="buttons_";this.settingsView.appendChild(f);var g=document.createElement("div");g.className="load-btn_ btn_ gp-btn_";g.textContent="\ud83e\udc45";g.title=this.l.LOAD_JSON_TTL;g.addEventListener("click",
()=>{this.settingsView.classList.toggle("json-data_");k.focus()});g.addEventListener("animationend",n=>{n.target.classList.remove("failure_");n.target.classList.remove("success_")});f.appendChild(g);var k=document.createElement("input");k.className="json-data-input_ gp-input_";k.placeholder=this.l.JSON_INPUT_PH;k.spellcheck=!1;k.autocomplete="off";k.addEventListener("input",n=>{this.jsonDataInputHandler(a,n.target,g)});k.addEventListener("focus",n=>{document.dispatchEvent(new CustomEvent("_keys_lock",
...
()=>{this.reset(a)});f.appendChild(l);f=document.createDocumentFragment();for(let n in e){l=e[n]||{};const {type:p,args:r,hidden:q,description:y,text:z}=l;if(this.isLabel(n)){var m=document.createElement("div");m.className=z?"group-label_":"separator_";m.classList.toggle(l.size,!!l.size);m.textContent=z;f.appendChild(m)}else if(p&&!q)if("component"===p)[l]=r,m=document.createElement("div"),m.className="component_",m.appendChild(l()),f.appendChild(m);else if("buttons"===p){const A=document.createElement("div");
A.className="buttons_";[l]=r;l().forEach(t=>{t.classList.add("btn_");A.appendChild(t)});f.appendChild(A)}else if("buttons-g"===p){const A=document.createElement("div");A.className="buttons_";r.forEach(([t,w,u])=>{const x=document.createElement("div");x.className="btn_";x.title=w;x.textContent=t;x.addEventListener("click",u);A.appendChild(x)});f.appendChild(A)}else l=this.buildTooltip(y),f.appendChild(l),l=this.buildLabel(n,y),f.appendChild(l),l=this.buildControl(e,n,d[n],b,c,d),f.appendChild(l)}h.appendChild(f);
this.controls=h;this.selectSettings.value=a}}isLabel(a){return/^_\d+/.test(a)}buildTooltip(a){const b=document.createElement("div");b.className="tooltip_";b.textContent="?";if(!a)return b.classList.add("disabled_"),b;b.addEventListener("pointerenter",c=>{let d=window.gpSettingsTooltip;d||(d=document.createElement("div"),d.className="gp-settings-tooltip_",document.body.appendChild(d),window.gpSettingsTooltip=d);d.textContent=a;const {x:e,y:h,width:f,height:g}=c.target.getBoundingClientRect();({height:c}=
d.getBoundingClientRect());d.style.cssText=`top: ${h-(c-g)/2}px; left: ${e+f+12}px`;window.gpSettingsTooltip=d});b.addEventListener("pointerleave",c=>{window.gpSettingsTooltip&&(window.gpSettingsTooltip.remove(),window.gpSettingsTooltip=null)});return b}buildLabel(a,b){const c=document.createElement("div");c.className="label_";c.title=b||"";c.textContent=a.split(/(?<![A-Z])(?=[A-Z])|(?=[A-Z]\B)/).join(" ");return c}buildControl(a,b,c,d,e,h){const {type:f,data:g,args:k,formatter:l,proxyProp:m,inputTrigger:n}=
a[b];a=document.createElement("div");a.className="control_";let p;switch(f){case "range":const q=document.createElement("div");q.className="preview_ gp-range-preview_";q.textContent=l?l(c):c;const [y,z,A,t]=k;p=document.createElement("input");p.className="gp-range_";p.type="range";p.min=y;p.max=z;p.step=A;p.value=c;p.name=b;t&&(p.style.direction="rtl");p.addEventListener("input",u=>{q.textContent=l?l(Number.parseFloat(u.target.value)):u.target.value});a.appendChild(p);a.appendChild(q);break;case "list":p=
document.createElement("select");p.className="gp-list_";p.autocomplete="off";p.name=b;(m&&h[m]||(k.length?k:[c])).forEach((u,x)=>{const v=document.createElement("option");v.value=u;v.textContent=l?l(u,x):u;p.appendChild(v)});p.value=c;a.appendChild(p);break;case "list-m":case "list-md":var r="list-m"===f?k:c;p=document.createElement("form");p.className="gp-list-m_";p.name=b;r.forEach((u,x)=>{var v=document.createElement("input");v.id=`ctrl-${b}-${x}_`;v.type="checkbox";v.value=u;v.name=b;v.checked=
c.includes(u);p.appendChild(v);v=document.createElement("label");v.textContent=l?l(u,x):u;v.setAttribute("for",`ctrl-${b}-${x}_`);p.appendChild(v)});a.appendChild(p);break;case "list-i":const w=[];p=document.createElement("form");p.className="gp-list-i_";p.name=b;Object.keys(g.icons).forEach((u,x)=>{const v=document.createElement("input");v.id=`ctrl-${b}-${x}_`;v.type="checkbox";v.name=b;v.value=u;v.checked=c===u;v.addEventListener("click",C=>{w.forEach(D=>{D.checked=D===v})});p.appendChild(v);w.push(v);
const B=document.createElement("label");B.className="gp-list-i-icon_";B.title=u.split("-").map(C=>C[0].toUpperCase()+C.slice(1)).join(" ");B.style.backgroundImage=`url("${g.icons[u]}")`;B.setAttribute("for",`ctrl-${b}-${x}_`);p.appendChild(B)});a.appendChild(p);break;case "checkbox":p=document.createElement("div");p.className="gp-tumbler_";r=document.createElement("input");r.id=`ctrl-${b}_`;r.type="checkbox";r.name=b;r.checked=c;p.appendChild(r);r=document.createElement("label");r.setAttribute("for",
...
storage:e,settings:Object.assign(h,q),changedSettings:q}}))});return a}parseValue(a,b){switch(a.type){case "range":return Number.parseFloat(a.value);case "checkbox":return a.form?"list-i"===b?a.value:[...a.form].filter(c=>c.checked).map(c=>c.value):a.checked;default:return a.value}}jsonDataInputHandler(a,b,c){c.classList.remove("failure_");c.classList.remove("success_");try{let d=JSON.parse(b.value);const {settings:e,source:h,storage:f}=this.viewsData.get(a);d=this.filterSettings(d,e);if(Object.keys(d).length){const g=
this.copy(d);Object.assign(e,g);this.renderSettings(a);this.dispatchEvent(new CustomEvent("update",{detail:{source:h,storage:f,settings:e,changedSettings:g}}));c.classList.add("success_")}else c.classList.add("failure_")}catch(d){c.classList.add("failure_")}b.value=""}filterSettings(a,b){for(let c in a)c in b||delete a[c];return a}reset(a){const {settings:b,defaultSettings:c,source:d,storage:e}=this.viewsData.get(a),h=this.copy(c);Object.assign(b,h);for(let f in b)f in h||delete b[f];this.renderSettings(a);
...
null;this.addEventListener("stroke",({detail:{data:f,symmetryStrokesData:g=[]}})=>{this.drawFunction(this.canvasCtx,[f,...g]);this.strokeOptions=[];this.strokeCoords=[];this.symmetryCoords=[];g.length&&(this.symmetryStrokesGroups[f[1]]=f[1],g.map(k=>{this.symmetryStrokesGroups[k[1]]=f[1]}));this.clearStroke();this.history.push(f,...g);this.undos.length&&(this.undos=[]);this.shouldBeSigned()&&this.sign()});this.addEventListener("stroke_started",({detail:{data:f,symmetryStrokesData:g=[]}})=>{const k=
!!g.length,l=f[0];this.gpproxy.getSendQueueSize()&&this.isToolInTime(l)||(this.gpproxy.tool(1,f,k),g.forEach(m=>{this.gpproxy.tool(1,m,k)}))});this.addEventListener("stroke_process",({detail:{data:f,symmetryStrokesData:g=[]}})=>{const k=!!g.length;this.gpproxy.tool(3,f,k);g.forEach(l=>{this.gpproxy.tool(3,l,k)})});this.addEventListener("stroke_ended",({detail:{data:f,symmetryStrokesData:g=[]}})=>{const k=!!g.length,l=k||this.gpproxy.getSendQueueSize()&&this.isToolInTime(f[0])?1:3;this.gpproxy.tool(l,
f,k);g.forEach(m=>{this.gpproxy.tool(l,m,k)})});this.addEventListener("stroke_undo",({detail:{data:f,symmetryStrokesData:g=[]}})=>{const k=!!g.length;this.gpproxy.undo(f[1],k);g.forEach(l=>{this.gpproxy.undo(l[1],k)})});this.addEventListener("stroke_redo",({detail:{data:f,symmetryStrokesData:g=[]}})=>{const k=!!g.length;this.gpproxy.redo(f,k);g.forEach(l=>{this.gpproxy.redo(l,k)})});this.addEventListener("settings_updated",({detail:{settings:f}})=>{this.updateSettings(f);this.saveSettings()});this.gpproxy.addEventListener("localization",
({detail:{data:f,type:g}})=>{"draw"===g&&(this.localization=f)});this.gpproxy.addEventListener("turn_started",()=>{this.terminate();this.resetTurnSettings();this.setTimer()});this.gpproxy.addEventListener("turns_ended",f=>{this.terminate();this.stopTimer()});this.gpproxy.addEventListener("send_queue_updated",({detail:{size:f}})=>{this.iface.updateActionQueueSize(f)});this.iface.addEventListener("settings_updated",({detail:{settings:f}})=>{this.settings=Object.assign(this.settings,f);this.updateSettings(f);
this.saveSettings()});this.iface.addEventListener("bindings_updated",({detail:{bindings:f}})=>{this.updateBindings(f);this.saveBindings()});document.addEventListener("_url_changed",({detail:{path:f}})=>{"/draw"===f&&(this.enabled=this.isPermittedMode(),this.terminate(),this.inject())});document.addEventListener("_keys_lock",({detail:{state:f}})=>{this.setKeysLockState(f)});document.addEventListener("_subs_status",({detail:{status:f}})=>{this.isSubscribed=f});const h=this;Element.prototype.addEventListener=
new Proxy(Element.prototype.addEventListener,{apply(f,g,k){if(!(g.classList?.contains("drawingContainer")&&["mousedown","pointerdown","contextmenu","touchstart"].includes(k[0])&&h.isPermittedMode()))return f.apply(g,k)}});this.timer&&this.timer.addEventListener("finish",f=>{this.timer.settings.readyAfterTimeExpires&&this.gpproxy.isGameTime(GPProxy_.GAME_TIME.HOSTS_DECISION)&&this.gpproxy.isOwner()&&this.iface.readyBtn.click()});this.reference&&this.reference.addEventListener("state_changed",({detail:{isEnabled:f}})=>
...
this.isBrightnessSetting||this.isZooming||this.isHand)){var c=this.getOCoords(a);this.strokeOptions=this.getStrokeOptions(this.tool,b);this.strokeCoords=[c];this.isSymmetryEnabled&&this.convertToSymmetryCoords(c).forEach((d,e)=>{const h=this.strokeOptions.slice();h[1]=this.strokeId++;this.symmetryStrokesOptions[e]=h;this.symmetryCoords[e]=[d]});this.lineSmoothingCounter=1;this.tool===GPPainter_.TOOL.FILL?(this.updateStrokeCoords(this.strokeCoords,this.getCoords(a)),a=this.buildStrokeData(this.strokeOptions,
...
this.isDropperDown=!1;this.drawingArea.classList.remove("dropper-s_");this.stroke.removeEventListener("pointermove",this.dropperMove)}getPixelColor(a,b){var c=document.createElement("canvas");c.width=1;c.height=1;c=c.getContext("2d");c.fillStyle=GPPainter_.BACKGROUND_COLOR;c.fillRect(0,0,1,1);this.backgroundData?.length&&c.drawImage(this.background,a,b,1,1,0,0,1,1);this.lastDrawData?.length&&c.drawImage(this.lastDraw,a,b,1,1,0,0,1,1);c.drawImage(this.canvas,a,b,1,1,0,0,1,1);const [d,e,h,f]=c.getImageData(0,
0,1,1).data;return f?`${this.rgbToHex(d,e,h)}`:GPPainter_.BACKGROUND_COLOR}thicknessKeyDown(a){this.isThicknessSetting=!0;this.renderPointer(this.pointerX,this.pointerY,void 0,void 0,!0);document.addEventListener("keyup",this.thicknessKeyUp,{once:!0})}thicknessKeyUp(a){this.isThicknessSetting&&(this.isThicknessSetting=!1)}thicknessDown(a){if(this.isThicknessSetting&&0===a.button){var [b,c]=this.getCoords(a);this.thicknessMoveHandler=this.thicknessMove(b,c,b,c);this.stroke.setPointerCapture(a.pointerId);
this.stroke.addEventListener("pointermove",this.thicknessMoveHandler);this.stroke.addEventListener("pointerup",d=>{this.stroke.removeEventListener("pointermove",this.thicknessMoveHandler);this.dispatchEvent(new CustomEvent("thickness",{detail:{thickness:this.thickness}}))},{once:!0});this.thicknessMoveHandler(a)}}thicknessMove(a,b,c,d){return e=>{const [h,f]=this.getCoords(e);e=Math.round(Math.max(Math.min(this.thickness+("horizontal"===this.settings.thicknessAxis?h-c:f-d)*("down / right"===this.settings.upThicknessDirection?
1:-1),this.settings.maxThickness),this.settings.minThickness));c=h;d=f;e&&(this.thickness=e,this.iface.updateThickness(e),this.renderPointer(a,b,void 0,void 0,!0))}}opacityKeyDown(a){this.isOpacitySetting=!0;this.renderPointer(this.pointerX,this.pointerY,this.opacity,void 0,!0);document.addEventListener("keyup",this.opacityKeyUp,{once:!0})}opacityKeyUp(a){this.isOpacitySetting&&(this.isOpacitySetting=!1)}opacityDown(a){if(this.isOpacitySetting&&0===a.button){var [b,c]=this.getCoords(a);this.opacityMoveHandler=
this.opacityMove(b,c,b,c);this.stroke.setPointerCapture(a.pointerId);this.stroke.addEventListener("pointermove",this.opacityMoveHandler);this.stroke.addEventListener("pointerup",d=>{this.stroke.removeEventListener("pointermove",this.opacityMoveHandler);this.drawPointer(d,null)},{once:!0});this.opacityMoveHandler(a)}}opacityMove(a,b,c,d){return e=>{const [h,f]=this.getCoords(e);e=Math.max(this.settings.minOpacity,Math.min(Math.round(100*(Number(this.opacity)+-Math.sign(("horizontal"===this.settings.opacityAxis?
h-c:f-d)*("up / left"===this.settings.upOpacityDirection?1:-1))*this.settings.opacityStep))/100,1));c=h;d=f;this.setOpacity(e);this.renderPointer(a,b,e,void 0,!0)}}brightnessKeyDown(a){this.isBrightnessSetting=!0;this.renderPointer(this.pointerX,this.pointerY,this.opacity,void 0,!0);document.addEventListener("keyup",this.brightnessKeyUp,{once:!0})}brightnessKeyUp(a){this.isBrightnessSetting&&(this.isBrightnessSetting=!1)}brightnessDown(a){if(this.isBrightnessSetting&&0===a.button){var [b,c,d]=GPColorUtils_.hexToHsv(this.color);
this.brightness=d;var [e,h]=this.getCoords(a);this.brightnessMoveHandler=this.brightnessMove(e,h,e,h,b,c);this.stroke.setPointerCapture(a.pointerId);this.stroke.addEventListener("pointermove",this.brightnessMoveHandler);this.stroke.addEventListener("pointerup",f=>{this.stroke.removeEventListener("pointermove",this.brightnessMoveHandler);this.drawPointer(f,null);this.dispatchEvent(new CustomEvent("brightness",{detail:{brightness:this.brightness}}))},{once:!0});this.brightnessMoveHandler(a)}}brightnessMove(a,
b,c,d,e,h){return f=>{const [g,k]=this.getCoords(f);f=Math.max(.01,Math.min(1,Math.round(100*(this.brightness+-Math.sign(("horizontal"===this.settings.brightnessAxis?g-c:k-d)*("up / left"===this.settings.upBrightnessDirection?1:-1))*this.settings.brightnessStep))/100));c=g;d=k;this.brightness=f;f=GPColorUtils_.hsvToHex(e,h,f);this.setColor(f);this.renderPointer(a,b,this.opacity,void 0,!0)}}handKeyDown(a){this.isHand=!0;this.drawingArea.classList.add("hand_");document.addEventListener("keyup",this.handKeyUp,
...
this.pointerY,null,this.thickness),this.isZooming=!1)}zoomDown(a){if(this.isZooming&&0===a.button){this.isZoomedToPoint&&this.resetZoom();var [b,c]=this.getCoords(a),d=a.pageX,e=a.pageY;this.zoomMoveHandler=this.zoomMove(b,c,d,e,d,e);this.stroke.setPointerCapture(a.pointerId);this.stroke.addEventListener("pointermove",this.zoomMoveHandler);this.stroke.addEventListener("pointerup",h=>{this.stroke.removeEventListener("pointermove",this.zoomMoveHandler);1>=this.zoomer.getScale()&&this.resetZoom()},{once:!0});
this.zoomMoveHandler(a)}}zoomMove(a,b,c,d,e,h){return f=>{const g=f.pageX;f=f.pageY;const k=("horizontal"===this.settings.zoomAxis?g-e:f-h)*("up / left"===this.settings.zoomInDirection?1:-1);2<Math.abs(k)&&(this.zoomer.zoom({deltaScale:-Math.sign(k),x:c-window.scrollX,y:d-window.scrollY,scaleSensitivity:this.settings.zoomSensitivity}),e=g,h=f);this.renderPointer(a,b,null,1)}}mouseZoom(a){this.isZoomedToPoint&&this.resetZoom();this.zoomer.zoom({deltaScale:-Math.sign(a.deltaY),x:a.pageX-window.scrollX,
...
document.removeEventListener("keyup",c))};document.addEventListener("keyup",c)}else this.isSymmetryEnabled=!0,this.symmetryGuides.classList.add("shown_"),this.iface.updateSymmetryPanel(this.isSymmetryEnabled);var b=c=>{c.code===a.code&&(this.isSymmetryGuidesEditable=this.isSymmetryGuidesMoved=!1,this.symmetryGuides.classList.remove("editable_"),document.removeEventListener("keyup",b))};document.addEventListener("keyup",b)}}symmetryGuidesDownHandler(a){function b(d){const [e,h]=d.touches?[d.touches[0].clientX,
d.touches[0].clientY]:[d.clientX,d.clientY];d=d.target.getBoundingClientRect();let [f,g]=[Math.round((e-d.left)/c),Math.round((h-d.top)/c)];this.isMirrored&&(f=GPPainter_.ORIGINAL_WIDTH-f);this.updateSymmetryGuides(Math.max(0,Math.min(GPPainter_.ORIGINAL_WIDTH,f)),Math.max(0,Math.min(GPPainter_.ORIGINAL_HEIGHT,g)))}if(this.isSymmetryGuidesEditable)if(2===a.button)this.resetSymmetryGuides();else{var c=this.getActualScale();b=b.bind(this);a.target.setPointerCapture(a.pointerId);a.target.addEventListener("pointermove",
...
b=Math.round(b);c=Math.round(c);var p;parseInt(d.replace("#","0x"));!function(){p=[];for(var t=-1;t<=e;t++)p[t]=[];p[-1]=[];p[e]=[];for(t=-1;t<=h;t++)p[-1][t]=1,p[e][t]=1;for(t=0;t<e;t++)p[t][-1]=1,p[t][h]=1}();var r=a.getImageData(0,0,e,h).data;a=4*(b+c*e);a=[r[a],r[a+1],r[a+2],r[a+3]];d=[];for(var q=0;q<=e;q++)d[q]=[];for(;m(b-1,c,a);)b--;for(;m(b,c-1,a);)c--;var y;q=f(a,b,c);c={x:b,y:c,w:q.w,h:q.h,ref:0,andada:0,stack:0};var z=q.w,A=[].concat(n([q.x,q.y,q.w+1,q.h+1]),[0]);do{b=0;for(2==c.ref&&
(b+=c.andada);b<=c.h;)-1!=(y=(q=l(a,c.x+c.w+1,c.y+c.h-b)).h)?(d[y].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:1,andada:c.h+1-b,stack:A.length}),A.push.apply(A,n([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),y>z&&(z=y),b+=y):b++;b=0;for(1==c.ref&&(b+=c.andada);b<=c.h;)-1!=(y=(q=k(a,c.x-1,c.y+b)).h)?(d[y].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:2,andada:c.h+1-b,stack:A.length}),A.push.apply(A,n([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),y>z&&(z=y),b+=y):b++;b=0;for(4==c.ref&&(b+=c.andada);b<=c.w;)-1!=(y=(q=f(a,c.x+b,c.y+
c.h+1)).w)?(d[y].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:3,andada:c.w+1-b,stack:A.length}),A.push.apply(A,n([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),y>z&&(z=y),b+=y):b++;b=0;for(3==c.ref&&(b+=c.andada);b<=c.w;)-1!=(y=(q=g(a,c.x+c.w-b,c.y-1)).w)?(d[y].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:4,andada:c.w+1-b,stack:A.length}),A.push.apply(A,n([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),y>z&&(z=y),b+=y):b++;for(c=d[z].pop();null==c&&0<z;)c=d[--z].pop()}while(null!=c);return A}setTimer(){this.timer&&(this.timer.reset(),
...
d]=this.calculateColorSize(a.length);this.palette.classList.toggle("max-size_",c);this.palette.classList.toggle("empty_",!a.length);this.palette.style.setProperty("--gp-color-size",`${b}px`);this.palette.style.setProperty("--gp-palette-columns-count",`${d}`);this.palette.innerHTML="";a.forEach((e,h)=>{const f=document.createElement("div");f.className="color_";f.dataset.color=`#${e}`;f.style.backgroundColor=`#${e}`;this.isCustomPalette&&(f.draggable=!0,f.dataset.id=h);this.palette.appendChild(f)})}saveCustomPalette(){const {default:a,
...
d=document.createElement("div");d.className="gp-btn_ btn_ default-btn_";d.textContent=this.l.RESET_SETTINGS_BTN_LBL;d.title=this.l.RESET_SETTINGS_BTN_TTL;d.addEventListener("click",this.resetSettings);c.appendChild(d);c=document.createDocumentFragment();d=this.painter.getUISettings();for(let e in d){const {type:h,args:f,description:g,text:k,inputTrigger:l}=this.painter.getUISetting(e)||{};if(e.startsWith("_")){d=document.createElement("div");d.className="group-label_";d.textContent=k;c.appendChild(d);
continue}if(!h)continue;d=document.createElement("div");d.className="label_";d.title=g;d.textContent=e.split(/(?=[A-Z])/).join(" ");let m=document.createElement("div");m.className="control_";let n;switch(h){case "range":const p=document.createElement("div");p.className="preview_ gp-range-preview_";p.textContent=a[e];const [r,q,y]=f;n=document.createElement("input");n.className="gp-range_";n.type=h;n.min=r;n.max=q;n.step=y;n.value=a[e];n.name=e;n.addEventListener("input",t=>{p.textContent=t.target.value});
m.appendChild(n);m.appendChild(p);break;case "list":n=document.createElement("select");n.className="gp-list_";n.name=e;f.forEach(t=>{const w=document.createElement("option");w.value=t;w.textContent=t;n.appendChild(w)});n.value=a[e];m.appendChild(n);break;case "checkbox":n=document.createElement("div");n.className="gp-tumbler_";const z=document.createElement("input");z.id=`ctrl-${e}_`;z.type="checkbox";z.name=e;z.checked=a[e];n.appendChild(z);const A=document.createElement("label");A.for=`ctrl-${e}_`;
A.addEventListener("click",t=>{z.click()});n.appendChild(A);m.appendChild(n);break;case "color":n=document.createElement("input"),n.className="gp-color_",n.type=h,n.value=a[e],n.name=e,m.appendChild(n)}n.addEventListener(l?"input":"change",p=>{this.dispatchEvent(new CustomEvent("settings_updated",{detail:{settings:{[p.target.name]:this.parseSettingsValue(p.target)}}}))});c.appendChild(d);c.appendChild(m)}b.appendChild(c);this.toggleScrollbar(b);b.scrollTop=this.settingsScrollPos}getInitialSettings(){return Object.assign({},
...
b){const c=parseInt(a.substr(1,2),16),d=parseInt(a.substr(3,2),16),e=parseInt(a.substr(5,2),16);this.style.setProperty("--gp-panels-bg-color",`rgb(${c} ${d} ${e} / ${b}%)`);const [h,f,g]=GPColorUtils_.rgbToHsv(c,d,e),[k,l,m]=GPColorUtils_.hsvToRgb(Math.max(h-.04246122552758713,0),Math.max(f-.012523565849717255,0),Math.max(g-.05882352941176472,0));this.style.setProperty("--gp-panels-elems2-color",`rgb(${k}, ${l}, ${m})`);this.painter.isDisabled&&(this.updateColorInput(a),this.updateColor2Input(a))}updatePanelsElements(a,
...
d=c.dataTransfer.items[0].getAsFile():c.dataTransfer.files.length&&(d=c.dataTransfer.files[0]);if(d&&GPReference_.IMAGE_TYPES.has(d.type)){const e=new FileReader;e.addEventListener("loadend",h=>{this.showPreview(e.result,0,0,{query:"file",type:d.type})});e.readAsDataURL(d)}},!0);a.addEventListener("dragover",c=>{c.preventDefault();c.dataTransfer.dropEffect="move"},!0);a=document.createElement("div");a.className="wrapper_";this.container.appendChild(a);var b=document.createElement("div");b.className=
...
Array(this.columnsCount).fill(0),h=Array(this.columnsCount).fill().map(()=>document.createDocumentFragment());if(b)for(this.results.innerHTML="",this.results.scrollTop=0,this.results.dataset.query=c,this.columns=[],b=0;b<this.columnsCount;b++){const f=document.createElement("div");f.className="column_";f.dataset.index=`${b}`;this.columns[b]=f;this.results.appendChild(f)}else e.map((f,g)=>+this.columns[g].dataset.height);a.forEach(f=>{var g=f.image.width/f.image.height;const k=document.createElement("div");
k.className="preview_";k.style.backgroundColor=f.dominantColor;k.dataset.url=f.image.url;k.dataset.previewURL=f.preview.url;k.dataset.width=f.image.width;k.dataset.height=f.image.height;k.dataset.aspectRatio=g;const l=new Image;l.width=g*GPReference_.PREVIEW_MAX_HEIGHT;l.height=GPReference_.PREVIEW_MAX_HEIGHT;l.dataset.src=f.preview.url;k.appendChild(l);this.imageObserver.observe(l);g=this.getMinHeightColumnIndex(e);h[g].appendChild(k);e[g]+=f.image.height/f.image.width});this.columns.forEach((f,
g)=>{f.dataset.height=e[g];f.appendChild(h[g])});a=!!this.results.getBoundingClientRect().height;void 0!==d&&setTimeout(()=>{this.results.scrollTop=d},a?0:500);a&&setTimeout(()=>{this.isNextPageNeeded()&&this.searchImages(c,!0)},a?0:500)}}getMinHeightColumnIndex(a){return a.reduce((b,c,d,e)=>c<e[b]?d:b,0)}isNextPageNeeded(){return!this.isLastPage&&!this.pendingSearch&&this.columns.some(a=>{const b=this.results.scrollTop,c=this.results.getBoundingClientRect().height;a=a.getBoundingClientRect().height;
...
this.history.get(this.image.metadata.url);a&&(a.pos=this.zoomer.getState())}renderHistory(){this.historyList.innerHTML="";const a=document.createDocumentFragment();Array.from(this.history).reverse().forEach(([b,{width:c,height:d,query:e}],h)=>{const f=document.createElement("div");f.className="history-item_";f.dataset.url=b;f.dataset.width=c;f.dataset.height=d;f.dataset.query=e;b=document.createElement("div");b.className="index_";b.textContent=`${this.history.size-h}.`;f.appendChild(b);h=document.createElement("div");
...
e&&b.length>h)if(e=this.getCache(),!this.cache[this.density]&&b.at(-1).index+1<h)this.drawFunction(a,b,c,d);else{const f=Math.floor((b.at(-1).index+1)/h)-1;a.putImageData(e[f],0,0);b=b.slice(-(b.at(-1).index+1-(f+1)*h));this.drawFunction(a,b,c,d)}else this.drawFunction(a,b,c,d)}}drawFunction(a,b,c=this.density,d=!0,e=!1){a.lineCap=a.lineJoin="round";a.save();if(b){let n=[];for(var h=0;h<b.length;h++){var f=b[h],g=f[0];f=(f[1],f.slice(2));if(10!=g){var k=void 0;a.save();if(8!=g){for(k=1;k<f.length;k++)f[k]=
[f[k][0]*c,f[k][1]*c];if(2<f.length)switch(g){case 1:g=f[0];a.strokeStyle=g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;k=[f[1][0],f[1][1]];a.beginPath();a.moveTo(...k);for(var l=2;l<f.length;l++)k=[f[l][0],f[l][1]],g=[f[l-1][0],f[l-1][1]],a.quadraticCurveTo(g[0],g[1],g[0]+(k[0]-g[0])/2,g[1]+(k[1]-g[1])/2);a.lineTo(k[0],k[1]);a.stroke();break;case 2:a.strokeStyle="#FFFFFF";a.lineWidth=f[0]*c;k=[f[1][0],f[1][1]];d&&(a.globalCompositeOperation="destination-out");a.beginPath();a.moveTo(...k);
for(l=2;l<f.length;l++)k=[f[l][0],f[l][1]],g=[f[l-1][0],f[l-1][1]],a.quadraticCurveTo(g[0],g[1],g[0]+(k[0]-g[0])/2,g[1]+(k[1]-g[1])/2);a.lineTo(k[0],k[1]);a.stroke();d&&(a.globalCompositeOperation="source-over");break;case 3:g=f[0];a.strokeStyle=g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;a.beginPath();a.moveTo(f[1][0],f[1][1]);a.lineTo(f[2][0],f[2][1]);a.stroke();break;case 4:g=f[0];a.strokeStyle=g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;a.beginPath();a.rect(f[1][0],f[1][1],
f[2][0]-f[1][0],f[2][1]-f[1][1]);a.stroke();break;case 6:g=f[0];a.fillStyle=g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;a.beginPath();a.rect(f[1][0],f[1][1],f[2][0]-f[1][0],f[2][1]-f[1][1]);a.fill();break;case 5:g=f[0];a.strokeStyle=g[0];a.lineWidth=g[1];a.globalAlpha=g[2];a.lineWidth*=c;g=(f[2][0]-f[1][0])/2;k=(f[2][1]-f[1][1])/2;l=Math.round(f[1][0]+g);f=Math.round(f[1][1]+k);var m=(Math.sqrt(2)-1)/3*4;a.beginPath();a.moveTo(l,f-k);a.bezierCurveTo(l+m*g,f-k,l+g,f-m*k,l+g,f);a.bezierCurveTo(l+
g,f+m*k,l+m*g,f+k,l,f+k);a.bezierCurveTo(l-m*g,f+k,l-g,f+m*k,l-g,f);a.bezierCurveTo(l-g,f-m*k,l-m*g,f-k,l,f-k);a.stroke();break;case 7:g=f[0],a.fillStyle=g[0],a.lineWidth=g[1],a.globalAlpha=g[2],a.lineWidth*=c,g=(f[2][0]-f[1][0])/2,k=(f[2][1]-f[1][1])/2,l=Math.round(f[1][0]+g),f=Math.round(f[1][1]+k),m=(Math.sqrt(2)-1)/3*4,a.beginPath(),a.moveTo(l,f-k),a.bezierCurveTo(l+m*g,f-k,l+g,f-m*k,l+g,f),a.bezierCurveTo(l+g,f+m*k,l+m*g,f+k,l,f+k),a.bezierCurveTo(l-m*g,f+k,l-g,f+m*k,l-g,f),a.bezierCurveTo(l-
g,f-m*k,l-m*g,f-k,l,f-k),a.fill()}else if(![6,7,11].includes(g)){switch(g){case 2:a.fillStyle="#FFFFFF";a.lineWidth=f[0]*c;k=f[1];d&&(a.globalCompositeOperation="destination-out");break;case 10:a.fillStyle="#FF0000";a.lineWidth=2*c;a.globalAlpha=1;k=f[1];break;default:k=f[0],a.fillStyle=k[0],a.lineWidth=k[1],a.globalAlpha=k[2],a.lineWidth*=c,k=f[1]}a.beginPath();a.arc(k[0],k[1],a.lineWidth/2,0,2*Math.PI);a.fill();2==g&&d&&(a.globalCompositeOperation="source-over")}}else{g=f[0];a.fillStyle=g[0];a.globalAlpha=
g[1];a.beginPath();for(g=1;g<f.length;g+=this.fi)a.rect(f[g]*c,f[g+1]*c,f[g+2]*c,f[g+3]*c);a.fill()}a.restore()}else e&&n.push(f)}for(b=0;b<n.length;b++){d=n[b];for(e=1;e<d.length;e++)d[e]=[d[e][0]*c,d[e][1]*c];a.strokeStyle="#CC0053";a.lineWidth=2*c;a.globalAlpha=1;e=[d[1][0],d[1][1]];a.beginPath();a.moveTo(...e);for(h=2;h<d.length;h++)e=[d[h][0],d[h][1]],g=[d[h-1][0],d[h-1][1]],a.quadraticCurveTo(g[0],g[1],g[0]+(e[0]-g[0])/2,g[1]+(e[1]-g[1])/2);a.lineTo(e[0],e[1]);a.stroke()}}a.restore()}handleDelimiters(a,
b){const c=a.findLastIndex(d=>11===d[0]);if(~c){const d=a.slice(c+1);a=(b??[]).concat(a.slice(0,c).filter(e=>11!==e[0]));return[d,a]}return[a,b]}initCacheWorker(){let {width:a,height:b}=getComputedStyle(this.container);a=parseInt(a);b=parseInt(b);var {density:c}=this.getDensityByDimensions(a,b);let {width:d,height:e}=getComputedStyle(this.mainContainer);d=parseInt(d);e=parseInt(e);const {density:h}=this.getDensityByDimensions(d,e),{density:f}=this.getDensityByDimensions(window.screen.width,window.screen.height),
g=[...(new Set([c,h,f]))];this.worker||(c=URL.createObjectURL(new Blob([this.getFuncBody(this.cacheWorkerBody)])),this.worker=new Worker(c),URL.revokeObjectURL(c),this.worker.addEventListener("message",({data:k})=>{"worker-ready"===k.type?g.forEach(l=>{this.worker.postMessage({type:"get-cache",density:l})}):"cache-updated"===k.type&&(this.cache=k.cache,this.dispatchEvent(new CustomEvent("cache-updated",{detail:{density:k.density}})))}));this.worker.postMessage({type:"init",data:this.data,background:this.background,
draw:this.getFuncBody(this.drawFunction),fi:this.fi,CACHE_INTERVAL:GPTimelapsePlayer_.CACHE_INTERVAL,ORIGINAL_WIDTH:GPTimelapsePlayer_.ORIGINAL_WIDTH,ORIGINAL_HEIGHT:GPTimelapsePlayer_.ORIGINAL_HEIGHT,BACKGROUND_COLOR:GPTimelapsePlayer_.BACKGROUND_COLOR})}cacheWorkerBody(){function a(h){var f=b*h,g=c*h;if(!this.cache[h]){this.cache[h]=[];const l=e,m=Math.floor(self.data.length/l);f=new OffscreenCanvas(f,g);g=f.getContext("2d",{willReadFrequently:!0});let n;self.background&&(g.fillStyle=d,g.fillRect(0,
0,f.width,f.height),self.draw(g,this.background,h,!1),n=g.getImageData(0,0,f.width,f.height),g.clearRect(0,0,f.width,f.height));for(let p=0;p<m;p++){var k=p*l;k=self.data.slice(k,k+l);self.draw(g,k,h);k=g.getImageData(0,0,f.width,f.height);this.cache[h].push(k)}self.background&&this.cache[h].push(n)}}let b,c,d,e;self.data;self.draw;self.cache=[];self.addEventListener("message",({data:h})=>{"init"===h.type?(self.data=h.data,self.background=h.background,self.fi=h.fi,self.draw=new Function("a","l","f",
...
e=h.CACHE_INTERVAL,b=h.ORIGINAL_WIDTH,c=h.ORIGINAL_HEIGHT,d=h.BACKGROUND_COLOR,self.postMessage({type:"worker-ready"})):"get-cache"===h.type&&(self.cache[h.density]||a(h.density),self.postMessage({type:"cache-updated",cache:self.cache,density:h.density}))})}getFuncBody(a){a=a.toString();return a.substring(a.indexOf("{")+1,a.lastIndexOf("}"))}}window.GPTimelapsePlayer_=GPTimelapsePlayer_;class GPTimer_ extends EventTarget{static STARTED="started";static PAUSED="paused";static STOPPED="stopped";static SETTING="setting";static SOUND_ALERT_URL="https://garticphone.com/sounds/timeout.mp3";static SETTING_MINUTE_STEP=5;static POINTER_TIMER_CHANGE_SENSITIVITY=Math.round(window.screen.height/1080*15);static DEFAULT_SETTINGS={reverted:!0,delimiterFlicker:!0,soundAlert:!0,soundAlertVolume:1,runAfterTurnStart:!1,readyAfterTimeExpires:!1,defaultTimer:"1800000",timers:[]};static SETTINGS_UI={reverted:{type:"checkbox",
description:"\u0412\u043a\u043b\u044e\u0447\u0430\u0435\u0442 \u043e\u0442\u043e\u0431\u0440\u0430\u0436\u0435\u043d\u0438\u0435 \u043e\u0442\u0441\u0447\u0451\u0442\u0430 \u0432\u0440\u0435\u043c\u0435\u043d\u0438 \u0432 \u043e\u0431\u0440\u0430\u0442\u043d\u043e\u043c \u043d\u0430\u043f\u0440\u0430\u0432\u043b\u0435\u043d\u0438\u0438"},delimiterFlicker:{type:"checkbox",description:"\u0412\u043a\u043b\u044e\u0447\u0430\u0435\u0442 \u043c\u0435\u0440\u0446\u0430\u043d\u0438\u0435 \u0440\u0430\u0437\u0434\u0435\u043b\u0438\u0442\u0435\u043b\u044f \u0447\u0430\u0441\u043e\u0432 \u0438 \u043c\u0438\u043d\u0443\u0442"},
_2:{text:"Sound Alert"},soundAlert:{type:"checkbox",description:"\u0417\u0432\u0443\u043a\u043e\u0432\u043e\u0435 \u043e\u043f\u043e\u0432\u0435\u0449\u0435\u043d\u0438\u0435, \u0441\u0438\u0433\u043d\u0430\u043b\u0438\u0437\u0438\u0440\u0443\u044e\u0449\u0435\u0435 \u043e \u0437\u0430\u0432\u0435\u0440\u0448\u0435\u043d\u0438\u0438 \u0432\u0440\u0435\u043c\u0435\u043d\u0438"},soundAlertVolume:{type:"range",args:[0,1,.01],formatter:this.volumeFormatter,description:"\u0423\u0440\u043e\u0432\u0435\u043d\u044c \u0433\u0440\u043e\u043c\u043a\u043e\u0441\u0442\u0438 \u0437\u0432\u0443\u043a\u043e\u0432\u043e\u0433\u043e \u043e\u043f\u043e\u0432\u0435\u0449\u0435\u043d\u0438\u044f"},
_3:{text:"Triggers"},runAfterTurnStart:{type:"checkbox",description:"\u0417\u0430\u043f\u0443\u0441\u043a\u0430\u0442\u044c \u0442\u0430\u0439\u043c\u0435\u0440 \u0430\u0432\u0442\u043e\u043c\u0430\u0442\u0438\u0447\u0435\u0441\u043a\u0438 \u043f\u043e\u0441\u043b\u0435 \u043d\u0430\u0447\u0430\u043b\u0430 \u0440\u0430\u0443\u043d\u0434\u0430"},readyAfterTimeExpires:{type:"checkbox",description:"\u041d\u0430\u0436\u0438\u043c\u0430\u0442\u044c \u043a\u043d\u043e\u043f\u043a\u0443 \u0433\u043e\u0442\u043e\u0432\u043d\u043e\u0441\u0442\u0438 \u043f\u043e\u0441\u043b\u0435 \u0438\u0441\u0442\u0435\u0447\u0435\u043d\u0438\u044f \u0432\u0440\u0435\u043c\u0435\u043d\u0438"},
_4:{text:"Timers"},defaultTimer:{type:"list",args:GPTimer_.DEFAULT_SETTINGS.timers,proxyProp:"timers",formatter:this.timeFormatter,description:"\u0422\u0430\u0439\u043c\u0435\u0440 \u043f\u043e \u0443\u043c\u043e\u043b\u0447\u0430\u043d\u0438\u044e (\u0443\u0441\u0442\u0430\u043d\u0430\u0432\u043b\u0438\u0432\u0430\u0435\u0442\u0441\u044f \u043f\u0440\u0438 \u0432\u0445\u043e\u0434\u0435 \u043d\u0430 \u0441\u0430\u0439\u0442)"},timers:{type:"list-md",formatter:this.timeFormatter,description:"\u0421\u043f\u0438\u0441\u043e\u043a \u0441\u043e\u0445\u0440\u0430\u043d\u0451\u043d\u043d\u044b\u0445 \u0442\u0430\u0439\u043c\u0435\u0440\u043e\u0432"}};static timeFormatter(a){return`${Math.floor(a/
...
"86400000","604800000","2678400000"];static NEW_ACCOUNT_PERIODS_ALIASES=["Hour","Day","Week","Month"];static ALBUM_ITEM_TYPE={IMAGE:"image",TEXT:"text",ANIMATION:"animation"};static CENSORING_REASON={BANNED_PLAYER:"banned_player",BACKGROUND:"background",UNKNOWN_PLAYER:"unknown_player",NEW_ACCOUNT:"new_account",FORGERY:"forgery",BY_USER:"by_user",ANIMATION_FRAME:"animation_frame"};static PREVIEW_PATHS=["write","memory"];static PREVIEW_MONITOR_INTERVAL=200;static PREVIEW_MONITOR_TIMEOUT=2E3;static DEFAULT_SETTINGS={showUI:!1,
autoKick:!0,autoKickNewAccounts:!1,copyUsernamesWithClick:!1,compactMode:!0,playersColorIndication:!0,censorDrawings:!0,censorSentences:!0,censorNicknames:!1,censorUnknownPlayers:!1,censorNewAccounts:!1,newAccountPeriod:"86400000",previewEnabled:!1,previewPanelPos:{x:0,y:0,o:"br"},previewPanelWidth:280,banTwitchUsers:!1,twitchToken:"",sendBanReports:!0};static SETTINGS_UI={autoKick:{type:"checkbox",description:"\u0412\u044b\u0433\u043e\u043d\u044f\u0442\u044c \u0438\u0437 \u043b\u043e\u0431\u0431\u0438 \u0438\u0433\u0440\u043e\u043a\u043e\u0432 \u0438\u0437 \u0441\u043f\u0438\u0441\u043a\u0430 \u0437\u0430\u0431\u0430\u043d\u0435\u043d\u043d\u044b\u0445"},
autoKickNewAccounts:{type:"checkbox",description:"\u0412\u044b\u0433\u043e\u043d\u044f\u0442\u044c \u0438\u0437 \u043b\u043e\u0431\u0431\u0438 \u0438\u0433\u0440\u043e\u043a\u043e\u0432 \u0441 \u043d\u043e\u0432\u044b\u043c\u0438 \u0430\u043a\u043a\u0430\u0443\u043d\u0442\u0430\u043c\u0438 (Twitch)"},copyUsernamesWithClick:{type:"checkbox",description:"\u041a\u043e\u043f\u0438\u0440\u043e\u0432\u0430\u0442\u044c \u0438\u043c\u044f \u043f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u0442\u0435\u043b\u0438 \u043f\u0440\u0438 \u043d\u0430\u0436\u0430\u0442\u0438\u0438 \u043d\u0430 \u0435\u0433\u043e \u043d\u0438\u043a \u0432 \u0430\u043b\u044c\u0431\u043e\u043c\u0435 \u0438\u043b\u0438 \u0441\u043f\u0438\u0441\u043a\u0435 \u0438\u0433\u0440\u043e\u043a\u043e\u0432"},
_1:{text:"Interface"},compactMode:{type:"checkbox",description:"\u041a\u043e\u043c\u043f\u0430\u043a\u0442\u043d\u044b\u0439 \u0440\u0435\u0436\u0438\u043c \u043e\u0442\u043e\u0431\u0440\u0430\u0436\u0435\u043d\u0438\u044f \u043f\u0430\u043d\u0435\u043b\u0438 \u0441\u043f\u0438\u0441\u043a\u0430 \u0438\u0433\u0440\u043e\u043a\u043e\u0432"},playersColorIndication:{type:"checkbox",description:"\u041e\u0442\u043e\u0431\u0440\u0430\u0436\u0430\u0442\u044c \u0446\u0432\u0435\u0442\u043e\u0432\u044b\u0435 \u043e\u0431\u043e\u0437\u043d\u0430\u0447\u0435\u043d\u0438\u044f \u0437\u0430\u0431\u0430\u043d\u0435\u043d\u043d\u044b\u0445, \u043d\u0435\u0438\u0437\u0432\u0435\u0441\u0442\u043d\u044b\u0445 \u0438 \u0438\u0433\u0440\u043e\u043a\u043e\u0432 \u0441 \u043d\u043e\u0432\u044b\u043c\u0438 \u0430\u043a\u043a\u0430\u0443\u043d\u0442\u0430\u043c\u0438 \u0432 \u0441\u043f\u0438\u0441\u043a\u0435 \u0438\u0433\u0440\u043e\u043a\u043e\u0432 \u0440\u043e\u0434\u043d\u043e\u0433\u043e \u0438\u043d\u0442\u0435\u0440\u0444\u0435\u0439\u0441\u0430"},
...
_4:{text:"Twitch Integration"},banTwitchUsers:{type:"checkbox",description:"\u0411\u0430\u043d\u0438\u0442\u044c/\u0440\u0430\u0437\u0431\u0430\u043d\u0438\u0432\u0430\u0442\u044c \u0438\u0433\u0440\u043e\u043a\u043e\u0432 \u043d\u0430 \u0442\u0440\u0430\u043d\u0441\u043b\u044f\u0446\u0438\u0438 Twitch \u043f\u0440\u0438 \u0431\u0430\u043d\u0435/\u0440\u0430\u0437\u0431\u0430\u043d\u0435 \u0432 \u0438\u0433\u0440\u0435"},twitchToken:{type:"input",args:{hidden:!0},description:"Twitch-\u0442\u043e\u043a\u0435\u043d \u0434\u043b\u044f \u0431\u0430\u043d\u0430/\u0440\u0430\u0437\u0431\u0430\u043d\u0430 \u0438\u0433\u0440\u043e\u043a\u043e\u0432 \u043d\u0430 \u0432\u0430\u0448\u0435\u0439 \u0442\u0440\u0430\u043d\u0441\u043b\u044f\u0446\u0438\u0438"},
getTwitchToken:{type:"buttons",args:[this.getTwitchTokenBtn]},_5:{text:"Reports"},sendBanReports:{type:"checkbox",description:"\u041e\u0442\u0441\u044b\u043b\u0430\u0442\u044c \u043e\u0442\u0447\u0451\u0442\u044b, \u0441\u043e\u0434\u0435\u0440\u0436\u0430\u0449\u0438\u0435 \u0438\u043d\u0444\u043e\u0440\u043c\u0430\u0446\u0438\u044e \u043e \u0440\u0438\u0441\u0443\u043d\u043a\u0435 \u0438 \u0430\u0432\u0442\u043e\u0440\u0435, \u043f\u0440\u0438 \u0431\u043b\u043e\u043a\u0438\u0440\u043e\u0432\u043a\u0435 \u0438\u0433\u0440\u043e\u043a\u0430 \u0447\u0435\u0440\u0435\u0437 \u043d\u0430\u0436\u0430\u0442\u0438\u0435 \u043d\u0430 \u0430\u0432\u0430\u0442\u0430\u0440 \u0432 \u0430\u043b\u044c\u0431\u043e\u043c\u0435"}};static newAccountPeriodFormatter(a,
...
super();GPModeration_.instance=this;this.gpproxy=a;this.av=d;this.ac=e;this.abt=h;this.l=f;this.usersDB=new GPKnownUsersDB_;this.s=b.setSettings(this,this.updateSettings);this.list;this.buttons;this.banlist=new Set(JSON.parse(window.localStorage.getItem("ul_banlist"))||[]);this.twitchUserIds=new Map;this.isMiddlewareActive=!1;this.searchPreviewTimer;b=new GPAlbumObserver_(a);b.addHandler(this.handleItem.bind(this));b.addHandler(this.handlePreviewItem.bind(this));this.albumItems=b.getItems();this.playersObserver=
new GPPlayersObserver_(a);this.playersObserver.addHandler(this.handlePlayer.bind(this));this.reportSender=new GPImageReportSender_(GPImageReportSender_.BAN,this.gpproxy,c);this.preview=new GPPreview_(this.gpproxy,this.s.previewEnabled,this.s.previewPanelPos,this.s.previewPanelWidth,this.l);this.preview.addEventListener("status",({detail:{status:g}})=>{this.s.previewEnabled=g});this.preview.addEventListener("panel-pos-changed",({detail:g})=>{this.s.previewPanelPos=g});this.preview.addEventListener("panel-width-changed",
({detail:{width:g}})=>{this.s.previewPanelWidth=g});this.censorItems=new Map;this.unknownUsers=new Set;this.suspiciousItems=this.ac?.getSuspiciousItems()??new Set;this.userDatesCreated={};this.isAlbumBgHidden=!1;a=document.documentElement.style;a.setProperty("--mod-item-banned-player",`'${this.l.ITEM_BANNED_PLAYER}'`);a.setProperty("--mod-item-censored-bg",`'${this.l.ITEM_CENSORED_BG}'`);a.setProperty("--mod-item-unknown-player",`'${this.l.ITEM_UNKNOWN_PLAYER}'`);a.setProperty("--mod-item-new-account",
`'${this.l.ITEM_NEW_ACCOUNT}'`);a.setProperty("--mod-item-censored",`'${this.l.ITEM_CENSORED}'`);this.middleware=this.middleware.bind(this);this.usersClickHandler=this.usersClickHandler.bind(this);this.banlistClickHandler=this.banlistClickHandler.bind(this);this.albumItemClickHandler=this.albumItemClickHandler.bind(this);this.albumItemEnterHandler=this.albumItemEnterHandler.bind(this);this.albumItemLeaveHandler=this.albumItemLeaveHandler.bind(this);this.preventAlbumContentInteraction=this.preventAlbumContentInteraction.bind(this);
this.imagePreviewOverHandler=this.imagePreviewOverHandler.bind(this);this.imagePreviewLeaveHandler=this.imagePreviewLeaveHandler.bind(this);this.imagePreviewDownHandler=this.imagePreviewDownHandler.bind(this);this.handlePreviewImage=this.handlePreviewImage.bind(this);this.addEventListener("user_updated",()=>{this.saveBanlist()});this.addEventListener("user_updated",()=>{this.handleItems()});this.gpproxy.addEventListener("game_rejoin",()=>{this.gpproxy.isAuthorized&&this.parseUnknownUsers()});this.gpproxy.addEventListener("album_rejoin",
()=>{this.gpproxy.isAuthorized&&(this.init(),this.gpproxy.isAuthorized&&(this.gpproxy.addWSMsgMiddleware([GPProxy_.EVENT.ALBUM_ITEM,GPProxy_.EVENT.ALBUM_INFO],this.middleware),this.isMiddlewareActive=!0))});this.gpproxy.addEventListener("data",({detail:{event:g}})=>{this.gpproxy.isAuthorized&&(g===GPProxy_.EVENT.GAME_STARTED?this.parseUnknownUsers():g===GPProxy_.EVENT.GALLERY_SETTINGS?(this.gpproxy.addWSMsgMiddleware([GPProxy_.EVENT.ALBUM_ITEM,GPProxy_.EVENT.ALBUM_INFO],this.middleware),this.isMiddlewareActive=
!0,this.init()):this.isMiddlewareActive&&[GPProxy_.EVENT.NEW_GAME,GPProxy_.EVENT.ALBUM_SCORE,GPProxy_.EVENT.GAME_QUIT,GPProxy_.EVENT.GAME_STARTED].includes(g)&&(this.gpproxy.removeWSMsgMiddleware(this.middleware),this.isMiddlewareActive=!1,this.terminate()))});this.gpproxy.addEventListener("albums_ended",()=>{if(this.gpproxy.isAuthorized&&this.gpproxy.isOwner()){var g=this.gpproxy.getUsers().filter(({nick:k,viewer:l})=>!this.gpproxy.isYou(k)&&!l&&!this.banlist.has(this.formatUsername(k))).map(({nick:k})=>
k);this.usersDB.addUsers(g)}});this.gpproxy.addEventListener("users_updated",({detail:{users:g}})=>{this.clearPlayers(g);this.renderUsers(g);this.requestUserDatesCreated(g)});this.gpproxy.addEventListener("owner_status",()=>{this.updatePlayers()});document.addEventListener("_user_dates_created_updated",({detail:{userDatesCreated:g}})=>{this.updateUserDatesCreatedCache(g);this.renderUsers()});document.addEventListener("_tw_ids_usernames",({detail:{usernamesMap:g}})=>{this.updateUserIdsCache(g)});document.addEventListener("_url_changed",
({detail:{path:g}})=>{this.isPreviewScreen(g)&&(document.addEventListener("pointerover",this.imagePreviewOverHandler),document.addEventListener("pointerdown",this.imagePreviewDownHandler),document.addEventListener("_url_changed",()=>{document.removeEventListener("pointerover",this.imagePreviewOverHandler);document.removeEventListener("pointerdown",this.imagePreviewDownHandler)},{once:!0}),this.searchPreviewImage().then(this.handlePreviewImage))});window.addEventListener("message",g=>{"https://gpmod.github.io"!==
g.origin&&"https://localhost"!==g.origin||"gp-twitch-token"!==g.data?.type||(this.twitchAuthWindow.postMessage({type:"close"},g.origin),this.twitchAuthWindow=null,this.s.twitchToken=g.data.token)});this.ac&&this.ac.addEventListener("trigger",this.acTriggerHandler.bind(this));this.initTwitchTokenBtn();this.render();this.initInitialState();this.s.banTwitchUsers&&this.gpproxy.isTwitchAuth()&&this.validateTwitchToken()}getElement(){return this.container}init(){document.addEventListener("pointerdown",
this.albumItemClickHandler);document.addEventListener("click",this.preventAlbumContentInteraction,!0)}terminate(){this.censorItems.clear();this.unknownUsers.clear();document.removeEventListener("pointerdown",this.albumItemClickHandler);document.addEventListener("click",this.preventAlbumContentInteraction,!0)}initInitialState(){const a=this.gpproxy.getUsers();this.clearPlayers(a);this.renderUsers(a);this.requestUserDatesCreated(a);this.initUsernamesCopying()}initUsernamesCopying(){this.gpproxy.isAuthorized&&
...
this.twitchTokenBtn.textContent="Get Twitch Token";this.twitchTokenBtn.addEventListener("click",a=>{this.twitchAuthWindow&&this.twitchAuthWindow.close();this.twitchAuthWindow=window.open(GPModeration_.TWITCH_AUTH_URL)})}requestTwitchTokenStatus(){return this.s.twitchToken?fetch("https://id.twitch.tv/oauth2/validate",{headers:{Authorization:`OAuth ${this.s.twitchToken}`}}).then(a=>a.ok).catch(()=>!1):Promise.resolve(!1)}validateTwitchToken(a=!0){this.s.twitchToken&&this.requestTwitchTokenStatus().then(b=>
{!b&&a&&this.abt?.addNotice(Object.assign({type:GPAbout_.NOTICE_TYPE.TWITCH_TOKEN,text:this.l.INVALID_TWITCH_TOKEN,onClick:()=>{this.twitchTokenBtn.click()},onClose:()=>{this.s.banTwitchUsers=!1}}))})}banTwitchUser(a){if(a=this.twitchUserIds.get(a)){const b=this.gpproxy.getUser().authId;fetch(`${GPModeration_.TWITCH_BANS_API}?broadcaster_id=${b}&moderator_id=${b}`,{method:"POST",headers:{Authorization:`Bearer ${this.s.twitchToken}`,"Client-Id":GPModeration_.TWITCH_APP_CLIENT_ID,"Content-Type":"application/json"},
body:JSON.stringify({data:{user_id:a,reason:"Gartic Phone"}})}).then(c=>{401===c.status&&this.validateTwitchToken()})}}unbanTwitchUser(a){if(a=this.twitchUserIds.get(a)){const b=this.gpproxy.getUser().authId;fetch(`${GPModeration_.TWITCH_BANS_API}?broadcaster_id=${b}&moderator_id=${b}&user_id=${a}`,{method:"DELETE",headers:{Authorization:`Bearer ${this.s.twitchToken}`,"Client-Id":GPModeration_.TWITCH_APP_CLIENT_ID}}).then(c=>{401===c.status&&this.validateTwitchToken()})}}updateUserIdsCache(a){a.forEach((b,
c)=>{this.twitchUserIds.set(b,c)})}updateSettings({detail:{settings:a}}){"showUI"in a&&(this.container.classList.toggle("shown_",a.showUI),this.container.classList.remove("banlist_"),this.renderUsers(),this.activateBtn.classList.toggle("pressed_",a.showUI));"autoKick"in a&&(this.container.classList.toggle("autokick_",a.autoKick),this.clearPlayers());"copyUsernamesWithClick"in a&&this.initUsernamesCopying();"compactMode"in a&&this.container.classList.toggle("compact_",a.compactMode);"playersColorIndication"in
a&&this.updatePlayers();"newAccountPeriod"in a&&this.renderUsers();["censorSentences","censorNicknames"].some(b=>b in a)&&this.gpproxy.isGameStage(GPProxy_.GAME_STAGE.ALBUM)&&this.handleItems();["censorDrawings","censorUnknownPlayers","censorNewAccounts"].some(b=>b in a)&&(this.gpproxy.isGameStage(GPProxy_.GAME_STAGE.ALBUM)?this.handleItems():this.gpproxy.isGameStage(GPProxy_.GAME_STAGE.GAME)&&this.searchPreviewImage(!0).then(this.handlePreviewImage));"banTwitchUsers"in a&&a.banTwitchUsers&&this.gpproxy.isTwitchAuth()&&
this.validateTwitchToken()}}window.GPModeration_=GPModeration_;
}).call(this)
