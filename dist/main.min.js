...
GPAssets_.ANTICHEAT_SUSPICIOUS_ART_PLACEHOLDER}getTriggers(a,b,c){let d=this.itemsTriggers.get(a);d||(d=this.calcTriggers(b,c),this.itemsTriggers.set(a,d),d.size&&this.isSubscribed&&this.suspiciousItems.add(a));return d}calcTriggers(a,b){const c=new Set;if(!a?.length)return c;const {animate:d,first:o}=this.gpproxy.getGameConfig();let e=0;var g=0;let f=null,h=null,k=0,l=0,m=[],n=[],p=0,r=0,j,zz,zzc=0;for(let q=0;q<a.length;q++){const y=a[q],A=y[0];if(b&&this.isMStroke(y)){const {code:z,author:t}=this.decodeMStroke(y);
z!==this.gpproxy.getGameCode()&&c.add(20);t!==this.formatUsername(b)&&c.add(21)}if(1===A){1===d&&2===y[2][1]&&4===y.length&&e++;const z=y.length-3,[t,w]=y[3];t===f&&3>z?k++:(k>GPAntiCheat_.SEQUENCE_MIN_SIZE&&(p=Math.max(p,k),m.push(k)),k=0);w===h&&3>z?l++:(l>GPAntiCheat_.SEQUENCE_MIN_SIZE&&(r=Math.max(r,l),n.push(l)),l=0);f=t;h=w}else 8===A&&g++;void 0===j&&11!==o&&(8!==A?j=!1:10<y.length-3&&(j=!0));void 0===zz&&11===o&&(8!==A?zz=!1:(zzc++,zzc>=GPAntiCheat_.CM_SEQUENCE_TRIGGER&&(zz=!0)));1!==A&&(f=h=null,k=l=0)}b=e/a.length*100;g=g/a.length*100;a.length>GPAntiCheat_.MIN_STROKES_TRIGGER&&(b>GPAntiCheat_.MIN_PERCENTAGE_TRIGGER||g>GPAntiCheat_.MIN_PERCENTAGE_TRIGGER)&&
c.add(1);(m.length>GPAntiCheat_.SEQUENCES_COUNT_TRIGGER||n.length>GPAntiCheat_.SEQUENCES_COUNT_TRIGGER)&&c.add(2);(p>GPAntiCheat_.MAX_SEQUENCE_TRIGGER||r>GPAntiCheat_.MAX_SEQUENCE_TRIGGER)&&c.add(3);j&&c.add(4);zz&&c.add(5);return c}isForgery(a){return a.has(20)||a.has(21)}isMStroke(a){return!(1===a[0]&&a[3][0]>GPAntiCheat_.CANVAS_WIDTH&&a[3][1]>GPAntiCheat_.CANVAS_HEIGHT)||a[3][0]-GPAntiCheat_.CANVAS_WIDTH-GPAntiCheat_.M_STROKE_OFFSET||a.at(-1)[1]-GPAntiCheat_.CANVAS_HEIGHT-GPAntiCheat_.M_STROKE_OFFSET?!1:!0}decodeMStroke(a){const [b,
