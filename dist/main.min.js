...
y:f,width:g,height:h}=b.getBoundingClientRect();({height:a}=c.getBoundingClientRect());const k=b=f-(a-h)/2,l=window.innerHeight;b+a>l-8&&(b=l-a-8);8>b&&(b=8);c.style.top=`${b}px`;c.style.left=`${e+g+12}px`;c.style.setProperty("--offset-top",`${k-b}px`);return c}}window.GPApp_=GPApp_;class GPUtils_{static CANVAS_TYPE={DEFAULT:1,SQUARE:2};static CANVAS_TYPE_NAME={[this.CANVAS_TYPE.DEFAULT]:"default",[this.CANVAS_TYPE.SQUARE]:"square"};static CANVAS_DIMENSIONS={[this.CANVAS_TYPE.DEFAULT]:[1516,848],[this.CANVAS_TYPE.SQUARE]:[848,848]};static getCanvasTypeName(a){return this.CANVAS_TYPE_NAME[a]??this.CANVAS_TYPE_NAME[this.CANVAS_TYPE.DEFAULT]}static getCanvasDimensions(a){return this.CANVAS_DIMENSIONS[a]??this.CANVAS_DIMENSIONS[this.CANVAS_TYPE.DEFAULT]}static async compressImageData(b,
...
{id:4,name:"GPPainter_"},{id:11,name:"GPGlobalBanlist_",required:!0}],this,c)}const b={author:"liliezzzz",name:"gp-mod",tag:"gpmod",version:"10.5.6",betaFlag:!1,changelogURL:"https://discord.com/channels/929715202600095775/953145081471324270/1486683981905657877",summaryURL:"https://discord.com/channels/929715202600095775/929715202600095778/1476267642649182348",updateNoticeFlag:!0,updateNoticeText:"",discordURL:"https://discord.gg/xxtCmzt8uA",subscribeURL:"https://boosty.to/gpmod"};if(function(){return navigator?.userAgentData?.brands.some(d=>
...
this.canvas.width;e.height=this.canvas.height;d=e.getContext("2d");d.drawImage(this.background,0,0,e.width,e.height);d.drawImage(this.lastDraw,0,0,e.width,e.height);d.drawImage(this.canvas,0,0,e.width,e.height)}else d=this.canvasCtx;return this.getFillRects(d,a,b,c,this.canvas.width,this.canvas.height).map((e,f)=>4>f%5?Math.round(e/this.density):e)}getFillRects(b,c,d,e,f,h){let g;var l=(r,v,x)=>{let z=v,E=x;if(!q(v,x,r))return{w:-1,h:-1};for(;q(z+1,E,r);)z++;let G=z;do for(z=v-1,E++;q(z+1,E,r)&&z+1<=G;)z++;while(z==G);r=--E-x;return{x:v,y:x,w:G-v,h:r}},m=(r,
v,x)=>{let z=v,E=x;if(!q(v,x,r))return{w:-1,h:-1};for(;q(z-1,E,r);)z--;let G=z;do for(z=v+1,E--;q(z-1,E,r)&&z-1>=G;)z--;while(z==G);r=x-++E;return{x:G,y:E,w:v-G,h:r}},n=(r,v,x)=>{let z=v,E=x;if(!q(v,x,r))return{w:-1,h:-1};for(;q(z,E+1,r);)E++;let G=E;do for(E=x-1,z--;q(z,E+1,r)&&E+1<=G;)E++;while(E==G);r=v-++z;return{x:z,y:x,w:r,h:G-x}},p=(r,v,x)=>{let z=v,E=x;if(!q(v,x,r))return{w:-1,h:-1};for(;q(z,E-1,r);)E--;let G=E;do for(E=x+1,z++;q(z,E-1,r)&&E-1>=G;)E--;while(E==G);r=--z-v;return{x:v,y:G,w:r,
h:x-G}};let q=(r,v,x)=>{if(g[r][v])return!1;r=4*(r+v*f);r=B.slice(r,r+4);v=Math.abs(x[1]-r[1]);let z=Math.abs(x[2]-r[2]),E=Math.abs(x[3]-r[3]);return 20>Math.abs(x[0]-r[0])&&20>v&&20>z&&20>E||0==r[3]&&255==x[0]&&255==x[1]&&255==x[2]||0==x[3]&&255==r[0]&&255==r[1]&&255==r[2]};var y=r=>{var v=r[0];let x=r[1],z=v+r[2],E=x+r[3];for(;v<z;v++)for(let G=x;G<E;G++)g[v][G]=!0;return r};c=Math.round(c);d=Math.round(d);e=parseInt(e.replace("#","0x"));var w=e%256,t=(e=Math.floor(e/256))%256,D=e=Math.floor(e/
256);(()=>{g=[];for(var r=-1;r<=f;r++)g[r]=[];g[-1]=[];g[f]=[];for(r=-1;r<=h;r++)g[-1][r]=1,g[f][r]=1;for(r=0;r<f;r++)g[r][-1]=1,g[r][h]=1})();let B=b.getImageData(0,0,f,h).data;b=4*(c+d*f);b=[B[b],B[b+1],B[b+2],B[b+3]];e=[];for(let r=0;r<=f;r++)e[r]=[];if(q(c,d,[D,t,w,255]))return[];for(var A;q(c-1,d,b);)c--;for(;q(c,d-1,b);)d--;w=l(b,c,d);t={x:c,y:d,w:w.w,h:w.h,ref:0,andada:0,stack:0};D=w.w;c=[...y([w.x,w.y,w.w+1,w.h+1]),0];do{d=0;for(2==t.ref&&(d+=t.andada);d<=t.h;)-1!=(A=(w=p(b,t.x+t.w+1,t.y+
t.h-d)).h)?(e[A].push({x:w.x,y:w.y,w:w.w,h:w.h,ref:1,andada:t.h+1-d,stack:c.length}),c.push(...y([w.x,w.y,w.w+1,w.h+1]),t.stack),A>D&&(D=A),d+=A):d++;d=0;for(1==t.ref&&(d+=t.andada);d<=t.h;)-1!=(A=(w=n(b,t.x-1,t.y+d)).h)?(e[A].push({x:w.x,y:w.y,w:w.w,h:w.h,ref:2,andada:t.h+1-d,stack:c.length}),c.push(...y([w.x,w.y,w.w+1,w.h+1]),t.stack),A>D&&(D=A),d+=A):d++;d=0;for(4==t.ref&&(d+=t.andada);d<=t.w;)-1!=(A=(w=l(b,t.x+d,t.y+t.h+1)).w)?(e[A].push({x:w.x,y:w.y,w:w.w,h:w.h,ref:3,andada:t.w+1-d,stack:c.length}),
c.push(...y([w.x,w.y,w.w+1,w.h+1]),t.stack),A>D&&(D=A),d+=A):d++;d=0;for(3==t.ref&&(d+=t.andada);d<=t.w;)-1!=(A=(w=m(b,t.x+t.w-d,t.y-1)).w)?(e[A].push({x:w.x,y:w.y,w:w.w,h:w.h,ref:4,andada:t.w+1-d,stack:c.length}),c.push(...y([w.x,w.y,w.w+1,w.h+1]),t.stack),A>D&&(D=A),d+=A):d++;for(t=e[D].pop();null==t&&0<D;)t=e[--D].pop()}while(null!=t);let C=new Map;l=(r,v,x,z,E)=>{if(!g[r][v]){x=r+x;var G=v+z;if(!(0>x||x>=f||0>G||G>=h))for(z=4*(r+v*f),x=4*(x+G*f),G=0;4>G;G++)if(20<=Math.abs(B[z+G]-B[x+G])){z=r*
h+v;C.has(z)||C.set(z,[r,v,E]);break}}};for(m=0;m<c.length;m+=5){n=c[m];p=c[m+1];y=c[m+2];A=c[m+3];if(n+y<f)for(d=p;d<p+A;d++)l(n+y,d,1,0,m);if(0<n)for(d=p;d<p+A;d++)l(n-1,d,-1,0,m);if(p+A<h)for(d=n;d<n+y;d++)l(d,p+A,0,1,m);if(0<p)for(A=n;A<n+y;A++)l(A,p-1,0,-1,m)}for(let [,[r,v,x]]of C)c.push(r,v,1,1,x);return c}setTimer(){this.timer&&
...
background:b,canvasType:c,auth:d,meta:{author:e,title:f,date:g}},container:h,mainContainer:k},l=GPTimelapsePlayer_.DEFAULT_DISPLAY_MODE,m=!1){super();GPTimelapsePlayer_.players.add(this);this.canvasType=c;this.canvasTypeName=GPUtils_.getCanvasTypeName(c);const [p,r]=GPUtils_.getCanvasDimensions(c);this.density=GPTimelapsePlayer_.CANVAS_DENSITY;this.originalWidth=p;this.originalHeight=r;this.width=this.originalWidth*this.density;this.height=this.originalHeight*this.density;this.originalData=a.slice();
...
`${a}px`}})}setPanelPosition(a,b,c,d){if(!c||!d){const [k,l]=this.panel.getBoundingClientRect();c=k;d=l}const e=document.documentElement.clientWidth,f=document.documentElement.clientHeight,g=Math.max(Math.min(a,e-c-this.panelBorderWidth),this.panelBorderWidth),h=Math.max(Math.min(b,f-d-this.panelBorderWidth),this.panelBorderWidth);this.panel.style.inset="unset";this.panel.dataset.anchor="";b+d/2>f/2?(this.panelOffsetY=Math.round(f-(h+d)),this.panel.style.bottom=`${this.panelOffsetY}px`,this.panel.dataset.anchor+=
"b"):(this.panelOffsetY=Math.round(h),this.panel.style.top=`${this.panelOffsetY}px`,this.panel.dataset.anchor+="t");a+c/2>e/2?(this.panelOffsetX=Math.round(e-(g+c)),this.panel.style.right=`${this.panelOffsetX}px`,this.panel.dataset.anchor+="r"):(this.panelOffsetX=Math.round(g),this.panel.style.left=`${this.panelOffsetX}px`,this.panel.dataset.anchor+="l");this.panelAnchor=this.panel.dataset.anchor}getCoords(a){const [b,c]=a.touches?[a.touches[0].clientX,a.touches[0].clientY]:[a.clientX,a.clientY];
