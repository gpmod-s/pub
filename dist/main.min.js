...
this.resizeHandler.bind(this));document.addEventListener("_gp_notice",({detail:p})=>{this.addNotice(p)});document.addEventListener("_outdated-scripts",({detail:{outdatedScripts:p}})=>{p.size&&(this.printOutdatedScripts(p),this.addNotice({type:GPAbout_.NOTICE_TYPE.SCRIPTS,scripts:p}))});this.render(c,d,e,f,l,m);this.resizeHandler();this.shouldUpdateNoticeBeShown()&&(this.mm.isFirstRun()?this.s.lastNotifiedVersion=this.version:this.showUpdateNotice());this.s.windowsInkWarning&&this.addNotice({type:GPAbout_.NOTICE_TYPE.NORMAL,
...
null:null;document.dispatchEvent(new CustomEvent(this.eventType,{detail:{noticeColor:this.noticeColor,author:p,authorId:w,sender:m,senderId:l,itemData:e,sourceFile:u,service:v,itemId:a,albumId:b,isImage:B,gameCode:this.gpproxy.getGameCode(),gameConfig:d,keyHash:this.keyHash,...c}}))}}}updateTwitchIdsCache(a){a.forEach((b,c)=>{this.twitchIdsCache.set(b,c)})}}window.GPImageReportSender_=GPImageReportSender_;class GPLocalization_{static LANGUAGE={RU:"ru",EN:"en",ES:"es"};static MISSING_STR="N/A";static DEFAULT_DATA={lang:"ru",entries:{}};constructor(a=GPLocalization_.DEFAULT_DATA){this.lang=a.lang;this.entries=this.initEntries(a.entries)}get(a){return this.entries[a]}getLang(){return this.lang}initEntries(a={}){return new Proxy(a,{get:(b,c)=>new Proxy(b[c]??{},{get:(d,e)=>"lang"===e?this.lang:d[e]??GPLocalization_.MISSING_STR})})}};class GPUtils_{static async compressImageData(a,b=!0){const {data:c,background:d,auth:e,meta:f}=a;a={m:{a:f.author,t:f.title,d:(new Date(f.date)).toISOString()},d:c};d&&(a.b=d);e&&(a.a=e);return b?this.wCompressData(a):this.compressData(a)}static async compressData(a){a=JSON.stringify(a);a=this.stringToArrayBuffer(a);a=await this.compressArrayBuffer(a);return this.arrayBufferToBlob(a)}static arrayBufferToBlob(a){return new Blob([a],{type:"octet/stream"})}static async compressArrayBuffer(a){var b=
...
return u=Math[v[16]](u^u>>>16,2246822507)^Math[v[16]](w^w>>>13,3266489909),w=Math[v[16]](w^w>>>16,2246822507)^Math[v[16]](u^u>>>13,3266489909),String(4294967296*(2097151&w)+(u>>>0))}if(this.s.allowAnalytics){var m={};if(this.lm.length){var p=GPModulesManager_.DEFAULT_PERMISSIONS,r=null;try{if(!a||!this.publicKey)throw r=[GPModulesManager_.STATE.KEY],new GPAuthError_;if(!d)if(this.s.twitchAuth){d=1;const [t,v]=await k(this.s.twitchAuth);if(c=t)this.root.gpproxy.setTwitchOAuth(v);else throw this.s.twitchAuth=
null,this.root.abt?.showTwitchOAuthTokenNotice({onButtonClick:()=>{this.requestTwitchToken()}}),new GPAuthError_;}else throw new GPAuthError_;const n=1===d?"t":"d";m=JSON.parse(await g(a,this.publicKey));const q=JSON.parse(await g(m[n],c));m.e&&this.setEditionTitle(m.e,m.m);m.g&&(r=[GPModulesManager_.STATE.GIFT]);if(q.ed){if(q.ed<Date.now())throw r=[GPModulesManager_.STATE.EXPIRED],2&q.p&&(p|=16),new GPAuthError_;r=[GPModulesManager_.STATE.TIME_LEFT,q.ed-Date.now()]}else m.g||(r=[GPModulesManager_.STATE.SUBSCRIPTION]);
r&&this.setEditionState(...r);f.call(this,p|q.p);h.call(this,p|q.p);this.renderModulesList()}catch(n){n instanceof GPAuthError_||n instanceof GPDecryptError_||console.error(n),e?r??=[GPModulesManager_.STATE.SUBSCRIBE]:this.publicKey&&(r??=[GPModulesManager_.STATE.AUTH]),r&&this.setEditionState(...r),f.call(this,p),h.call(this,p)}this.dispatchEvent(new CustomEvent("modules-loaded",{detail:{decryptedData:m,authTypeId:d,isAuthorized:e}}));this.handleSubExpiration(r)}else this.dispatchEvent(new CustomEvent("modules-loaded",
